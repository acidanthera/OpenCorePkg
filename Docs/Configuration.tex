...
In case one uses these sources, for example, \href{https://dortania.github.io}{Dortania}'s
...
Be warned that regardless of the sources used one is required to fully understand every
...
for legacy systems. To run OpenCore on such a legacy system one can install
...
\item \texttt{Above 4G Decoding} or similar enabled in firmware
  ...
  with the same issue are known, consider this option to be first to check if one
  encounters erratic boot failures.
...
\href{https://github.com/LongSoft/UEFITool/blob/master/UEFIPatch/patches.txt}{patching it}
  if no option is available (for sophisticated users only). See
\hyperref[kernelpropsquirks]{VerifyMsrE2}
  notes for more details.
\item \texttt{CSM} (Compatibility Support Module) disabled in firmware settings
  if present. On NVIDIA 6xx/AMD 2xx or older, GOP ROM might be flashed first. Use
...
When debugging sleep issues Power Nap and automatic power off may be disabled,
...
\item
  ...
  \begin{itemize}
    ...
    \item Consumer Ivy Bridge (\texttt{0x0306A9}) as Apple disabled XCPM for Ivy Bridge
      ...
      should be patched to enforce XCPM on these CPUs instead of this option.
...
\item
  ...
  \begin{enumerate}
    ...
    \item Open the firmware image in UEFITool and find \texttt{CFG Lock} unicode string.
      If it is not present, the firmware may not have this option and one should stop.
...
\item
  ...
  \texttt{keepsyms=1} for kext frames broken on these systems.
...
overwrite OpenCore, make sure to enable it if one plans to use them.
...
\item
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{None} --- Avoid hibernation.
...
\item
  ...
  To start using personalised Apple Secure Boot one will have to reinstall the
  operating system or personalise it. Until the operating system is personalised
  one will only be able to load macOS DMG recovery. If DMG recovery is missing,
...
\item
  ...
  Use at user's own risk on boards known to be compatible.
...
\end{lstlisting}
  ...
  \emph{Note 1}: While it may appear obvious, but an external
  method is required to verify \texttt{OpenCore.efi} and \texttt{BOOTx64.efi} for
  secure boot path. For this one is recommended to at least enable UEFI SecureBoot
  with a custom certificate, and sign \texttt{OpenCore.efi} and \texttt{BOOTx64.efi}
  with user's custom key. More details on customising secure boot on modern firmwares
...
\item \label{secureapplesb}
  ...
  \begin{enumerate}
    \tightlist
    \item Just like on T2 Macs one will not be able to install any unsigned kernel
  ...
  it is likely the case. When this happens one can either reinstall the operating
...
\item
  ...
  Simply said, one is required to enable this quirk to be able to reliably
...
\begin{itemize}
  ...
  \item Last released installer images for macOS~10.6 are macOS~10.6.7
    ...
    assuming that one legally owns macOS~10.6. Read \texttt{DIGEST.txt}
...
\begin{enumerate}
\item Enable Apple Secure Boot by setting \texttt{SecureBootModel} if one needs to
...
\item Make sure that \texttt{ScanPolicy} restricts loading from undesired
  ...
  filesystems.
\item Sign all the installed drivers and tools with private key. Do not sign
...
\item Enable UEFI Secure Boot in firmware preferences and install the
  certificate with a private key one owns. Details on how to generate a certificate
  ...
  If one needs to launch option ROMs or decided to use signed Linux drivers
...
incorrect. It is very common to have \texttt{GND} swapped with \texttt{RX}, thus one has
