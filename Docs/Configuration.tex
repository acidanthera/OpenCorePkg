...
\item
  ...
  \begin{itemize}
    ...
    \item Enabling support of a CPU model that is not yet supported by a specific version of macOS which usually are old.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty string\\
  \textbf{Description}: Emulates CPUID and apply \texttt{DummyPowerManagement} on specified macOS version or older.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty string\\
  \textbf{Description}: Emulates CPUID and apply \texttt{DummyPowerManagement} on specified macOS version or newer.
...
\item \label{secureapplesb}
  ...
  is your system volume identifier. On HFS+ installations the manifests should be
...
\item
  ...
  \item Consider using serial port to inspect early kernel boot failures. For this you may
    need \texttt{debug=0x108}, \texttt{serial=5}, and \texttt{msgbuf=1048576} boot arguments.
