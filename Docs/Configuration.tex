...
\item
  ...
  This option overrides the maximum slide instead of \texttt{TOTAL\_SLIDE\_NUM} (256)
  when \texttt{ProvideCustomSlide} is enabled. When generating random slides, this
  option can avoid bad slides that are unavailable and cause boot failure.

  \emph{Note}: The necessity of this quirk is determined by random boot failure when
  \texttt{ProvideCustomSlide} is enabled. Usually the debug log may contain messages
  like \texttt{AAPL: [EB|`LD:LKC] \} Err(0x9)} when \texttt{AppleDebug} is enabled.
