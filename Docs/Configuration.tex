...
\item
  \texttt{DisableExternalAppleGpu}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Disable external GPUs on real Macs.

  This option disables external GPUs on real Macs by setting
  \texttt{FA4CE28D-B62F-4C99-9CC3-6815686E30F9:gpu-power-prefs}
  which cannot be done before \texttt{ExitBootServices}.
