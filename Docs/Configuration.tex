...
\item
  ...
  \emph{Note}: By default, serial logging is disabled. \texttt{serial=VALUE} boot argument should be
  specified for XNU to print logs to the serial port.
  ...
  \emph{Note 3}: For this patch to be correctly applied, both \texttt{Init} and \texttt{Override}
...
\item
  ...
  In order for OpenCore to print logs to the serial port, bit \texttt{3} for \texttt{Target} under section \texttt{Misc->Debug} must be set.
