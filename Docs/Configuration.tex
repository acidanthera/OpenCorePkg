...
\texttt{OC\_STORAGE\_SAFE\_PATH\_MAX} (192 characters including
...
\item
  ...
  \item The Memory attributes table is ignored. \texttt{EfiRuntimeServicesCode}
    ...
    type.
  \item Apple kernel Memory map entry consolidation may work incorrectly for low memory descriptors,
    which are initially marked as
    \href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/i386_vm_init.c#L282}{preallocated}
    by Apple kernel, but then may get consolidated and lose their preallocation status due to a
    \href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/i386_vm_init.c#L339}{bug}.
    Since Apple kernel later
    \href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/pmap.c#L1204}{frees low memory},
    this may result in use-after-free errors and various kinds of kernel panics at boot time.
    The issue was \href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-1699/osfmk/i386/i386_vm_init.c#L450}{fixed}
    in Mac OS X 10.7 kernel. 
  \end{itemize}
...
\item
  ...
  \begin{itemize}
    \tightlist
    \item \texttt{Auto} --- Automatically select one set of icons based on the \texttt{background-color}
      and \texttt{DefaultBackground} colours: \texttt{Acidanthera\textbackslash GoldenGate} for Syrah Black and
      \texttt{Acidanthera\textbackslash Chardonnay} for Light Gray. \texttt{background-color} variable has
      priority.
...
\item
  \texttt{4D1FDA02-38C7-4A6A-9CC6-4BCCA8B30102:background-color}
  \break
  Acidanthera override for \texttt{DefaultBackgroundColor} useful for legacy OS X
  versions, e.g. OS X 10.9, which does not display grey boot screen when
  \texttt{DefaultBackgroundColor} is set, regardless of its value.
\end{itemize}
...
builtin icon set. The chosen icon set may depend on the \texttt{background-color} and
\texttt{DefaultBackgroundColor} variable values. Refer to \texttt{PickerVariant} for more details.
...
\tightlist
  ...
  \item \texttt{OpticalDrive} --- Optical disc drive (128x128).
...
\tightlist
  ...
  should not load this driver, as a firmware driver serving the same purpose is included in OpenDuet.
...
\item
  \texttt{InstallSimpleTextInputEx}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Install builtin \texttt{SimpleTextInputEx} protocol implementation
  based on \texttt{AppleEvent} protocol.

  This option is intended for old Macs and other systems
  that do not have the \texttt{SimpleTextInputEx} protocol in their firmware.
  Without this option some UEFI Shell commands, such as \texttt{edit}, would not work.

  \emph{Note}: This option requires available \texttt{AppleEvent} protocol instance
  and does not replace existing instances of \texttt{SimpleTextInputEx}.
...
\begin{itemize}
  ...
  \item Last released installer image for Mac OS X~10.5 is Mac OS X~10.5.7
    build \texttt{9J3050} (for \texttt{MacBookPro5,3}). The original \texttt{9J3050} image can be found
    ...
    for details. This image is limited to \texttt{MacBookPro5,2}, \texttt{MacBookPro5,3},
    \texttt{MacBookPro5,4}, \texttt{MacBookPro5,5} model identifiers.
    To fix compatibility with other models one can hex-edit the raw installer image replacing
    the logic of \texttt{/usr/libexec/cpu\_check} script. For example, it can be changed
    to always return \texttt{0} by altering the ternary operator of the \texttt{exit} function call:
\begin{lstlisting}[label=hex105, style=ocbash]
...
\begin{itemize}
  ...
  \item Due to lack of \texttt{createinstallmedia} utility and because the DMG installer
...
\begin{lstlisting}[label=convertdmg, style=ocbash]
...
\end{lstlisting}
    ...
    It will be necessary to replace \texttt{\{X\}} and \texttt{\{Y\}} with disk
    ...
    format with the help of \texttt{qemu-img} utility.
\end{itemize}
