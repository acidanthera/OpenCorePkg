...
\item
  ...
  \begin{enumerate}
    ...
    via UEFI Shell) or in \texttt{boot-args} variable override any compatibility
    checks and force the specified architecture.
    \item Determined EfiBoot version restricts architecture choice:
      \begin{itemize}
      \item 10.4-10.5 --- \texttt{i386}
      \item 10.6-10.7 --- \texttt{i386} or \texttt{x86\_64}
...
\item
  ...
  \emph{Note}: You will have to reinstall the operating system or use macOS recovery
  after setting this value to non-zero. Installing the operating system with
  \texttt{ApECID} value set to non-zero is only possible through macOS recovery.
