...
\item
  ...
  The patch changes the PMIO register base address from the default range \texttt{0x3F8-0x3FF}
  to an actucal one retrieved by \texttt{GetSerialRegisterBase} function call, such that the
  serial device will function correctly.

  \emph{Note}: In addition to this patch, kext \texttt{Apple16X50PCI0} should be prevented from attaching
  to have \texttt{kprintf} working properly. This can be achieved by setting (i.e. \texttt{Delete}, then \texttt{Add})
  the \texttt{class-code} property of the PCIe serial port device to \texttt{FFFFFFFF} in \texttt{DeviceProperties} section.
  ...
  \emph{Note 2}: For this patch to be correctly applied, \texttt{Init} and \texttt{Override}
  must be enabled with all keys properly set in \texttt{Custom}, under section \texttt{Misc->Serial}.

  \emph{Note 3}: Since this patch does not support MMIO, \texttt{UseMmio} under section \texttt{Misc->Serial->Custom} must therefore be disabled.

  \emph{Note 4}: The serial baud rate must be correctly set in both \texttt{BaudRate} under section \texttt{Misc->Serial->Custom} and via
  \texttt{serialbaud=VALUE} boot argument, both of which should match against each other.
...
\item
  ...
  \texttt{Note}: This option can be set with the help of
  \href{https://github.com/acidanthera/OpenCorePkg/tree/master/Utilities/FindSerial}{\texttt{FindSerial.sh}}.
...
\item
  ...
  \item For macOS to correctly recognise and set up serial ports, the \texttt{CustomPciSerialDevice} quirk
    may be enabled when a PCIe serial port card is installed.
  ...
  \item In lieu of the \texttt{msgbuf} boot argument, \href{https://github.com/acidanthera/DebugEnhancer}{DebugEnhancer}
    can be a better solution since this argument no longer works on macOS 12 and above.
