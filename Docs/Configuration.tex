...
\item
  ...
  to an actucal one retrieved by \texttt{GetSerialRegisterBase} function call, such that
  the real PMIO address of the serial device will also be recognised by XNU.
  ...
  \emph{Note 2}: For this patch to be correctly applied, both \texttt{Init} and \texttt{Override}
  must be enabled with all keys properly set in \texttt{Custom}, under section \texttt{Misc->Serial}.

  \emph{Note 3}: Since this patch does not provide MMIO support, \texttt{UseMmio} under section \texttt{Misc->Serial->Custom} must be disabled.
...
\item
  ...
  \href{https://github.com/acidanthera/OpenCorePkg/tree/master/Utilities/FindSerial}{\texttt{FindSerial}} tool.
