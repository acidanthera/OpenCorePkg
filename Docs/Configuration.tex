...
\begin{enumerate}
  ...
  \item \texttt{Builtin} ---  Use OpenCore's updated re-implementation of the Apple Event protocol.
  Using this setting is recommended even on Apple hardware where the OEM version can be used, due to
  ...
  implementation being used on most (except the oldest) Apple systems. It results in no keyboard or mouse support
  otherwise. On all Apple hardware where a recent enough version of the OEM protocol is available -- whether
  connected automatically by Apple's firmware or not -- this option will work reliably; nevertheless for
  the reasons stated \texttt{Builtin} is recommended in preference to this option in most use cases.
  \end{itemize}
...
\item
  ...
  \emph{Note 1}: When using \texttt{KeySupport}, you may find that you get an additional slow key
  repeat before normal speed key repeat starts. If so, the initial key repeat delay is being
  ...
  \emph{Note 2}: On systems using \texttt{KeySupport}, the recommended value is \texttt{0}.
  On other systems the recommended value is \texttt{50}, which may optionally
...
\item
  ...
  \emph{Note 1}: When using \texttt{KeySupport}, you may find that you get one additional slow key repeat
  ...
  \emph{Note 2}: On systems using \texttt{KeySupport}, the recommended setting is to use the same
  value as \texttt{KeyForgetThreshold}. This value may then be tuned upwards for slower key repeat
  if desired, but not downwards.
  On systems not using \texttt{KeySupport} the recommended starting value is \texttt{5},
  and this may optionally be tuned up or down according to user preference.
