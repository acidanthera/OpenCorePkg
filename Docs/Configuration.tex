...
\item
  ...
  When \texttt{Automatic} is \texttt{true}, the original value from the firmware
  will be set if valid (i.e. \texttt{non-zero}). Otherwise, the value from \texttt{OcMacInfoLib}
  will be set. Under non-Automatic mode (i.e. when \texttt{Automatic} is \texttt{false}),
  a user-specified value will be set if available. Otherwise, the original value from 
  the firmware will be set if valid (i.e. \texttt{non-zero}).
  If still not, the fallback value (\texttt{zero}) will be set.
