...
\item
  ...
  \begin{itemize}
  \tightlist
  \item If \texttt{Auto} is treated as \texttt{Enabled} when \texttt{KeySupport} is \texttt{true} and \texttt{Disabled} otherwise.
  \item If \texttt{Enabled} the values \texttt{KeyInitialDelay} and \texttt{KeySubsequentDelay} are used.
  \item If \texttt{Disabled} Apple default values of 500ms (\texttt{50}) and 50ms (\texttt{5}) are used.
  \end{itemize}
...
\item
  ...
  When not using \texttt{KeySupport}, this option will configure the initial delay before
  key repeat as expected. The Apple OEM default value is \texttt{50} (500ms).
  ...
  effect, set \texttt{KeyInitialDelay} to \texttt{0}. If doing this, to avoid multiple
  reponses to single key presses, you should set \texttt{KeySubsequentDelay} to at least
  the value of your \texttt{KeyForgetThreshold} setting.
...
\item
  ...
  and set this option to at least the value of your \texttt{KeyForgetThreshold} setting. (In
  the unlikely event that you still get frequent, or occasional, double key responses after doing
  this, proceed to set this value to one or two greater than \texttt{KeyForgetThreshold} - use
