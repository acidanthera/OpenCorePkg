...
\item
  ...
  provided by the motherboard by default. This section overrides the PCD values listed in
  ...
  such that external serial ports (e.g. from a PCI card) will also function properly.
  ...
  Specifically, when booting macOS, 
...
\item
  ...
  \item To have \texttt{kprintf} working properly, \texttt{Apple16X50PCI0} should be prevented from attaching,
    by setting (i.e. \texttt{Delete}, then \texttt{Add}) the \texttt{class-code} property of the PCIe serial port device to \texttt{FFFFFFFF}
    in \texttt{DeviceProperties} section. As an alternative solution, a codeless kext \texttt{PCIeSerialDisable.kext} shown at
