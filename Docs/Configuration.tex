...
\item
  \texttt{InjectionTarget}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \texttt{Boot} (Inject into the Boot Kext Collection)\\
  \textbf{Description}: Injection target for the specific kernel extension. Ignored if running macOS 15 (Catalina) or older.

  \hypertarget{injectiontarget}Valid values:

  \begin{itemize}
    \tightlist
    \item \texttt{Boot} --- Inject into the Boot Kext Collection (default).
    \item \texttt{System} --- Inject into the System Kext Collection (\texttt{/System/Library/Extensions}).
    \item \texttt{Auxiliary} --- Inject into the Auxiliary Kext Collection (\texttt{/Library/Extensions}).
  \end{itemize}
...
\item
  \texttt{ExclusionTarget}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \texttt{Boot} (Block kext contained in the Boot Kext Collection)\\
  \textbf{Description}: Exclusion target for the specific kernel extension. Ignored if running macOS 15 (Catalina) or older.

  \emph{Note}: Refer to the \hyperlink{injectiontarget}{\texttt{Add\ InjectionTarget} description} for valid values.
