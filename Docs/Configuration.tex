...
\item
  ...
  \begin{itemize}
    \tightlist
    \item \texttt{Raw} --- by writing all the bytes as they are without making
      any order changes (\texttt{\{AA BB CC DD EE FF GG HH II JJ KK LL MM NN OO PP\}}).
      This method is also known \href{https://tools.ietf.org/html/rfc4122}{RFC 4122}
      encoding or Big Endian encoding.
  ...
  SMBIOS specification did not explicitly specify the encoding method for the
  UUID up to SMBIOS 2.6, where it stated that \texttt{Little Endian} encoding
  is to be used. This led to the confusion in both firmware implementations
  and system software as different vendors used different encodings prior to that.

  \begin{itemize}
    \tightlist
    \item Apple uses \texttt{Raw} method everywhere but it ignores SMBIOS UUID.
    \item \texttt{dmidecode} uses \texttt{Raw} method for SMBIOS 2.5.x or lower
    ...
    \item Windows uses \texttt{Little Endian} method everywhere, but it only affects
    the visual representation of the values.
  \end{itemize}

  OpenCore always sets a recent SMBIOS version (currently 3.2) when generating
  the modified tables. If \texttt{UseRawUuidEncoding} is enabled \texttt{Raw}
  method will be used to store the \texttt{SystemUUID} data. Otherwise
  \texttt{Little Endian} is chosen.
  ...
  SMBIOS they are always stored with \texttt{Raw} method.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: OEM specified or not installed\\
