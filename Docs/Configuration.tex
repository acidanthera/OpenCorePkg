...
\item
  ...
  \emph{Note}: While this quirk is generally reliable and replaces
  \texttt{EnableSafeModeSlide} in most of the cases, some boards
  will not be compatible due to hardware mapped in lower memory.
...
\item
  ...
  \emph{Note}: This quirk is deprecated in favour of \texttt{AllowRelocationBlock}
  and is not compatible with macOS 11.0 or macOS 10.7 and lower. The necessity of
  this quirk is determined by safe mode availability. If booting to safe mode fails
  and using \texttt{AllowRelocationBlock} does not help, this option can be tried
  to be enabled.
