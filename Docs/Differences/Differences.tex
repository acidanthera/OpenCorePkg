...
%DIF DEL PreviousConfiguration.tex   Tue Jan 31 07:55:23 2023
%DIF ADD ../Configuration.tex        Tue Jan 31 07:56:05 2023
...
\item
  ...
  in OpenCore \DIFdelbegin \DIFdel{(e.g. on a CDROM) }\DIFdelend with \texttt{FullNvramAccess} enabled. It will launch the Apple picker,
  which allows selection of an item to boot next (with \texttt{Enter}), or next and \DIFdelbegin \DIFdel{subsequently, i.e.
  as the blessed entry }\DIFdelend \DIFaddbegin \DIFadd{from then on until
  the next change }\DIFaddend (with \texttt{CTRL+Enter})\DIFdelbegin \DIFdel{, as normal. After }\DIFdelend \DIFaddbegin \DIFadd{. Note that after }\DIFaddend the selection is made, the system
  \DIFdelbegin \DIFdel{will reboot and }\DIFdelend \DIFaddbegin \emph{\DIFadd{will reboot}} \DIFadd{before }\DIFaddend the chosen entry \DIFdelbegin \DIFdel{will be booted. }\DIFdelend \DIFaddbegin \DIFadd{is booted. While this behaviour might seem surprising,
  it can be used both to switch which OpenCore installation is blessed, with }\texttt{\DIFadd{CTRL+Enter}}\DIFadd{,
  e.g. from a recovery OpenCore installation on CD (selected with the }\texttt{\DIFadd{C}} \DIFadd{key on boot) back
...
\DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{\DIFadd{EnableGop\{Direct\}}}}\textbf{\DIFadd{*}}
& \DIFadd{Early beta release firmware-embeddable driver providing pre-OpenCore non-native GPU support
  on MacPro5,1. Installation instructions can be found in the
  }\href{https://github.com/acidanthera/OpenCorePkg/blob/master/Staging/EnableGop/README.md}{\texttt{\DIFadd{Utilities/EnableGop}}}
  \DIFadd{directory of the OpenCore release zip file - proceed with caution. }\\
...
\item
  ...
  \textbf{\DIFadd{Description}}\DIFadd{: Use PciRootBridgeIo for }\DIFaddend \texttt{ResizeGpuBars} \DIFaddbegin \DIFadd{and }\texttt{\DIFadd{ResizeAppleGpuBars}}
...
\item
  \texttt{\DIFadd{ResizeGpuBars}}\DIFaddend \\
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{-1}\\
