...
%DIF ADD ../Configuration.tex        Thu Sep 11 11:30:19 2025
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (1.0\DIFdelbegin \DIFdel{.5}\DIFdelend \DIFaddbegin \DIFadd{.6}\DIFaddend )
...
\item
  ...
  \item The Memory attributes table is ignored. \texttt{EfiRuntimeServicesCode}
    ...
    type.
  \DIFaddbegin \item \DIFadd{Apple kernel Memory map entry consolidation may work incorrectly for low memory descriptors,
    which are initially marked as
    }\href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/i386_vm_init.c#L282}{\DIFadd{preallocated}}
    \DIFadd{by Apple kernel, but then may get consolidated and lose their preallocation status due to a
    }\href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/i386_vm_init.c#L339}{\DIFadd{bug}}\DIFadd{.
    Since Apple kernel later
    }\href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-792/osfmk/i386/pmap.c#L1204}{\DIFadd{frees low memory}}\DIFadd{,
    this may result in use-after-free errors and various kinds of kernel panics at boot time.
    The issue was }\href{https://github.com/apple-oss-distributions/xnu/blob/rel/xnu-1699/osfmk/i386/i386_vm_init.c#L450}{\DIFadd{fixed}}
    \DIFadd{in Mac OS X 10.7 kernel. 
  ...
  Memory Attribute Tables (MAT).
\DIFdelbegin \DIFdel{This quirk is typically unnecessary when using
  }\texttt{\DIFdel{OpenDuetPkg}} %DIFAUXCMD
\DIFdel{but may be required to boot Mac OS X 10.6, and earlier, for
...
\item
  ...
  \begin{itemize}
    \tightlist
    \item \texttt{Auto} --- Automatically select one set of icons based on the \DIFaddbegin \texttt{\DIFadd{background-color}}
      \DIFadd{and }\DIFaddend \texttt{DefaultBackground} \DIFdelbegin \DIFdel{colour}\DIFdelend \DIFaddbegin \DIFadd{colours}\DIFaddend : \texttt{Acidanthera\textbackslash GoldenGate} for Syrah Black and
      \texttt{Acidanthera\textbackslash Chardonnay} for Light Gray. \DIFaddbegin \texttt{\DIFadd{background-color}} \DIFadd{variable has
      priority.
    }\DIFaddend \item \texttt{Default} --- \texttt{Acidanthera\textbackslash GoldenGate}.
  \end{itemize}
...
\DIFaddbegin \item
  \texttt{\DIFadd{4D1FDA02-38C7-4A6A-9CC6-4BCCA8B30102:background-color}}
  \break
  \DIFadd{Acidanthera override for }\texttt{\DIFadd{DefaultBackgroundColor}} \DIFadd{useful for legacy OS X
  versions, e.g. OS X 10.9, which does not display grey boot screen when
  }\texttt{\DIFadd{DefaultBackgroundColor}} \DIFadd{is set, regardless of its value.
}\DIFaddend \end{itemize}
...
builtin icon set. The chosen icon set may depend on the \DIFaddbegin \texttt{\DIFadd{background-color}} \DIFadd{and
}\DIFaddend \texttt{DefaultBackgroundColor} variable \DIFdelbegin \DIFdel{value}\DIFdelend \DIFaddbegin \DIFadd{values}\DIFaddend . Refer to \texttt{PickerVariant} for more details.
...
\tightlist
  ...
  should not load this driver, as \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{a firmware driver serving the same purpose }\DIFaddend is included in OpenDuet.
...
\begin{itemize}
  ...
  \item Last released installer image for Mac OS X~10.5 is Mac OS X~10.5.7
    build \texttt{9J3050} (for \texttt{MacBookPro5,3}). \DIFdelbegin \DIFdel{Unlike the others,
    this image is not limited to the target model identifiers and can be used
    as is. }\DIFdelend The original \texttt{9J3050} image can be found
    ...
    for details. \DIFaddbegin \DIFadd{This image is limited to }\texttt{\DIFadd{MacBookPro5,2}}\DIFadd{, }\texttt{\DIFadd{MacBookPro5,3}}\DIFadd{,
    }\texttt{\DIFadd{MacBookPro5,4}}\DIFadd{, }\texttt{\DIFadd{MacBookPro5,5}} \DIFadd{model identifiers.
    To fix compatibility with other models one can hex-edit the raw installer image replacing
    the logic of }\texttt{\DIFadd{/usr/libexec/cpu\_check}} \DIFadd{script. For example, it can be changed
    to always return }\texttt{\DIFadd{0}} \DIFadd{by altering the ternary operator of the }\texttt{\DIFadd{exit}} \DIFadd{function call:
}\DIFmodbegin
\begin{lstlisting}[label=hex105, style=ocbash,alsolanguage=DIFcode]
%DIF > exit ( $CPUs{$hostCPU} ? 0 : 1 );
\end{lstlisting}
\DIFmodend
    \DIFaddend Note that this \DIFaddbegin \DIFadd{image }\DIFaddend is the earliest tested version of Mac OS X~10.5 with OpenCore.
\end{itemize}
...
\begin{itemize}
  ...
  \item Last released installer images for Mac OS X~10.4 are Mac OS X~10.4.10
    ...
    for details. Note that these are the earliest tested
    versions of Mac OS X~10.4 with OpenCore\DIFaddbegin \DIFadd{.
  }\item \DIFadd{Due to lack of }\texttt{\DIFadd{createinstallmedia}} \DIFadd{utility and because the DMG installer
    is a volume image, not of a disk image (with a partition table), it is necessary
    to either restore the DMG file to a partition on a GPT-formatted media or convert
    it to a disk image. The following commands could be used to do it:
}

\DIFmodbegin
\begin{lstlisting}[label=convertdmg, style=ocbash,alsolanguage=DIFcode]
%DIF > hdiutil create -o converted.dmg -size 14g -layout GPTSPUD -fs HFS+J
%DIF > hdiutil attach -nomount converted.dmg
%DIF > hdiutil attach -nomount /path/to/original/installer.dmg
%DIF > dd if=/dev/disk{Y} of=/dev/disk{X}s2 bs=16m
%DIF > # Optionally convert to preferrable disk image:
%DIF > qemu-img convert -p -O qcow2 /dev/disk{X} installer.qcow2
%DIF > hdiutil detach disk{X} && hdiutil detach disk{Y}
\end{lstlisting}
\DIFmodend

    \DIFadd{It will be necessary to replace }\texttt{\DIFadd{\{X\}}} \DIFadd{and }\texttt{\DIFadd{\{Y\}}} \DIFadd{with disk
    numbers for attached target image (converted.dmg) and original DMG installer
    (installer.dmg). Converting the resulting dmg can be done any other preferrable
    format with the help of }\texttt{\DIFadd{qemu-img}} \DIFadd{utility}\DIFaddend .
\end{itemize}
