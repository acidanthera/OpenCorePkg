...
\item
  ...
  \emph{Note 3}: On Macs with problematic GOP, it may be difficult to \DIFdelbegin \DIFdel{access the Apple picker}\DIFdelend \DIFaddbegin \DIFadd{re-bless OpenCore if its bless status
  is lost}\DIFaddend . The \texttt{BootKicker} utility can be \DIFdelbegin \DIFdel{blessed to workaround this problemeven without loading
  OpenCore . On some Macs however,
  the }\DIFdelend \DIFaddbegin \DIFadd{used to work around this problem, if set up as a Tool
  in OpenCore (e.g. on a CDROM) with }\texttt{\DIFadd{FullNvramAccess}} \DIFadd{enabled. It will launch the Apple picker,
  which allows selection of an item to boot next (with }\texttt{\DIFadd{Enter}}\DIFadd{), or next and subsequently, i.e.
  as the blessed entry (with }\DIFaddend \texttt{\DIFdelbegin \DIFdel{BootKicker}\DIFdelend \DIFaddbegin \DIFadd{CTRL+Enter}\DIFaddend }\DIFdelbegin \DIFdel{utility cannot be run from OpenCore}\DIFdelend \DIFaddbegin \DIFadd{), as normal. After the selection is made, the system
  will reboot and the chosen entry will be booted}\DIFaddend .
...
& \DIFdelbegin \DIFdel{Enter }\DIFdelend \DIFaddbegin \DIFadd{Display }\DIFaddend Apple BootPicker menu (\DIFdelbegin \DIFdel{exclusive }\DIFdelend for Macs with compatible \DIFdelbegin \DIFdel{GPUs}\DIFdelend \DIFaddbegin \DIFadd{firmware}\DIFaddend ). \\
