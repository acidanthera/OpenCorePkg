...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.7\DIFdelbegin \DIFdel{.6}\DIFdelend \DIFaddbegin \DIFadd{.7}\DIFaddend )
       ...
       Copyright \textcopyright \DIFdelbegin \DIFdel{2018-2021 }\DIFdelend \DIFaddbegin \DIFadd{2018-2022 }\DIFaddend vit9696
...
\DIFaddbegin \DIFadd{When updating the LaTeX documentation (e.g. }\texttt{\DIFadd{Configuration.tex}}\DIFadd{) please
do }\textit{\DIFadd{not}} \DIFadd{rebuild the PDF files till merging to master happens. This avoids
unnecessary merge conflicts:
}\begin{itemize}
\tightlist
\item \DIFadd{External contributors using the pull-request approach should request
  the maintainers to handle the PDF rebuild in the pull-request message.
}\item \DIFadd{Internal contributors should rebuild the documentation at merge time
...
\DIFaddend The only officially supported toolchain is \texttt{XCODE5}. Other toolchains
...
\item
  ...
  \DIFaddbegin \DIFadd{Be warned that one is required to set the correct
  }\href{https://github.com/dortania/bugtracker/issues/190}{\DIFadd{frequency vectors}} \DIFadd{matching
  the installed CPU.
}\DIFaddend 
  ...
  \begin{itemize}
    ...
    \texttt{Cpuid1Mask}: \texttt{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}
    \item \DIFaddbegin \DIFadd{Comet Lake U62 (}\texttt{\DIFadd{0x0A0660}}\DIFadd{) to Comet Lake U42 (}\texttt{\DIFadd{0x0806EC}}\DIFadd{):}\\
    ...
    \item \DIFaddend Rocket Lake (\texttt{0x0A0670}) to Comet Lake (\texttt{\DIFdelbegin \DIFdel{0x0906EB}\DIFdelend \DIFaddbegin \DIFadd{0x0A0655}\DIFaddend }):\\
    \texttt{Cpuid1Data}: \texttt{\DIFdelbegin \DIFdel{EB }\DIFdelend \DIFaddbegin \DIFadd{55 }\DIFaddend 06 \DIFdelbegin \DIFdel{09 }\DIFdelend \DIFaddbegin \DIFadd{0A }\DIFaddend 00 00 00 00 00 00 00 00 00 00 00 00 00}\\
    \texttt{Cpuid1Mask}: \texttt{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}
    \item \DIFdelbegin \DIFdel{Comet Lake U62 (}\DIFdelend \DIFaddbegin \DIFadd{Alder Lake (}\DIFaddend \texttt{\DIFdelbegin \DIFdel{0x0A0660}\DIFdelend \DIFaddbegin \DIFadd{0x090672}\DIFaddend }) to Comet Lake \DIFdelbegin \DIFdel{U42 (}\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend \texttt{\DIFdelbegin \DIFdel{0x0806EC}\DIFdelend \DIFaddbegin \DIFadd{0x0A0655}\DIFaddend }):\\
    \texttt{Cpuid1Data}: \texttt{\DIFdelbegin \DIFdel{EC }\DIFdelend \DIFaddbegin \DIFadd{55 }\DIFaddend 06 \DIFdelbegin \DIFdel{08 }\DIFdelend \DIFaddbegin \DIFadd{0A }\DIFaddend 00 00 00 00 00 00 00 00 00 00 00 00 00}\\
    \texttt{Cpuid1Mask}: \texttt{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}
  \end{itemize}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Requirement}: 10.8 \DIFaddbegin \DIFadd{(10.14)}\DIFaddend \\
  \textbf{Description}: Provides current CPU info to the kernel.

  This quirk \DIFdelbegin \DIFdel{currently }\DIFdelend \DIFaddbegin \DIFadd{works differently depending on the CPU:
}

  \begin{itemize}
  \tightlist
  \item \DIFadd{For Microsoft Hyper-V it }\DIFaddend provides the correct TSC and FSB values
    to the kernel, as well as disables CPU topology validation \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(10.8+).
  }\item \DIFadd{For Intel CPUs it adds support for asymmetrical SMP systems
...
\DIFdel{: These patches currently target Microsoft Hyper-V and may need to be extended for other purposes.}%DIFDELCMD < 
...
& \DIFdelbegin \DIFdel{HDA audio support driver }\DIFdelend \DIFaddbegin \hyperref[uefiaudio]{HDA audio support driver} \DIFaddend in UEFI firmware for most Intel and some other analog audio controllers.
...
\DIFaddbegin \subsection{\DIFadd{AudioDxe}}\label{uefiaudio}

\DIFadd{High Definition Audio support driver in UEFI firmware for most Intel and some other analog audio controllers.
}

\subsubsection{\DIFadd{Configuration}}

\DIFadd{Most UEFI audio configuration is handled via the }\hyperref[uefiaudioprops]{\texttt{UEFI Audio Properties}}
\DIFadd{section, but if required the following additonal configuration options (which are needed to produce sound
on most Apple hardware, and possibly some others) may be specified in }\texttt{\DIFadd{UEFI/Drivers/Arguments}}\DIFadd{:
}

\begin{itemize}
  \tightlist
	\item \texttt{\DIFadd{-}{}\DIFadd{-gpio-setup}} \DIFadd{- Default value is }\texttt{\DIFadd{0}} \DIFadd{(GPIO setup disabled) if argument is not provided,
  or }\texttt{\DIFadd{7}} \DIFadd{(all GPIO setup stages stages enabled) if the argument is provided with no value. }\medskip

	\DIFadd{Available values, which may be combined by adding, are: }\medskip

  \begin{itemize}
  \tightlist
    \item \texttt{\DIFadd{0x00000001}} \DIFadd{(bit }\texttt{\DIFadd{0}}\DIFadd{) --- }\texttt{\DIFadd{GPIO\_SETUP\_STAGE\_DATA}}\DIFadd{,
    set GPIO pin data high on specified pins. Required e.g. on }\texttt{\DIFadd{MacBookPro10,2}} \DIFadd{and }\texttt{\DIFadd{MacPro5,1}}\DIFadd{.
    }\item \texttt{\DIFadd{0x00000002}} \DIFadd{(bit }\texttt{\DIFadd{1}}\DIFadd{) --- }\texttt{\DIFadd{GPIO\_SETUP\_STAGE\_DIRECTION}}\DIFadd{,
    set GPIO data direction to output on specified pins. Required e.g. on }\texttt{\DIFadd{MacPro5,1}}\DIFadd{.
    }\item \texttt{\DIFadd{0x00000004}} \DIFadd{(bit }\texttt{\DIFadd{2}}\DIFadd{) --- }\texttt{\DIFadd{GPIO\_SETUP\_STAGE\_ENABLE}}\DIFadd{,
    enable specified GPIO pins. Required e.g. on }\texttt{\DIFadd{MacPro5,1}}\DIFadd{.
  }\end{itemize} \medskip

  \DIFadd{If audio appears to be `playing' on the correct codec, e.g. based on the debug log, but no
  sound is heard on any channel, it is suggested to use }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup}} \DIFadd{(with no
  value) in the AudioDxe driver arguments. If specified with no value, all
  stages will be enabled (equivalent of specifying }\texttt{\DIFadd{7}}\DIFadd{). If this produces sound,
  it is then possible to try fewer bits, e.g. }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup=1}}\DIFadd{, }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup=3}}\DIFadd{,
  to find out which stages are actually required. }\medskip

  \emph{\DIFadd{Note}}\DIFadd{: Value }\texttt{\DIFadd{7}} \DIFadd{(all flags enabled) of this option -- as required for the
  }\texttt{\DIFadd{MacPro5,1}} \DIFadd{-- is compatible with most systems, but is known to cause problems with
  ...
  of other systems, hence this option is not enabled by default. }\medskip
	...
	\item \texttt{\DIFadd{-}{}\DIFadd{-gpio-pins}} \DIFadd{- Default: }\texttt{\DIFadd{0}}\DIFadd{, auto-detect. }\medskip

	\DIFadd{Specifies which GPIO pins should be operated on by }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup}}\DIFadd{. This is a bit mask,
  with possible values from }\texttt{\DIFadd{0x0}} \DIFadd{to }\texttt{\DIFadd{0xFF}}\DIFadd{. The usable maximum depends on
  the number if available pins on the audio out function group of the codec in use, e.g. it is
  }\texttt{\DIFadd{0x3}} \DIFadd{(lowest two bits) if two GPIO pins are present, }\texttt{\DIFadd{0x7}} \DIFadd{if three pins are present,
  etc. }\medskip

  \DIFadd{When }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup}} \DIFadd{is enabled (i.e. non-zero), then }\texttt{\DIFadd{0}} \DIFadd{is a special
  value for }\texttt{\DIFadd{-}{}\DIFadd{-gpio-pins}}\DIFadd{, meaning that the pin mask will be auto-generated based on the
  reported number of GPIO pins on the specified codec (see }\texttt{\DIFadd{AudioCodec}}\DIFadd{), e.g. if the
  codec's audio out function group reports 4 GPIO pins, a mask of }\texttt{\DIFadd{0xF}} \DIFadd{will be used.
  The value in use can be seen in the debug log in a line such as: }\medskip

  \texttt{\DIFadd{HDA: GPIO setup on pins 0x0F - Success}} \medskip

  \DIFadd{Values for driver parameters can be specified in hexadecimal beginning with }\texttt{\DIFadd{0x}} \DIFadd{or
  in decimal, e.g. }\texttt{\DIFadd{-}{}\DIFadd{-gpio-pins=0x12}} \DIFadd{or }\texttt{\DIFadd{-}{}\DIFadd{-gpio-pins=18}}\DIFadd{.
}\end{itemize}

\emph{\DIFadd{Note}}\DIFadd{: AudioDxe is a staging driver, refer to }\href{https://github.com/acidanthera/bugtracker/issues/740}{\DIFadd{acidanthera/bugtracker\#740}}
...
\item
  ...
  in the \DIFdelbegin %DIFDELCMD < \hyperref[uefiaudioprops]{Audio Properties} %%%
\DIFdelend \DIFaddbegin \hyperref[uefiaudioprops]{\texttt{Audio Properties}} \DIFaddend section below.

  \DIFdelbegin \DIFdel{Audio support }\DIFdelend \DIFaddbegin \DIFadd{Unless documented otherwise (e.g. }\texttt{\DIFadd{ResetTrafficClass}}\DIFadd{) settings in this section
  are for UEFI audio support only (e.g. OpenCore generated boot chime and audio assist) and are
  unrelated to any configuration needed for OS audio support (e.g. }\texttt{\DIFadd{AppleALC}}\DIFadd{).
}

  \DIFadd{UEFI audio support }\DIFaddend provides a way for upstream protocols to interact with the
  selected \DIFdelbegin \DIFdel{hardware and audio }\DIFdelend \DIFaddbegin \DIFadd{audio hardware and }\DIFaddend resources. All audio resources should reside
...
\DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{-1}} \DIFadd{will play sound to all channels simultaneously. If this does not work it will usually be quickest
  ...
  \emph{\DIFadd{Note}}\DIFadd{: In addition to this option, most Apple hardware also requires the
  }\texttt{\DIFadd{-}{}\DIFadd{-gpio-setup}} \DIFadd{driver argument which is dealt with in the
