...
\item
  ...
  \begin{itemize}
    ...
    \texttt{Cpuid1Mask}: \texttt{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}
    \DIFaddbegin \item \DIFadd{Rocket Lake (}\texttt{\DIFadd{0x0A0670}}\DIFadd{) to Comet Lake (}\texttt{\DIFadd{0x0906EB}}\DIFadd{):}\\
    \texttt{\DIFadd{Cpuid1Data}}\DIFadd{: }\texttt{\DIFadd{EB 06 09 00 00 00 00 00 00 00 00 00 00 00 00 00}}\\
    \texttt{\DIFadd{Cpuid1Mask}}\DIFadd{: }\texttt{\DIFadd{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}}
...
\DIFaddbegin \subsection{\DIFadd{AppleInput Properties}}\label{uefiappleinputprops}

\begin{enumerate}

  \item
  \texttt{\DIFadd{AppleEvent}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Auto}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Determine whether OC builtin or OEM Apple Event protocol is used.
}

  \DIFadd{This option determines whether Apple's OEM Apple Event protocol is used (where available), or
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{\DIFadd{Auto}} \DIFadd{--- Performs automatic choice of implementation. Because of optimisations used to achieve
  fast boot times, this actually means that the OpenCore re-implementation will be found and used
  except in the case that OpenCore was been explicitly selected and started (not just auto-booted) from
  Apple's boot picker (where present).
  }\item \texttt{\DIFadd{Builtin}} \DIFadd{--- Use OpenCore's updated re-implementation of the Apple Event protocol. Recommended.
  }\item \texttt{\DIFadd{OEM}} \DIFadd{--- Assume Apple's protocol will be available at driver connection. This results in Apple's
  implementation being reliably used on Apple systems. It results in no keyboard or mouse support otherwise.
  }\end{itemize}

\item
  \texttt{\DIFadd{CustomDelays}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Auto}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enable custom key repeat delays.
}
...
\item
  \texttt{\DIFadd{KeyInitialDelay}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}} \DIFadd{(no initial delay, immediate repeat)}\\
...
\item
  ...
  \DIFadd{Configures the gap between key repeats. The Apple OEM default value is }\texttt{\DIFadd{5}} \DIFadd{(50ms).
...
\item
  \texttt{\DIFadd{PointerSpeedDiv}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{1}}\DIFadd{)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Configure pointer speed divisor in OpenCore implementation
  of Apple Event protocol.
}

  \DIFadd{Configures the divisor for pointer movements. The Apple OEM default value is }\texttt{\DIFadd{1}}\DIFadd{.
  }\texttt{\DIFadd{0}} \DIFadd{is an invalid value for this option.
}

\item
  \texttt{\DIFadd{PointerSpeedMul}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}}\DIFadd{)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Configure pointer speed multiplier in OpenCore implementation
  of Apple Event protocol.
}

  \DIFadd{Configures the multiplier for pointer movements. The Apple OEM default value is }\texttt{\DIFadd{1}}\DIFadd{.
}
