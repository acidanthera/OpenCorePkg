...
\item
  ...
  \DIFadd{Consider a GPU with 2 BARs:
  }\begin{itemize}
    \tightlist
    \item \texttt{\DIFadd{BAR0}} \DIFadd{supports sizes from 256 MB to 8 GB. Its value is 4 GB.
    }\item \texttt{\DIFadd{BAR1}} \DIFadd{supports sizes from 2 MB to 256 MB. Its value is 256 MB.
  }\end{itemize}

  \emph{\DIFadd{Example 1}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeAppleGpuBars}} \DIFadd{to 1 GB will change
  }\texttt{\DIFadd{BAR0}} \DIFadd{to 1 GB and leave }\texttt{\DIFadd{BAR1}} \DIFadd{unchanged.
  }\\
  \emph{\DIFadd{Example 2}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeAppleGpuBars}} \DIFadd{to 1 MB will change
  }\texttt{\DIFadd{BAR0}} \DIFadd{to 256 MB and }\texttt{\DIFadd{BAR0}} \DIFadd{to 2 MB.
  }\\
  \emph{\DIFadd{Example 3}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeAppleGpuBars}} \DIFadd{to 16 GB will make no
  changes.
}

  \emph{\DIFadd{Note 1}}\DIFadd{: See }\texttt{\DIFadd{ResizeGpuBars}} \DIFadd{quirk for general GPU PCI BAR
  size configuration and more details about the technology.
}

  \emph{\DIFadd{Note 2}}\DIFadd{: Certain GPU drivers do not support non-standard BAR sizes,
  causing sleep wake issues, for this reason for macOS it is recommended
  to use minimal supported BAR sizes, i.e. specify }\texttt{\DIFadd{0}} \DIFadd{(1 MB).
}
...
\item
  ...
  \emph{Note}: The need for this quirk is indicated by early boot failures (\DIFdelbegin \DIFdel{e.g. halts }\DIFdelend \DIFaddbegin \DIFadd{note: includes halt }\DIFaddend at black
  screen \DIFdelbegin \DIFdel{), particularly in }\DIFdelend \DIFaddbegin \DIFadd{as well as more obvious crash). Particularly likely to affect }\DIFaddend early boot of \DIFdelbegin \DIFdel{the Linux kernel}\DIFdelend \DIFaddbegin \DIFadd{Windows or Linux (but
  not always both) on affected systems}\DIFaddend . Only firmware released after 2017 is typically affected.
...
%DIFAUXCMD
\DIFdel{or to the closely related (but not identical, see next paragraph) }\href{https://fedoraproject.org/wiki/Changes/BootLoaderSpecByDefault}{\DIFdel{systemd BootLoaderSpecByDefault}}%DIFAUXCMD
\DIFdel{.
In effect this means Linux distributions where the available boot options are found in }\DIFdelend \DIFaddbegin \DIFadd{aims to automatically detect and boot most Linux distros without additional configuration.
}

\DIFadd{Usage is as follows:
}

\begin{itemize}
\tightlist
  \item \DIFadd{Add }\texttt{\DIFadd{OpenLinuxBoot.efi}} \DIFadd{and also typically (see below) }\texttt{\DIFadd{ext4\_x64.efi}} \DIFadd{to the }\texttt{\DIFadd{config.plist}}
  \texttt{\DIFadd{Drivers}} \DIFadd{section.
  }\item \DIFadd{Make sure }\texttt{\DIFadd{RequestBootVarRouting}} \DIFadd{and }\texttt{\DIFadd{LauncherOption}} \DIFadd{are enabled in }\DIFaddend \texttt{\DIFdelbegin \DIFdel{\{ESP\}/loader/entries/*}\DIFdelend \DIFaddbegin \DIFadd{config}\DIFaddend .\DIFdelbegin \DIFdel{conf}\DIFdelend \DIFaddbegin \DIFadd{plist}\DIFaddend }\DIFdelbegin \DIFdel{files (for instance }\texttt{\DIFdel{/boot/efi/loader/entries/*.conf}}%DIFAUXCMD
\DIFdel{)
...
layout with kernel files typically on the partition which will be mounted as }\DIFdelend \DIFaddbegin \DIFadd{;
  it is also recommended to enable }\DIFaddend \texttt{\DIFdelbegin \DIFdel{/boot}\DIFdelend \DIFaddbegin \DIFadd{HideAuxiliary}\DIFaddend } \DIFdelbegin \DIFdel{applies to most Fedora-related distros including Fedora itself, RHEL and variants.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{BootLoaderSpecByDefault includes the possibility of expanding GRUB variables
in its }\texttt{\DIFdel{*.conf}} %DIFAUXCMD
\DIFdel{files }\DIFdelend \DIFaddbegin \DIFadd{in order to hide older Linux kernels except when required
  ...
  }\item \DIFadd{Install Linux as normal if this has not been done earlier }\DIFaddend -- \DIFdelbegin \DIFdel{and this is used in practice in certain distros such as CentOS.
  In order to correctly handle this , }\texttt{\DIFdel{OpenLinuxBoot}} %DIFAUXCMD
\DIFdel{extracts all variables from
}\texttt{\DIFdel{\{boot\}/grub2/grubenv}} %DIFAUXCMD
\DIFdel{and any unconditionally set variables from }\texttt{\DIFdel{\{boot\}/grub2/grub.cfg}}%DIFAUXCMD
\DIFdel{.
This has proved sufficient in practice to extract the required variables seen so far in distros which use this GRUB-specific feature.
  }\DIFdelend \DIFaddbegin \DIFadd{OpenLinuxBoot is not involved in this stage.
...
to manually add the missing options, e.g. with }\texttt{\DIFadd{partuuidopts:\{partuuid\}+=\{opts\}}} \DIFadd{to target a specific
distro (or just with }\texttt{\DIFadd{autoopts+=\{opts\}}}\DIFadd{, which applies to all installed distros, if only one distro is in use).
}
...
\DIFdelend \DIFaddbegin \DIFadd{OpenLinuxBoot }\DIFaddend is selected as the default boot entry
  ...
  (check \texttt{cat /proc/cmdline} when booted via \DIFdelbegin \DIFdel{your existing }\DIFdelend \DIFaddbegin \DIFadd{the distro's default }\DIFaddend bootloader) on Ubuntu and related distros,
...
\item
  ...
  \emph{\DIFadd{Example 1}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeGpuBars}} \DIFadd{to 1 GB will change
  }\texttt{\DIFadd{BAR0}} \DIFadd{to 1 GB and leave }\texttt{\DIFadd{BAR1}} \DIFadd{unchanged.
  }\\
  \emph{\DIFadd{Example 2}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeGpuBars}} \DIFadd{to 1 MB will change
  }\texttt{\DIFadd{BAR0}} \DIFadd{to 256 MB and }\texttt{\DIFadd{BAR0}} \DIFadd{to 2 MB.
  }\\
  \emph{\DIFadd{Example 3}}\DIFadd{: Setting }\texttt{\DIFadd{ResizeGpuBars}} \DIFadd{to 16 GB will change
  }\texttt{\DIFadd{BAR0}} \DIFadd{to 8 GB and leave }\texttt{\DIFadd{BAR1}} \DIFadd{unchanged.
}
