...
%DIF ADD ../Configuration.tex        Sun Sep  6 15:26:35 2020
...
\DIFdelbegin %DIFDELCMD < \begin{longtable}[]{@{}ll@{}}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdel{Type }\DIFdelend \DIFaddbegin \begin{center}
\begin{tabular}{|l|l|} 
\hline
\textbf{\DIFadd{Type}} \DIFaddend & \DIFdelbegin \DIFdel{Value}%DIFDELCMD < \tabularnewline
%DIFDELCMD < \midrule
%DIFDELCMD < \endhead
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Value}} \\
\hline
\DIFaddend \texttt{plist\ string} & Empty string
(\texttt{\textless{}string\textgreater{}\textless{}/string\textgreater{}}) \DIFdelbegin %DIFDELCMD < \tabularnewline
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \\
\hline
\DIFaddend \texttt{plist\ data} & Empty data
(\texttt{\textless{}data\textgreater{}\textless{}/data\textgreater{}}) \DIFdelbegin %DIFDELCMD < \tabularnewline
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \\
\hline
\DIFaddend \texttt{plist\ integer} & 0
(\texttt{\textless{}integer\textgreater{}0\textless{}/integer\textgreater{}}) \DIFdelbegin %DIFDELCMD < \tabularnewline
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \\
\hline
\DIFaddend \texttt{plist\ boolean} & False
(\texttt{\textless{}false/\textgreater{}}) \DIFdelbegin %DIFDELCMD < \tabularnewline
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \\
\hline
\DIFaddend \texttt{plist\ tristate} & False
(\texttt{\textless{}false/\textgreater{}}) \DIFdelbegin %DIFDELCMD < \tabularnewline
...
\item
  \DIFaddbegin \texttt{\DIFadd{Force}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ array}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Load kernel drivers from system volume if they are not cached.
}

  \DIFadd{Designed to be filled with }\texttt{\DIFadd{plist\ dict}} \DIFadd{values, describing each driver.
  See }\hyperref[kernelpropsforce]{Force Properties} \DIFadd{section below.
  ...
  operating systems, where various dependency kexts, like }\texttt{\DIFadd{IOAudioFamily}}
  \DIFadd{or }\texttt{\DIFadd{IONetworkingFamily}} \DIFadd{may not be present in the kernel cache by default.
  Kernel driver load order follows the item order in the array, thus the dependencies
  should be written prior to their consumers. }\texttt{\DIFadd{Force}} \DIFadd{happens before
  }\texttt{\DIFadd{Add}}\DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: The signature of the ``forced'' kernel drivers is not checked anyhow,
...
\DIFaddbegin \item
  ...
  \textbf{\DIFadd{Description}}\DIFadd{: Define kernelspace operation mode via parameters
  described in }\hyperref[kernelpropsscheme]{Scheme Properties} \DIFadd{section below.
}
...
\begin{enumerate}
\item
  \DIFaddbegin \texttt{\DIFadd{Arch}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Any}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext architecture (}\texttt{\DIFadd{Any}}\DIFadd{, }\texttt{\DIFadd{i386}}\DIFadd{, }\texttt{\DIFadd{x86\_64}}\DIFadd{).
}

\item
  \DIFaddend \texttt{BundlePath}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty string\\
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Any}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext block architecture (}\texttt{\DIFadd{Any}}\DIFadd{, }\texttt{\DIFadd{i386}}\DIFadd{, }\texttt{\DIFadd{x86\_64}}\DIFadd{).
}
...
\DIFaddbegin \subsection{\DIFadd{Force Properties}}\label{kernelpropsforce}
...
\item
  \texttt{\DIFadd{BundlePath}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext bundle path (e.g.
  }\texttt{\DIFadd{System\textbackslash Library \textbackslash Extensions \textbackslash IONetworkingFamily.kext}}\DIFadd{).
}
...
\item
  ...
  \textbf{\DIFadd{Description}}\DIFadd{: This kernel driver will not be added when not present
  unless set to }\texttt{\DIFadd{true}}\DIFadd{.
}

\item
  \texttt{\DIFadd{ExecutablePath}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext executable path relative to bundle
  (e.g. }\texttt{\DIFadd{Contents/MacOS/IONetworkingFamily}}\DIFadd{).
}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext identifier to perform presence checking
  before adding (e.g. }\texttt{\DIFadd{com.apple.iokit.IONetworkingFamily}}\DIFadd{).
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Adds kernel driver on specified macOS version or older.
}

  \emph{\DIFadd{Note}}\DIFadd{: Refer to }\hyperlink{kernmatch}{\texttt{Add} \texttt{Add MaxKernel} description} \DIFadd{for matching logic.
}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Adds kernel driver on specified macOS version or newer.
}
...
\item
  \texttt{\DIFadd{PlistPath}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext }\texttt{\DIFadd{Info.plist}} \DIFadd{path relative to bundle
  (e.g. }\texttt{\DIFadd{Contents/Info.plist}}\DIFadd{).
}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Any}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Kext patch architecture (}\texttt{\DIFadd{Any}}\DIFadd{, }\texttt{\DIFadd{i386}}\DIFadd{, }\texttt{\DIFadd{x86\_64}}\DIFadd{).
}

\item
  \DIFaddend \texttt{Base}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty string\\
...
\DIFaddbegin \subsection{\DIFadd{Scheme Properties}}\label{kernelpropsscheme}

\DIFadd{These properties are particularly relevant for older macOS operating systems.
For more details on how to install and troubleshoot such macOS installation
refer to }\hyperref[legacyapple]{Legacy Apple OS}\DIFadd{.
}

\begin{enumerate}

\item
  \texttt{\DIFadd{FuzzyMatch}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Use }\texttt{\DIFadd{kernelcache}} \DIFadd{with different checksums when available.
}

  \DIFadd{On macOS 10.6 and earlier }\texttt{\DIFadd{kernelcache}} \DIFadd{filename has a checksum, which essentially
  is }\texttt{\DIFadd{adler32}} \DIFadd{from SMBIOS product name and EfiBoot device path. On certain firmwares
  EfiBoot device path differs between UEFI and macOS due to ACPI or hardware specifics,
  rendering }\texttt{\DIFadd{kernelcache}} \DIFadd{checksum as always different.
}

  \DIFadd{This setting allows matching the latest }\texttt{\DIFadd{kernelcache}} \DIFadd{with a suitable architecture
  when the }\texttt{\DIFadd{kernelcache}} \DIFadd{without suffix is unavailable, improving macOS 10.6 boot
  performance on several platforms.
}

\item
  \texttt{\DIFadd{KernelArch}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  ...
  \hline
  \textbf{\DIFadd{Model}} & \textbf{\DIFadd{10.6 (minimal)}} & \textbf{\DIFadd{10.6 (client)}} & \textbf{\DIFadd{10.6 (server)}} & \textbf{\DIFadd{10.7 (any)}} \\
  \hline
  \DIFadd{Macmini }& \DIFadd{4,x (Mid 2010) }& \DIFadd{5,x (Mid 2011) }& \DIFadd{4,x (Mid 2010) }& \DIFadd{3,x (Early 2009) }\\
  \hline
  \DIFadd{MacBook }& \DIFadd{Unsupported }& \DIFadd{Unsupported }& \DIFadd{Unsupported }& \DIFadd{5,x (2009/09) }\\
  \hline
  \DIFadd{MacBookAir }& \DIFadd{Unsupported }& \DIFadd{Unsupported }& \DIFadd{Unsupported }& \DIFadd{2,x (Late 2008) }\\
  \hline
  \DIFadd{MacBookPro }& \DIFadd{4,x (Early 2008) }& \DIFadd{8,x (Early 2011) }& \DIFadd{8,x (Early 2011) }& \DIFadd{3,x (Mid 2007) }\\
  \hline
  \DIFadd{iMac }& \DIFadd{8,x (Early 2008) }& \DIFadd{12,x (Mid 2011) }& \DIFadd{12,x (Mid 2011) }& \DIFadd{7,x (Mid 2007) }\\
  \hline
  \DIFadd{MacPro }& \DIFadd{3,x (Early 2008) }& \DIFadd{5,x (Mid 2010) }& \DIFadd{3,x (Early 2008) }& \DIFadd{3,x (Early 2008) }\\
  \hline
  \DIFadd{Xserve }& \DIFadd{2,x (Early 2008) }& \DIFadd{2,x (Early 2008) }& \DIFadd{2,x (Early 2008) }& \DIFadd{2,x (Early 2008) }\\
  \hline
  \end{tabular}
  \end{center}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Auto}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Prefer specified kernel cache type (}\texttt{\DIFadd{Auto}}\DIFadd{, }\texttt{\DIFadd{Cacheless}}\DIFadd{,
  }\texttt{\DIFadd{Mkext}}\DIFadd{, }\texttt{\DIFadd{Prelinked}}\DIFadd{) when available.
}

  \DIFadd{Different variants of macOS support different kernel caching variants designed to improve
  ...
  specifying }\texttt{\DIFadd{Mkext}} \DIFadd{one will disable }\texttt{\DIFadd{Prelinked}} \DIFadd{for e.g. 10.6 but not 10.7.
}

  \DIFadd{The list of available kernel caching types and its current support in OpenCore is listed below.
}

  \begin{center}
  \begin{tabular}{|p{0.67in}|c|c|c|c|c|c|c|} 
  \hline
  \textbf{\DIFadd{macOS}} & \textbf{\DIFadd{i386 NC}} & \textbf{\DIFadd{i386 MK}} & \textbf{\DIFadd{i386 PK}} & \textbf{\DIFadd{x86\_64 NC}} & \textbf{\DIFadd{x86\_64 MK}} & \textbf{\DIFadd{x86\_64 PK}} & \textbf{\DIFadd{x86\_64 KC}} \\
  \hline
  \DIFadd{10.4 }& \DIFadd{NO }& \DIFadd{NO (V1) }& \DIFadd{NO }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }\\
  \hline
  \DIFadd{10.5 }& \DIFadd{NO }& \DIFadd{NO (V1) }& \DIFadd{NO }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }\\
  \hline
  \DIFadd{10.6 }& \DIFadd{NO }& \DIFadd{NO (V2) }& \DIFadd{NO }& \DIFadd{YES }& \DIFadd{YES (V2) }& \DIFadd{YES }& \DIFadd{--- }\\
  \hline
  \DIFadd{10.7 }& \DIFadd{NO }& \DIFadd{--- }& \DIFadd{NO }& \DIFadd{YES }& \DIFadd{--- }& \DIFadd{YES }& \DIFadd{--- }\\
  \hline
  \DIFadd{10.8-10.9 }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{YES }& \DIFadd{--- }& \DIFadd{YES }& \DIFadd{--- }\\
  \hline
  \DIFadd{10.10-10.15 }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{YES }& \DIFadd{--- }\\
  \hline
  \DIFadd{11.0+ }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{--- }& \DIFadd{YES }& \DIFadd{YES }\\
  \hline
  \end{tabular}
  \end{center}
...
\DIFaddend \section{Misc}\label{misc}
...
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Warning}}\DIFaddend : Some firmwares are reported to have broken NVRAM garbage collection.
  ...
  \item \DIFaddbegin \texttt{\DIFadd{OCI4}} \DIFadd{--- OcAppleImg4Lib
  }\item \DIFaddend \texttt{OCIC} --- OcImageConversionLib
...
\item \DIFaddbegin \label{securedmgloading}
  \texttt{\DIFadd{DmgLoading}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{\DIFadd{Disabled}} \DIFadd{--- loading DMG images will fail. }\texttt{\DIFadd{Disabled}}
    \DIFadd{policy will still let macOS Recovery to load in most cases as there
    usually are }\texttt{\DIFadd{boot.efi}} \DIFadd{files compatible with Apple Secure Boot.
    Manually downloaded DMG images stored in }\texttt{\DIFadd{com.apple.recovery.boot}}
    \DIFadd{directories will not load, however.
  }\item \texttt{\DIFadd{Signed}} \DIFadd{--- only Apple-signed DMG images will load. Due to
    Apple Secure Boot design }\texttt{\DIFadd{Signed}} \DIFadd{policy will let any Apple-signed
    ...
    }\texttt{\DIFadd{Any}} \DIFadd{policy is strongly not recommended and will cause a boot failure
...
\DIFaddbegin \item \label{secureapplesb}
  ...
  \DIFadd{Sets Apple Secure Boot hardware model and policy. Specifying
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{\DIFadd{Default}} \DIFadd{--- Recent available model, currently set to }\texttt{\DIFadd{j137}}\DIFadd{.
  }\item \texttt{\DIFadd{Disabled}} \DIFadd{--- No model, Secure Boot will be disabled.
  }\item \texttt{\DIFadd{j137}} \DIFadd{--- }\texttt{\DIFadd{iMacPro1,1 (December 2017) minimum macOS 10.13.2 (17C2111)}}
  \item \texttt{\DIFadd{j680}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro15,1 (July 2018) minimum macOS 10.13.6 (17G2112)}}
  \item \texttt{\DIFadd{j132}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro15,2 (July 2018) minimum macOS 10.13.6 (17G2112)}}
  \item \texttt{\DIFadd{j174}} \DIFadd{--- }\texttt{\DIFadd{Macmini8,1 (October 2018) minimum macOS 10.14 (18A2063)}}
  \item \texttt{\DIFadd{j140k}} \DIFadd{--- }\texttt{\DIFadd{MacBookAir8,1 (October 2018) minimum macOS 10.14.1 (18B2084)}}
  \item \texttt{\DIFadd{j780}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro15,3 (May 2019)  minimum macOS 10.14.5 (18F132)}}
  \item \texttt{\DIFadd{j213}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro15,4 (July 2019) minimum macOS 10.14.5 (18F2058)}}
  \item \texttt{\DIFadd{j140a}} \DIFadd{--- }\texttt{\DIFadd{MacBookAir8,2 (July 2019) minimum macOS 10.14.5 (18F2058)}}
  \item \texttt{\DIFadd{j152f}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro16,1 (November 2019) minimum macOS 10.15.1 (19B2093)}}
  \item \texttt{\DIFadd{j160}} \DIFadd{--- }\texttt{\DIFadd{MacPro7,1 (December 2019) minimum macOS 10.15.1 (19B88)}}
  \item \texttt{\DIFadd{j230k}} \DIFadd{--- }\texttt{\DIFadd{MacBookAir9,1 (March 2020) minimum macOS 10.15.3 (19D2064)}}
  \item \texttt{\DIFadd{j214k}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro16,2 (May 2020) minimum macOS 10.15.4 (19E2269)}}
  \item \texttt{\DIFadd{j223}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro16,3 (May 2020) minimum macOS 10.15.4 (19E2265)}}
  \item \texttt{\DIFadd{j215}} \DIFadd{--- }\texttt{\DIFadd{MacBookPro16,4 (June 2020) minimum macOS 10.15.5 (19F96)}}
  \item \texttt{\DIFadd{j185}} \DIFadd{--- }\texttt{\DIFadd{iMac20,1 (August 2020) minimum macOS 10.15.6 (19G2005)}}
  \item \texttt{\DIFadd{j185f}} \DIFadd{--- }\texttt{\DIFadd{iMac20,2 (August 2020) minimum macOS 10.15.6 (19G2005)}}
  \end{itemize}
  ...
  \DIFadd{Enabling Apple Secure Boot is more demanding to incorrect configurations,
  buggy macOS installations, and unsupported setups. Things to keep in mind:
}

  \begin{enumerate}
    \tightlist
    \item \DIFadd{Just like on T2 Macs you will not be able to install any unsigned kernel
    ...
    }\item \DIFadd{Operating systems released before Apple Secure Boot landed (e.g.
    ...
    }\item \DIFadd{On older CPUs (e.g. before Sandy Bridge) enabling Apple Secure Boot
      might cause slightly slower loading by up to 1 second.
    }\item \DIFadd{Since }\texttt{\DIFadd{Default}} \DIFadd{value will increase with time to support the latest
...
\DIFaddend \subsection{Entry Properties}\label{miscentryprops}
...
\item
  ...
  \DIFdelbegin \textbf{\DIFdel{Optional}}%DIFAUXCMD
\DIFdel{: When }\texttt{\DIFdel{Automatic}} %DIFAUXCMD
\DIFdel{is }\texttt{\DIFdel{false}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdelend \textbf{Description}: Update all fields. This section is read only
...
\item
  \DIFaddbegin \texttt{\DIFadd{AppleSecureBoot}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
...
and your operating system. On most x86 platforms trusted loading is implemented
via }\href{https://en.wikipedia.org/wiki/UEFI_Secure_Boot}{UEFI Secure Boot} \DIFadd{model.
Not only OpenCore fully supports this model, but it also extends its capabilities
to ensure sealed configuration via }\hyperref[securevaulting]{vaulting} \DIFadd{and
provide trusted loading to the operating systems using custom verification,
such as }\hyperref[secureapplesb]{Apple Secure Boot}\DIFadd{. Proper secure boot chain
requires several steps and careful configuration of select settings as explained below:
}

\begin{enumerate}
\item \DIFadd{Enable Apple Secure Boot by setting }\texttt{\DIFadd{SecureBootModel}} \DIFadd{if you need to
  ...
  }\hyperref[secureapplesb]{Apple Secure Boot} \DIFadd{section.
}\item \DIFadd{Disable DMG loading by setting }\texttt{\DIFadd{DmgLoading}} \DIFadd{to }\texttt{\DIFadd{Disabled}}
  \DIFadd{if you are concerned of loading old vulnerable DMG recoveries. This is
  }\textbf{\DIFadd{not}} \DIFadd{required, but recommended. For the actual tradeoffs
  see the details in }\hyperref[securedmgloading]{DMG loading} \DIFadd{section.
}\item \DIFadd{Make sure that APFS JumpStart functionality restricts the loading
  of old vulnerable drivers by setting }\texttt{\DIFadd{MinDate}} \DIFadd{and }\texttt{\DIFadd{MinVersion}}
  \DIFadd{to }\texttt{\DIFadd{0}}\DIFadd{. More details are provided in }\hyperref[uefiapfsprops]{APFS JumpStart}
  \DIFadd{section. An alternative is to install }\texttt{\DIFadd{apfs.efi}} \DIFadd{driver manually. 
}\item \DIFadd{Make sure that you do not need }\texttt{\DIFadd{Force}} \DIFadd{driver loading and can
  still boot all the operating systems you need.
}\item \DIFadd{Make sure that }\texttt{\DIFadd{ScanPolicy}} \DIFadd{restricts loading from undesired
  devices. It is a good idea to prohibit all removable drivers or unknown
  filesystems.
}\item \DIFadd{Sign all the installed drivers and tools with your private key. Do not sign
  tools that provide administrative access to your computer, like UEFI Shell.
}\item \DIFadd{Vault your configuration as explained }\hyperref[securevaulting]{Vaulting}
  \DIFadd{section.
}\item \DIFadd{Sign all OpenCore binaries (}\texttt{\DIFadd{BOOTX64.efi}}\DIFadd{, }\texttt{\DIFadd{BOOTIa32.efi}}\DIFadd{,
  }\texttt{\DIFadd{Bootstrap.efi}}\DIFadd{, }\texttt{\DIFadd{OpenCore.efi}}\DIFadd{) used on this system with
  the same private key.
}\item \DIFadd{Sign all third-party operating system (not made by Microsoft or Apple)
  bootloaders if you need them. For Linux there is an option to install
  Microsoft-signed Shim bootloader as explained on e.g.
  }\href{https://wiki.debian.org/SecureBoot}{Debian Wiki}\DIFadd{. 
}\item \DIFadd{Enable UEFI Secure Boot in your firmware preferences and install the
  certificate with a private key you own. Details on how to generate a certificate
  can be found in various articles, like }\href{https://habr.com/en/post/273497}{this one}\DIFadd{,
  and are out of the scope of this document. If you need to launch Windows you
  will also need to add the
  }\href{http://go.microsoft.com/fwlink/?LinkID=321192}{Microsoft Windows Production CA 2011}\DIFadd{.
  If you need to launch option ROMs or decided to use signed Linux drivers
  you will also need the
  }\href{http://go.microsoft.com/fwlink/?LinkId=321194}{Microsoft UEFI Driver Signing CA}\DIFadd{.
