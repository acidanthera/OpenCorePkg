...
%DIF ADD ../Configuration.tex        Sun Oct  4 18:57:22 2020
...
\newcommand{\sectionbreak}{\clearpage}
 %DIF > 
% Fix spacing for subsections in table of contents. %DIF > 
\makeatletter %DIF > 
\renewcommand{\l@subsection}{\@dottedtocline{2}{1.5em}{2.8em}} %DIF > 
\makeatother %DIF > 
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.6\DIFdelbegin \DIFdel{.1}\DIFdelend \DIFaddbegin \DIFadd{.2}\DIFaddend )
...
configuration file format used to \DIFdelbegin \DIFdel{setup }\DIFdelend \DIFaddbegin \DIFadd{set up }\DIFaddend the correct functioning of \DIFaddbegin \DIFadd{the }\DIFaddend macOS
...
shall be considered \DIFaddbegin \DIFadd{to be }\DIFaddend documentation or implementation bugs \DIFdelbegin \DIFdel{, and are requested to be
reported through }\DIFdelend \DIFaddbegin \DIFadd{which should be
reported via the }\DIFaddend \href{https://github.com/acidanthera/bugtracker}{Acidanthera Bugtracker}.
\DIFdelbegin \DIFdel{Errata }\DIFdelend \DIFaddbegin \DIFadd{An errata }\DIFaddend sheet is available in
\href{https://github.com/acidanthera/OpenCorePkg/blob/master/Docs/Errata/Errata.pdf}{OpenCorePkg repository}.

This document is structured as a specification \DIFdelbegin \DIFdel{, }\DIFdelend and is not meant to provide a \DIFdelbegin \DIFdel{step by
step algorithm for configuring }\DIFdelend \DIFaddbegin \DIFadd{step-by-step
guide to configuring an }\DIFaddend end-user \DIFdelbegin \DIFdel{board support package }\DIFdelend \DIFaddbegin \DIFadd{Board Support Package }\DIFaddend (BSP). The intended audience
of the document \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is anticipated to be }\DIFaddend programmers and engineers with \DIFaddbegin \DIFadd{a }\DIFaddend basic understanding of macOS internals
and UEFI \DIFdelbegin \DIFdel{functioning}\DIFdelend \DIFaddbegin \DIFadd{functionality}\DIFaddend . For these reasons\DIFaddbegin \DIFadd{, }\DIFaddend this document is available exclusively in English,
and all other sources or translations of this document are unofficial and may
contain errors.

Third-party articles, utilities, books, and alike\DIFaddbegin \DIFadd{, }\DIFaddend may be more useful for a wider audience as
they could provide guide-like material. However, they are \DIFdelbegin \DIFdel{prone }\DIFdelend \DIFaddbegin \DIFadd{subject }\DIFaddend to their authors' preferences,
tastes, \DIFdelbegin \DIFdel{this document misinterpretation, and essential }\DIFdelend \DIFaddbegin \DIFadd{misinterpretations of this document, and unavoidable }\DIFaddend obsolescence.
In \DIFdelbegin \DIFdel{case you use these sources, for example, }\DIFdelend \DIFaddbegin \DIFadd{cases of using such sources, such as }\DIFaddend \href{https://dortania.github.io}{Dortania}'s
...
please \DIFdelbegin \DIFdel{ensure to follow this document for every made decision and judge its }\DIFdelend \DIFaddbegin \DIFadd{refer back to this document on every decision made and re-evaluate potential }\DIFaddend consequences.

\DIFdelbegin \DIFdel{Be warned }\DIFdelend \DIFaddbegin \DIFadd{Please note }\DIFaddend that regardless of the sources used\DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{, users }\DIFaddend are required to fully understand every
\DIFdelbegin \DIFdel{dedicated }\DIFdelend OpenCore configuration option\DIFdelbegin \DIFdel{and concept prior to reporting any issues in
}\DIFdelend \DIFaddbegin \DIFadd{, and the principles behind them, before posting issues to the
...
\texttt{\\0}\DIFdelbegin \DIFdel{-termnator}\DIFdelend \DIFaddbegin \DIFadd{-terminator}\DIFaddend ) will be accessible within OpenCore.
...
for legacy systems. To run OpenCore on such a legacy system\DIFdelbegin \DIFdel{you can install
}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \texttt{OpenDuetPkg}
\DIFaddbegin \DIFadd{can be installed }\DIFaddend with a dedicated tool --- BootInstall (bundled with OpenCore).
\href{https://github.com/corpnewt/gibMacOS}{Third-party utilities} can be used to
perform this on systems \DIFdelbegin \DIFdel{different from }\DIFdelend \DIFaddbegin \DIFadd{other than }\DIFaddend macOS.
...
Add \texttt{.clang\_complete} file with similar content to \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend UDK root:
...
before sending the patch to ensure no double work and to avoid \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend patch being rejected.
...
\item
  ...
  }\href{https://github.com/acidanthera/BrightnessKeys}{\DIFadd{BrightnessKeys}} \DIFadd{instead.
}\item
...
are unsupported as they may corrupt ACPI tables and make \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend system unstable
due to area relocation. If \DIFdelbegin \DIFdel{you need such changes you may utilise }\DIFdelend \DIFaddbegin \DIFadd{such changes are needed, the utilisation of }\DIFaddend ``proxy''
patching or \DIFaddbegin \DIFadd{the padding of }\DIFaddend \texttt{NOP} \DIFaddbegin \DIFadd{to }\DIFaddend the remaining area \DIFaddbegin \DIFadd{might be taken into account}\DIFaddend .
...
If \DIFdelbegin \DIFdel{you are using this }\DIFdelend \DIFaddbegin \DIFadd{this is used }\DIFaddend for the first time on a customised firmware, there is a
list of checks to do first. Prior to starting\DIFdelbegin \DIFdel{please ensure that you have}\DIFdelend \DIFaddbegin \DIFadd{, the following requirements should be fulfilled}\DIFaddend :

\begin{itemize}
\tightlist
\item Most up-to-date UEFI firmware (check \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend motherboard vendor website).
...
\item \texttt{Above 4G Decoding} or similar enabled in firmware
  ...
  with the same issue are known, \DIFdelbegin \DIFdel{consider this option to be first to check if you
  have }\DIFdelend \DIFaddbegin \DIFadd{this option should be checked first whenever }\DIFaddend erratic boot
  failures \DIFaddbegin \DIFadd{are encountered}\DIFaddend .
...
\item \textbf{No} `slide` boot argument present in NVRAM or anywhere else.
  It is not necessary unless \DIFdelbegin \DIFdel{you cannot boot }\DIFdelend \DIFaddbegin \DIFadd{the system cannot be booted }\DIFaddend at all or
  \DIFdelbegin \DIFdel{see
  }\DIFdelend \texttt{No slide values are usable! Use custom slide!} message \DIFaddbegin \DIFadd{can be seen }\DIFaddend in the log.
...
\href{https://github.com/LongSoft/UEFITool/blob/master/UEFIPatch/patches.txt}{patching it}
  if \DIFdelbegin \DIFdel{you have enough skills and }\DIFdelend no option is available \DIFaddbegin \DIFadd{(for advanced users only)}\DIFaddend . See
\hyperref[kernelpropsquirks]{VerifyMsrE2}
  notes for more details.
\item \texttt{CSM} (Compatibility Support Module) disabled in firmware settings
  if present. \DIFdelbegin \DIFdel{You may need to flash GOP ROM on }\DIFdelend \DIFaddbegin \DIFadd{On }\DIFaddend NVIDIA 6xx/AMD 2xx or older\DIFaddbegin \DIFadd{, GOP ROM may have to be flashed first}\DIFaddend . Use
  ...
  in case \DIFdelbegin \DIFdel{you are not sure how}\DIFdelend \DIFaddbegin \DIFadd{of any potential confusion}\DIFaddend .
...
\item While it may not be required, sometimes
  \DIFdelbegin \DIFdel{you have to disable
  }\DIFdelend \texttt{Thunderbolt support}, \texttt{Intel SGX}, and \texttt{Intel Platform Trust}
  \DIFaddbegin \DIFadd{may have to be disabled }\DIFaddend in firmware settings present.
\end{itemize}

When debugging sleep issues \DIFdelbegin \DIFdel{you may want to (temporarily) disable }\DIFdelend Power Nap and automatic power off \DIFaddbegin \DIFadd{may be (temporarily) disabled}\DIFaddend ,
which appear to sometimes cause wake to black screen or boot loop
issues on older platforms. The particular issues may vary, but in general \DIFdelbegin \DIFdel{you should
check ACPI tables }\DIFdelend \DIFaddbegin \DIFadd{ACPI tables should be looked up }\DIFaddend first.
...
\item
  ...
  This is a security option \DIFdelbegin \DIFdel{allowing one to restrict }\DIFdelend \DIFaddbegin \DIFadd{that restricts the activation of }\DIFaddend single user mode
  \DIFdelbegin \DIFdel{usage
  }\DIFdelend by ignoring \texttt{CMD+S} hotkey and \texttt{-s} boot argument. The
  behaviour with this quirk enabled is supposed to match T2-based model
  behaviour. \DIFdelbegin \DIFdel{Read }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend \href{https://web.archive.org/web/20200517125051/https://support.apple.com/en-us/HT201573}{this archived article} to understand how to use single user mode with this quirk enabled.
...
\item
  ...
  This is a security option \DIFdelbegin \DIFdel{allowing one to restrict }\DIFdelend \DIFaddbegin \DIFadd{that restricts }\DIFaddend NVRAM access in macOS.
...
\item
  ...
  with Insyde firmware, like Acer V3-571G. Do not use this unless \DIFdelbegin \DIFdel{you fully understand
  the consequences }\DIFdelend \DIFaddbegin \DIFadd{a complete understanding of
  the consequences can be ensured}\DIFaddend .
...
\item
  ...
  \texttt{RebuildAppleMemoryMap} if \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware supports memory attributes table (MAT).
...
\item
  ...
  of the firmware. Do not use this \DIFdelbegin \DIFdel{unless you fully understand }\DIFdelend \DIFaddbegin \DIFadd{without a full understanding of }\DIFaddend the consequences.
...
\item
  ...
  This option performs memory map analysis of \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware and checks whether
...
\item
  ...
  and log the largest one that \DIFdelbegin \DIFdel{won't cause boot failure}\DIFdelend \DIFaddbegin \DIFadd{will not result in boot failures}\DIFaddend .
...
Device \DIFdelbegin \DIFdel{propertties }\DIFdelend \DIFaddbegin \DIFadd{properties }\DIFaddend are part of the \texttt{IODeviceTree} (\texttt{gIODT})
...
\item
  ...
  of \DIFdelbegin \DIFdel{deivce }\DIFdelend \DIFaddbegin \DIFadd{device }\DIFaddend paths to a map (\texttt{plist\ dict}) of variable names and their values
...
\item
  ...
  of \DIFdelbegin \DIFdel{deivce }\DIFdelend \DIFaddbegin \DIFadd{device }\DIFaddend paths to an array (\texttt{plist\ array}) of variable names in
...
\item
  ...
  To track the dependency order\DIFdelbegin \DIFdel{one can }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend inspect the \texttt{OSBundleLibraries} key
  in the \texttt{Info.plist} of the kext. Any kext mentioned in the
  \texttt{OSBundleLibraries} of the other kext must \DIFdelbegin \DIFdel{be }\DIFdelend precede this kext.
...
\item
  ...
  \DIFdelbegin \DIFdel{Keep in mind, }\DIFdelend \DIFaddbegin \emph{\DIFadd{Note 4}}\DIFadd{: Note }\DIFaddend that the following configurations are unsupported \DIFaddbegin \DIFadd{by XCPM }\DIFaddend (at least out of the box):

  \begin{itemize}
    \tightlist
    \item Consumer Ivy Bridge (\texttt{0x0306A9}) as Apple disabled XCPM for Ivy Bridge
      and recommends legacy power management for these CPUs. \DIFdelbegin \DIFdel{You will need to manually
      patch }\DIFdelend \texttt{\_xcpm\_bootstrap}
      \DIFdelbegin \DIFdel{to force }\DIFdelend \DIFaddbegin \DIFadd{should manually be patched to enforce }\DIFaddend XCPM on these CPUs instead of \DIFdelbegin \DIFdel{using }\DIFdelend this option.
...
\DIFaddbegin \item
  ...
  \DIFadd{on unsupported platforms, it can also be used to disable this kext in other situations
  (e.g. with }\texttt{\DIFadd{Cpuid1Data}} \DIFadd{left blank).
}
...
\item
  ...
  Certain firmwares lock \texttt{PKG\_CST\_CONFIG\_CONTROL} MSR register. \DIFdelbegin \DIFdel{To check its
  state one can use }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend bundled
  \texttt{VerifyMsrE2} tool \DIFdelbegin \DIFdel{. Select firmwares }\DIFdelend \DIFaddbegin \DIFadd{can be used to check its state. Some firmware }\DIFaddend have this
  register locked \DIFaddbegin \DIFadd{only }\DIFaddend on some cores\DIFdelbegin \DIFdel{only}\DIFdelend .
  ...
  \begin{enumerate}
    ...
    \item Open \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware image in UEFITool and find \texttt{CFG Lock} unicode string.
      If it is not present, \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware may not have this option and \DIFdelbegin \DIFdel{you should stop}\DIFdelend \DIFaddbegin \DIFadd{the process should therefore be discontinued}\DIFaddend .
    \item Extract the \texttt{Setup.bin} PE32 Image Section (the \DIFdelbegin \DIFdel{one }\DIFdelend UEFITool found) through \DIFaddbegin \DIFadd{the
      }\DIFaddend \texttt{Extract Body} menu option.
    ...
    \item Enter \texttt{setup\_var 0x123 0x00} command, where \texttt{0x123} should be replaced by
      \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend actual offset, and reboot.
  \end{enumerate}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Requirement}}\DIFadd{: 10.4 - 10.6}\\
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Requirement}: 10.6 (\DIFdelbegin \DIFdel{64-bit, }\DIFdelend not required for older)\\
...
\item
  ...
  \begin{itemize}
    ...
    \item \texttt{i386-user32} --- Use \texttt{i386} (32-bit) kernel when available
      and force the use of 32-bit userspace on 64-bit capable processors \DIFaddbegin \DIFadd{if supported
      by the operating system}\DIFaddend . On macOS 64-bit capable processors are assumed to
      support \texttt{SSSE3}. This is not the case for older 64-bit capable Pentium
      processors, which cause some applications to crash on macOS~10.6. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend behaviour
      corresponds to \texttt{-legacy} kernel boot argument. \DIFaddbegin \DIFadd{This option is unavailable
  ...
  \begin{enumerate}
    ...
    \item Determined EfiBoot version restricts architecture choice:
      \begin{itemize}
      \item 10.4-10.5 --- \texttt{i386} or \texttt{i386-user32} \DIFaddbegin \DIFadd{(only on 32-bit firmware)
      }\DIFaddend \item 10.6 \DIFdelbegin \DIFdel{-10.7 }\DIFdelend --- \texttt{i386}, \texttt{i386-user32}, or \texttt{x86\_64}
...
\item
  ...
  boot performance. This setting \DIFdelbegin \DIFdel{allows to prevent using }\DIFdelend \DIFaddbegin \DIFadd{prevents the use of }\DIFaddend faster kernel caching variants
  if slower variants are available for debugging and stability reasons. I.e.\DIFaddbegin \DIFadd{, }\DIFaddend by
  specifying \texttt{Mkext}\DIFdelbegin \DIFdel{one will disable }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \texttt{Prelinked} \DIFaddbegin \DIFadd{will be disabled }\DIFaddend for e.g. 10.6 but not \DIFaddbegin \DIFadd{for }\DIFaddend 10.7.
  ...
  \hline
  10.4 & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend (V1) & NO \DIFaddbegin \DIFadd{(V1) }\DIFaddend & --- & --- & --- & --- \\
  \hline
  10.5 & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend (V1) & NO \DIFaddbegin \DIFadd{(V1) }\DIFaddend & --- & --- & --- & --- \\
  \hline
  10.6 & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend (V2) & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES (V2) }\DIFaddend & YES & YES (V2) & YES \DIFaddbegin \DIFadd{(V2) }\DIFaddend & --- \\
  \hline
  10.7 & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES }\DIFaddend & --- & \DIFdelbegin \DIFdel{NO }\DIFdelend \DIFaddbegin \DIFadd{YES (V3) }\DIFaddend & YES & --- & YES \DIFaddbegin \DIFadd{(V3) }\DIFaddend & --- \\
  \hline
  10.8-10.9 & --- & --- & --- & YES & --- & YES \DIFaddbegin \DIFadd{(V3) }\DIFaddend & --- \\
  \hline
  10.10-10.15 & --- & --- & --- & --- & --- & YES \DIFaddbegin \DIFadd{(V3) }\DIFaddend & --- \\
  \hline
  11.0+ & --- & --- & --- & --- & --- & YES \DIFaddbegin \DIFadd{(V3) }\DIFaddend & YES \\
  \hline
  \end{tabular}
  \end{center}

  \DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: First version (V1) of 32-bit }\texttt{\DIFadd{prelinkedkernel}} \DIFadd{is unsupported
  due to kext symbol tables being corrupted by the tools. On these versions }\texttt{\DIFadd{Auto}}
  \DIFadd{will block }\texttt{\DIFadd{prelinkedkernel}} \DIFadd{booting. This also makes }\texttt{\DIFadd{keepsyms=1}} \DIFadd{for kext frames
...
overwrite OpenCore, make sure to enable it \DIFdelbegin \DIFdel{if you plan }\DIFdelend \DIFaddbegin \DIFadd{when planning }\DIFaddend to use them.
...
\item
  ...
  for Debian bootloader. This allows unusual boot paths to be \DIFdelbegin \DIFdel{automaticlly
  }\DIFdelend \DIFaddbegin \DIFadd{automatically
  }\DIFaddend discovered by the boot picker. Designwise they are equivalent to predefined blessed path, such as
...
\item
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{None} --- Avoid hibernation \DIFdelbegin \DIFdel{for your own good}\DIFdelend \DIFaddbegin \DIFadd{(Recommended)}\DIFaddend .
...
\item
  ...
  other keys instead of just \texttt{Shift} alone, which is not \DIFdelbegin \DIFdel{detectible }\DIFdelend \DIFaddbegin \DIFadd{detectable }\DIFaddend on many
...
\item
  ...
  To activate a development kernel \DIFdelbegin \DIFdel{you will need to add a }\texttt{\DIFdel{kcsuffix=development}}
  %DIFAUXCMD
\DIFdel{boot argument }\DIFdelend \DIFaddbegin \DIFadd{the boot argument }\texttt{\DIFadd{kcsuffix=development}} \DIFadd{should be added}\DIFaddend .
  Use \texttt{uname -a} command to ensure that \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend current loaded
...
\item
  ...
  in \texttt{RELEASE} builds. Use a \texttt{DEBUG} build if \DIFdelbegin \DIFdel{you need this option }\DIFdelend \DIFaddbegin \DIFadd{this option is needed}\DIFaddend .
...
\end{lstlisting}
  ...
  \texttt{DisableWatchDog} is set to \texttt{true} when \DIFdelbegin \DIFdel{you use }\DIFdelend a slow drive \DIFaddbegin \DIFadd{is used}\DIFaddend . Try to
  ...
  the relevant location (module) of the log line allowing \DIFdelbegin \DIFdel{one to better attribute }\DIFdelend \DIFaddbegin \DIFadd{better attribution of }\DIFaddend the line
...
\item
  ...
  personalised Apple Secure Boot identifiers. \DIFdelbegin \DIFdel{If you want to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend use this setting,
  ...
  To start using personalised Apple Secure Boot\DIFdelbegin \DIFdel{you will have to reinstall }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the operating system \DIFdelbegin \DIFdel{or personalise it. Until your }\DIFdelend \DIFaddbegin \DIFadd{will have
  to be reinstalled or personalised. Unless the }\DIFaddend operating system is personalised\DIFdelbegin \DIFdel{you will only be able to load }\DIFdelend \DIFaddbegin \DIFadd{,
  }\DIFaddend macOS DMG recovery \DIFdelbegin \DIFdel{. If you do not have DMG recovery you could always download it }\DIFdelend \DIFaddbegin \DIFadd{cannot be loaded. If DMG recovery is missing,
  it can be downloaded }\DIFaddend with \texttt{macrecovery} utility and put to
  \texttt{com.apple.recovery.boot} as explained in
  \hyperref[reinstallmacos]{Tips and Tricks} section. \DIFdelbegin \DIFdel{Keep in mind }\DIFdelend \DIFaddbegin \DIFadd{Note }\DIFaddend that
...
\end{lstlisting}
  ...
  When reinstalling the operating system, \DIFdelbegin \DIFdel{keep in mind }\DIFdelend \DIFaddbegin \DIFadd{note }\DIFaddend that current versions
...
\item
  ...
  the password. \DIFdelbegin \DIFdel{To perform authenticated restart one can use a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend dedicated terminal command \DIFaddbegin \DIFadd{can be used to perform authenticated restarts}\DIFaddend :
...
\item
  ...
  used for \DIFdelbegin \DIFdel{bootstraping }\DIFdelend \DIFaddbegin \DIFadd{bootstrapping }\DIFaddend OpenCore.

  \emph{Note 1}: Some \DIFdelbegin \DIFdel{firmewares }\DIFdelend \DIFaddbegin \DIFadd{firmwares }\DIFaddend may have broken NVRAM, no boot option support, or various other
  incompatibilities of any kind. While unlikely, the use of this option may even cause boot failure.
  \DIFdelbegin \DIFdel{Use at your own risk on }\DIFdelend \DIFaddbegin \DIFadd{This option should be used without any warranty exclusively on the }\DIFaddend boards known to be compatible.
...
\item \label{securevaulting}
  ...
  key embedded into \texttt{OpenCore.efi}. To embed the public key \DIFdelbegin \DIFdel{you should
  do }\DIFdelend either of the following \DIFaddbegin \DIFadd{should be performed}\DIFaddend :
...
\end{lstlisting}
  ...
  \emph{Note 1}: While it may appear obvious, \DIFdelbegin \DIFdel{but you have to use }\DIFdelend an external
  method \DIFaddbegin \DIFadd{is required }\DIFaddend to verify \texttt{OpenCore.efi} and \texttt{BOOTx64.efi} for
  secure boot path. For this\DIFdelbegin \DIFdel{you are recommended to at least }\DIFdelend \DIFaddbegin \DIFadd{, it is recommended to }\DIFaddend enable UEFI SecureBoot
  \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend a custom certificate \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{and to }\DIFaddend sign \texttt{OpenCore.efi} and \texttt{BOOTx64.efi}
  with \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend custom key. More details on customising secure boot on modern firmwares
...
\item \label{secureapplesb}
  ...
  of Apple Secure Boot. \DIFdelbegin \DIFdel{To achieve }\texttt{\DIFdel{Full Security}} %DIFAUXCMD
\DIFdel{one will need to
  also specify }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{ApECID} value \DIFaddbegin \DIFadd{must also be specified to
  achieve }\texttt{\DIFadd{Full Security}}\DIFaddend .

  Enabling Apple Secure Boot is more demanding to incorrect configurations,
  buggy macOS installations, and unsupported setups. Things to \DIFdelbegin \DIFdel{keep in mind}\DIFdelend \DIFaddbegin \DIFadd{consider}\DIFaddend :

  \begin{enumerate}
    \tightlist
    \item \DIFdelbegin \DIFdel{Just like on }\DIFdelend \DIFaddbegin \DIFadd{As with }\DIFaddend T2 Macs\DIFdelbegin \DIFdel{you will not be able to install any }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend unsigned kernel drivers and several signed kernel
    drivers\DIFaddbegin \DIFadd{, }\DIFaddend including NVIDIA Web Drivers\DIFaddbegin \DIFadd{, cannot be installed}\DIFaddend .
    ...
    \item System volume alterations on operating systems with sealing, like
      ...
      try to disable system volume encryption unless \DIFdelbegin \DIFdel{you disable }\DIFdelend Apple Secure Boot \DIFaddbegin \DIFadd{is disabled}\DIFaddend .
    \item If \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend platform requires certain settings, but they were not enabled,
      because the obvious issues did not trigger before, \DIFdelbegin \DIFdel{you may get boot failure }\DIFdelend \DIFaddbegin \DIFadd{boot failure might occur}\DIFaddend .
  ...
  Boot manifests on the \texttt{Preboot} partition causing boot failure. If \DIFdelbegin \DIFdel{you
  see the }\DIFdelend \DIFaddbegin \DIFadd{there is
  }\DIFaddend ``OCB: Apple Secure Boot prohibits this boot entry, enforcing!'' message,
  it is likely the case. When this happens\DIFdelbegin \DIFdel{you can }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend either reinstall the operating
  system or copy the manifests (files with \texttt{.im4m} extension, \DIFdelbegin \DIFdel{like
  }\DIFdelend \DIFaddbegin \DIFadd{such as
  }\DIFaddend \texttt{boot.efi.j137.im4m}) from \texttt{/usr/standalone/i386} to
  \texttt{/Volumes/Preboot/<UUID>/System/Library/CoreServices}. Here \texttt{<UUID>}
  is \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend system volume identifier. \DIFaddbegin \DIFadd{On HFS+ installations the manifests should be
  copied to }\texttt{\DIFadd{/System/Library/CoreServices}} \DIFadd{on the system volume.
}\DIFaddend 
...
\item
  ...
  \DIFdelbegin \DIFdel{your
  }\DIFdelend firmware variable services. Use it only when no hardware NVRAM implementation is provided
...
\item
  ...
  \DIFdelbegin \DIFdel{You can use }\DIFdelend \texttt{*} value \DIFaddbegin \DIFadd{can be used }\DIFaddend to accept all variables for select GUID.
...
To read NVRAM variable value from macOS\DIFdelbegin \DIFdel{one could use }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \texttt{nvram} \DIFdelbegin \DIFdel{by concatenating variable }\DIFdelend \DIFaddbegin \DIFadd{could be used
by concatenating }\DIFaddend GUID and name \DIFdelbegin \DIFdel{separated by }\DIFdelend \DIFaddbegin \DIFadd{variables separated by a }\DIFaddend \texttt{:} symbol.
...
\item
  ...
  will not enable ABC keyboard, unlike previous and subsequent macOS versions, and is thus not recommended in case \DIFdelbegin \DIFdel{you need }\DIFdelend 10.14 \DIFaddbegin \DIFadd{is needed}\DIFaddend .
\item
...
\item
  ...
  \item \DIFaddbegin \texttt{\DIFadd{arch=i386}} \DIFadd{(force kernel architecture to }\texttt{\DIFadd{i386}}\DIFadd{, see }\texttt{\DIFadd{KernelArch}}\DIFadd{)
  }\item \DIFaddend \texttt{batman=VALUE} (\texttt{AppleSmartBatteryManager} debug mask)
  ...
  \item \texttt{-no\_compat\_check} (disable model checking \DIFaddbegin \DIFadd{on 10.7+}\DIFaddend )
...
\item
  ...
  \item \texttt{serial=VALUE} --- enables serial logging.
    ...
    \item \texttt{\DIFdelbegin \DIFdel{1}\DIFdelend \DIFaddbegin \DIFadd{2}\DIFaddend } --- enables serial logging for \texttt{EXITBS:START} onwards.
...
manual, where \DIFdelbegin \DIFdel{one specifies }\DIFdelend all the values \DIFaddbegin \DIFadd{are specified }\DIFaddend (the default), and semi-automatic,
...
\item
  ...
  SMBIOS updates exclusive to macOS, avoiding a \DIFdelbegin \DIFdel{collission }\DIFdelend \DIFaddbegin \DIFadd{collision }\DIFaddend with existing
  Windows activation and custom OEM software but potentially breaking
  Apple-specific tools.
\item
  \texttt{Generic}\\
  \textbf{Type}: \texttt{plist\ \DIFdelbegin \DIFdel{dictonary}\DIFdelend \DIFaddbegin \DIFadd{dictionary}\DIFaddend }\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{SystemMemoryStatus}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  ...
  \emph{\DIFadd{Note}}\DIFadd{: On certain Mac models (namely }\texttt{\DIFadd{MacBookPro10,x}} \DIFadd{and any }\texttt{\DIFadd{MacBookAir}}\DIFadd{),
  SPMemoryReporter.spreporter will ignore }\texttt{\DIFadd{PT\_FEATURE\_HAS\_SOLDERED\_SYSTEM\_MEMORY}}
  \DIFadd{and assume that system memory is non-upgradable.
}
...
\item
  ...
  \item
    \texttt{0x00100000} --- Global reset ME \DIFdelbegin \DIFdel{Wachdog }\DIFdelend \DIFaddbegin \DIFadd{Watchdog }\DIFaddend Timer event (Same as
    \texttt{PRSTS} bit 6)
  \item
    \texttt{0x00200000} --- Global reset \DIFdelbegin \DIFdel{PowerManagment Wachdog }\DIFdelend \DIFaddbegin \DIFadd{PowerManagement Watchdog }\DIFaddend Timer
...
Loading an incompatible driver may lead \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend system to unbootable state or
...
To compile the drivers from UDK (EDK II) \DIFdelbegin \DIFdel{use }\DIFdelend the same command \DIFdelbegin \DIFdel{you normally use
}\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend for
OpenCore compilation \DIFaddbegin \DIFadd{can be taken}\DIFaddend , but choose a corresponding package:
...
on a FAT32 partition. In general it is unimportant whether the \DIFdelbegin \DIFdel{partitition }\DIFdelend \DIFaddbegin \DIFadd{partition }\DIFaddend scheme
...
\emph{Note 1}: \DIFdelbegin \DIFdel{You may have to copy }\DIFdelend \texttt{/System/Library/CoreServices/BridgeVersion.bin} \DIFaddbegin \DIFadd{should be copied
...
\href{https://github.com/acidanthera/OcBinaryData}{OcBinaryData repository}. \DIFdelbegin \DIFdel{You can find customised icons }\DIFdelend \DIFaddbegin \DIFadd{Customised icons can be found }\DIFaddend over the internet
...
\item
  ...
  drive to \DIFdelbegin \DIFdel{speedup }\DIFdelend \DIFaddbegin \DIFadd{speed up }\DIFaddend the boot process. Enable this option to be able to see all the
...
\item
  ...
  vulnerabilities, which can be used to inflict harm \DIFdelbegin \DIFdel{on your }\DIFdelend \DIFaddbegin \DIFadd{to the }\DIFaddend computer.
...
\item
  ...
  can be used to inflict harm \DIFdelbegin \DIFdel{on your }\DIFdelend \DIFaddbegin \DIFadd{to the }\DIFaddend computer. This option permits
...
\item
  ...
  This option allows to set this timeout based on \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend platform. The recommended
...
\item
  ...
  \DIFdelbegin \DIFdel{You may leave it as }\DIFdelend \DIFaddbegin \DIFadd{In case of issues, this option can be left as }\DIFaddend \texttt{0}\DIFdelbegin \DIFdel{in case there are issues}\DIFdelend .
...
\item
  ...
  To trigger the bug\DIFdelbegin \DIFdel{one should have }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend some valid boot options (e.g. OpenCore) \DIFdelbegin \DIFdel{and then
  }\DIFdelend \DIFaddbegin \DIFadd{are required. Then
  }\DIFaddend install Windows with \texttt{RequestBootVarRouting} enabled. As \DIFaddbegin \DIFadd{the }\DIFaddend Windows bootloader
  option will not be created by \DIFaddbegin \DIFadd{the }\DIFaddend Windows installer, the firmware will attempt to create
  \DIFdelbegin \DIFdel{it
  itself, and then corrupt }\DIFdelend \DIFaddbegin \DIFadd{this itself, leading to a corruption of }\DIFaddend its boot option list.
...
\item
  ...
  Simply said, \DIFdelbegin \DIFdel{you are required to enable this quirk to be able to }\DIFdelend \DIFaddbegin \DIFadd{this quirk is required to }\DIFaddend reliably
  use \href{https://support.apple.com/HT202796}{Startup Disk} preference
  pane in \DIFdelbegin \DIFdel{a }\DIFdelend firmware that is not compatible with macOS boot entries by design.
...
and CPUID are the obvious \DIFdelbegin \DIFdel{requiremenets }\DIFdelend \DIFaddbegin \DIFadd{requirements }\DIFaddend for proper functioning of an older
operating system, there are many other less obvious things to \DIFdelbegin \DIFdel{keep in mind}\DIFdelend \DIFaddbegin \DIFadd{consider}\DIFaddend .
...
}\href{https://web.archive.org/web/20170705003629/https://support.apple.com/en-us/HT204319}{\DIFadd{article}}\DIFadd{.
...
\begin{itemize}
  ...
  \item Cached kernel images often do not contain family drivers
    ...
    (\texttt{IOAudioFamily}) requiring \DIFdelbegin \DIFdel{one to use }\DIFdelend \DIFaddbegin \DIFadd{the use of }\DIFaddend \texttt{Force}
...
\begin{itemize}
  ...
  \item \DIFaddbegin \texttt{\DIFadd{SSSE3}} \DIFadd{support is a requirement for macOS 10.6 kernel
    ...
    assuming \DIFdelbegin \DIFdel{that you legally own }\DIFdelend macOS~10.6 \DIFaddbegin \DIFadd{is legally owned}\DIFaddend . Read \texttt{DIGEST.txt}
    for more details. \DIFdelbegin \DIFdel{Keep in mind, }\DIFdelend \DIFaddbegin \DIFadd{Note }\DIFaddend that these are the earliest tested
    versions of macOS~10.6 with OpenCore.
\end{itemize}

  \DIFdelbegin \DIFdel{You can also patch out model checking yourself }\DIFdelend \DIFaddbegin \DIFadd{Model checking may also be erased }\DIFaddend by editing \texttt{OSInstall.mpkg}
...
\begin{itemize}
  \item \DIFadd{All previous issues apply.
    ...
    }\href{https://mega.nz/folder/inRBTarD#zanf7fUbviwz3WHBU5xpCg}{\DIFadd{here}}\DIFadd{,
...
\begin{itemize}
  \item \DIFadd{All previous issues apply.
    ...
    }\href{https://mega.nz/folder/D3ASzLzA\#7sjYXE2X09f6aGjol\_C7dg}{\DIFadd{here}}\DIFadd{,
...
and }\DIFaddend operating system. On most x86 platforms trusted loading is implemented
...
\begin{enumerate}
\item Enable Apple Secure Boot by setting \texttt{SecureBootModel} \DIFdelbegin \DIFdel{if you need }\DIFdelend to
...
\item Disable DMG loading by setting \texttt{DmgLoading} to \texttt{Disabled}
  if \DIFdelbegin \DIFdel{you are concerned }\DIFdelend \DIFaddbegin \DIFadd{users have concerns }\DIFaddend of loading old vulnerable DMG recoveries. This is
...
\item Make sure that \DIFdelbegin \DIFdel{you do not need }\DIFdelend \texttt{Force} driver loading \DIFdelbegin \DIFdel{and can
  still boot }\DIFdelend \DIFaddbegin \DIFadd{is not needed and
  }\DIFaddend all the operating systems \DIFdelbegin \DIFdel{you need}\DIFdelend \DIFaddbegin \DIFadd{are still bootable}\DIFaddend .
\item Make sure that \texttt{ScanPolicy} restricts loading from undesired
  ...
  filesystems.
\item Sign all the installed drivers and tools with \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend private key. Do not sign
  tools that provide administrative access to \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend computer, like UEFI Shell.
\item Vault \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend configuration as explained \hyperref[securevaulting]{Vaulting}
  section.
...
\item Sign all third-party operating system (not made by Microsoft or Apple)
  bootloaders if \DIFdelbegin \DIFdel{you need them}\DIFdelend \DIFaddbegin \DIFadd{needed}\DIFaddend . For Linux there is an option to install
  Microsoft-signed Shim bootloader as explained on e.g.
  \href{https://wiki.debian.org/SecureBoot}{Debian Wiki}.
\item Enable UEFI Secure Boot in \DIFdelbegin \DIFdel{your }\DIFdelend firmware preferences and install the
  certificate with a private key\DIFdelbegin \DIFdel{you own}\DIFdelend . Details on how to generate a certificate
  can be found in various articles, like \href{https://habr.com/en/post/273497}{this one},
  and are out of the scope of this document. If \DIFdelbegin \DIFdel{you need to launch Windows you
  }\DIFdelend \DIFaddbegin \DIFadd{Windows is needed one
...
\subsection{Windows support}\label{troubleshootingwin}
  ...
  some extra precautions. Things to \DIFdelbegin \DIFdel{keep in mind}\DIFdelend \DIFaddbegin \DIFadd{consider}\DIFaddend :
  ...
  on old firmwares it may be invalid, i.e. not random. \DIFdelbegin \DIFdel{In case you still have }\DIFdelend \DIFaddbegin \DIFadd{If there still are }\DIFaddend issues,
  ...
  cases \DIFdelbegin \DIFdel{you will need }\DIFdelend Windows support software from
...
\DIFdelend \DIFaddbegin \href{https://www.7-zip.org}{\DIFadd{7-Zip}} \DIFadd{may be downloaded and installed
  ...
  \DIFdelbegin \DIFdel{In case you already have }\DIFdelend \DIFaddbegin \DIFadd{If there is }\DIFaddend a previous version of Boot Camp installed \DIFdelbegin \DIFdel{you will have to
  remove it }\DIFdelend \DIFaddbegin \DIFadd{it should be
  removed }\DIFaddend first by running \texttt{msiexec /x BootCamp.msi} command. \texttt{BootCamp.msi}
  ...
  \DIFdelbegin \DIFdel{sometimes you may have to address some of them }\DIFdelend \DIFaddbegin \DIFadd{the rest may still have to be addressed }\DIFaddend manually:
  ...
  \href{https://superuser.com/q/494432}{SuperUser} (this \DIFdelbegin \DIFdel{one }\DIFdelend is usually not needed).
  ...
  After installing Windows separately \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{the partition }\DIFaddend will have to \DIFdelbegin \DIFdel{relabel the partition }\DIFdelend \DIFaddbegin \DIFadd{be relabelled }\DIFaddend manually.
...
page. For IDA Pro\DIFdelbegin \DIFdel{you will need IDA Pro }\DIFdelend \DIFaddbegin \DIFadd{, version }\DIFaddend 7.3 or newer \DIFdelbegin \DIFdel{, refer to
}\DIFdelend \DIFaddbegin \DIFadd{is needed, and
}\DIFaddend \href{https://www.hex-rays.com/products/ida/support/tutorials/index.shtml}{Debugging the XNU Kernel with IDA Pro}
\DIFdelbegin \DIFdel{for more details}\DIFdelend \DIFaddbegin \DIFadd{may also help}\DIFaddend .

To obtain the log during boot \DIFdelbegin \DIFdel{you can make the use of }\DIFdelend serial port debugging \DIFaddbegin \DIFadd{can be used}\DIFaddend . Serial port
...
For macOS \DIFdelbegin \DIFdel{your best choice are }\DIFdelend \DIFaddbegin \DIFadd{the best choice is }\DIFaddend CP2102-based UART devices. Connect motherboard \texttt{TX}
...
incorrect. It is very common to have \texttt{GND} swapped with \texttt{RX}, thus\DIFdelbegin \DIFdel{you have
to connect }\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend motherboard ``\texttt{TX}'' \DIFaddbegin \DIFadd{must be connected }\DIFaddend to USB UART \texttt{GND}, and motherboard ``\texttt{GND}''
...
\DIFdelbegin \DIFdel{you will need }\DIFdelend \texttt{debug=0x8} boot argument \DIFaddbegin \DIFadd{is needed}\DIFaddend .
...
\item
  ...
  \begin{itemize}
  \tightlist
  \item \DIFdelbegin \DIFdel{You have a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend \texttt{DEBUG} or \texttt{NOOPT} version of OpenCore \DIFaddbegin \DIFadd{is used}\DIFaddend .
...
\item
  ...
  \item Consider using serial port to inspect early kernel boot failures. For this
    \DIFdelbegin \DIFdel{you may
    need }\DIFdelend \texttt{debug=0x108}, \texttt{serial=5}, and \texttt{msgbuf=1048576} \DIFdelbegin \DIFdel{arguments}\DIFdelend \DIFaddbegin \DIFadd{boot arguments are needed}\DIFaddend .
...
\item
  ...
  potentially including those created by macOS, \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{users }\DIFaddend are strongly encouraged to use the
  \texttt{RequestBootVarRouting} quirk, which will preserve \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend selection made in
...
\item \label{reinstallmacos}
  ...
  \DIFdelbegin \DIFdel{you may use
  }\DIFdelend \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Utilities/macrecovery/macrecovery.py}{macrecovery.py}
  \DIFdelbegin \DIFdel{,
  builtin tool}\DIFdelend \DIFaddbegin \DIFadd{can be used}\DIFaddend .
...
\item
  ...
  to \texttt{AptioMemoryFix} \DIFdelbegin \DIFdel{you may try enabling }\DIFdelend the following set of quirks \DIFaddbegin \DIFadd{should be enabled}\DIFaddend :
  \begin{itemize}
  \tightlist
