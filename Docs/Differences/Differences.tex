...
%DIF DEL PreviousConfiguration.tex   Sun Nov 29 23:41:31 2020
%DIF ADD ../Configuration.tex        Mon Nov 30 00:28:48 2020
...
\item
  ...
  otherwise equivalent to }\texttt{\DIFadd{Bootstrap}}\DIFadd{. This variant is useful for some Insyde firmwares,
  which cannot handle full device paths.
...
\item \label{secureapplesb}
  ...
  \item \texttt{x86legacy} --- \texttt{Macs and VMs without T2 chip minimum macOS 11.0\DIFaddbegin \DIFadd{.1 (20B29)}\DIFaddend }
  \end{itemize}
  ...
  \DIFaddbegin \DIFadd{Installing macOS with Apple Secure Boot enabled is not possible while using 
  HFS+ target volume. This may include HFS+ formatted drives when no spare 
  APFS drive is available.
}

  \DIFaddend Enabling Apple Secure Boot is more demanding to incorrect configurations,
