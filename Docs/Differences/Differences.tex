...
%DIF DEL PreviousConfiguration.tex   Wed May 11 12:25:56 2022
%DIF ADD ../Configuration.tex        Tue May 24 12:11:59 2022
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.8\DIFdelbegin \DIFdel{.0}\DIFdelend \DIFaddbegin \DIFadd{.1}\DIFaddend )
...
\item
  ...
  to have \texttt{kprintf} method working properly. This can be achieved by setting (i.e. \texttt{Delete}, then \texttt{Add})
  the \texttt{class-code} property of the PCI serial port device to \texttt{FFFFFFFF} in \texttt{DeviceProperties} section.
  As an alternative solution, a codeless kext \texttt{PCIeSerialDisable.kext} shown in the spoiler \texttt{PCIeSerialDisable.kext/Contents/Info.plist} at
  \href{https://github.com/acidanthera/bugtracker/issues/1954}{acidanthera/bugtracker\#1954}, may also be used.
  \DIFaddbegin \DIFadd{In addition, for certain Thunderbolt cards the IOKit personality }\texttt{\DIFadd{IOPCITunnelCompatible}} \DIFadd{also needs to be set to }\texttt{\DIFadd{true}}\DIFadd{,
  which can be done by the }\texttt{\DIFadd{PCIeSerialThunderboltEnable.kext}} \DIFadd{attached at }\href{https://github.com/acidanthera/bugtracker/issues/2003#issuecomment-1116761087}{\DIFadd{acidanthera/bugtracker\#2003}}\DIFadd{.
}\DIFaddend 
...
\item
  ...
  \DIFaddbegin \emph{\DIFadd{Note 3}}\DIFadd{: To address }\texttt{\DIFadd{AppleVTD}} \DIFadd{incompatibilities after applying this quirk,
  the }\texttt{\DIFadd{Reserved Memory Region}} \DIFadd{section of the corresponding device in the }\texttt{\DIFadd{DMAR}}
  \DIFadd{ACPI table might be removed. This table should be disassembled and edited, then recompiled to
  }\texttt{\DIFadd{AML}} \DIFadd{with tool }\texttt{\DIFadd{iASL}}\DIFadd{. For the patched }\texttt{\DIFadd{DMAR}} \DIFadd{table to be }\hyperref[acpipropsadd]{added}\DIFadd{,
  the original one should be }\hyperref[acpipropsdelete]{deleted}\DIFadd{.
  More details can be found at
  }\href{https://github.com/acidanthera/OpenCorePkg/commit/24414555f2c07e06a3674ec7a2aa1ce4860bbcc7#commitcomment-70530145}{\DIFadd{comment on commit 2441455}}\DIFadd{.
}
...
\item
  ...
  \item \DIFdelbegin \texttt{\DIFdel{ResetNvram}} %DIFAUXCMD
\DIFdel{--- this option erases certain UEFI variables and is
  normally executed by holding down the }\texttt{\DIFdel{CMD+OPT+P+R}} %DIFAUXCMD
\DIFdel{key combination during boot.
  Another way to erase UEFI variables is to choose }\texttt{\DIFdel{Reset NVRAM}} %DIFAUXCMD
\DIFdel{in the OpenCore picker.
  This option requires }\texttt{\DIFdel{AllowNvramReset}} %DIFAUXCMD
\DIFdel{to be set to }\texttt{\DIFdel{true}}%DIFAUXCMD
\DIFdel{.
  }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\DIFdelend \texttt{BootApple} --- this options performs booting to the first Apple
  ...
  a recovery partition. Hold the \texttt{CMD+R} \DIFdelbegin \DIFdel{key }\DIFdelend \DIFaddbegin \DIFadd{hotkey }\DIFaddend combination down to choose this option.
  \end{itemize}
...
\item
  \DIFdelbegin \texttt{\DIFdel{AllowNvramReset}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Allow }\texttt{\DIFdel{CMD+OPT+P+R}} %DIFAUXCMD
\DIFdel{handling and enable
  showing }\texttt{\DIFdel{NVRAM Reset}} %DIFAUXCMD
\DIFdel{entry in OpenCore picker.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\emph{\DIFdel{Note 1}}%DIFAUXCMD
\DIFdel{: It is known that some Lenovo laptops have a firmware
  bug, which makes them unbootable after performing NVRAM reset. Refer to
  }\href{https://github.com/acidanthera/bugtracker/issues/995}{\DIFdel{acidanthera/bugtracker\#995}}
  %DIFAUXCMD
\DIFdel{for details.
}%DIFDELCMD < 
...
\DIFdel{: Resetting NVRAM will also erase any boot options not backed up using
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend \texttt{AllowSetDefault}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  \DIFdelbegin \texttt{\DIFdel{AllowToggleSip}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
\DIFdel{: }\texttt{\DIFdel{plist\ boolean}}%DIFAUXCMD
...
%DIFDELCMD <   %%%
\DIFdel{This will toggle Apple NVRAM variable }\texttt{\DIFdel{csr-active-config}} %DIFAUXCMD
\DIFdel{between }\texttt{\DIFdel{0}} %DIFAUXCMD
\DIFdel{for
...
%DIFDELCMD <   %%%
\emph{\DIFdel{Note 1}}%DIFAUXCMD
\DIFdel{: It is strongly recommended not to make a habit of running macOS with
...
%DIFDELCMD <   %%%
\emph{\DIFdel{Note 2}}%DIFAUXCMD
\DIFdel{: OpenCore uses }\texttt{\DIFdel{0x27F}} %DIFAUXCMD
\DIFdel{while }\texttt{\DIFdel{csrutil disable}} %DIFAUXCMD
\DIFdel{on macOS Big Sur
  and Monterey sets }\texttt{\DIFdel{0x7F}}%DIFAUXCMD
\DIFdel{.
...
%DIFDELCMD <   \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{CSR\_ALLOW\_UNAPPROVED\_KEXTS}} %DIFAUXCMD
\DIFdel{(}\texttt{\DIFdel{0x200}}%DIFAUXCMD
\DIFdel{) is generally useful, in the case
...
\item %DIFAUXCMD
\texttt{\DIFdel{CSR\_ALLOW\_UNAUTHENTICATED\_ROOT}} %DIFAUXCMD
\DIFdel{(}\texttt{\DIFdel{0x800}}%DIFAUXCMD
\DIFdel{) is not included, as it is
...
%DIFDELCMD <   %%%
\emph{\DIFdel{Note3}}%DIFAUXCMD
\DIFdel{: For any other value which you may need to use, it is possible to
  configure }\texttt{\DIFdel{CsrUtil.efi}} %DIFAUXCMD
\DIFdel{as a }\texttt{\DIFdel{TextMode}} %DIFAUXCMD
\texttt{\DIFdel{Tools}} %DIFAUXCMD
\DIFdel{entry to configure a
  different value, e.g. use }\texttt{\DIFdel{toggle\ 0x77}} %DIFAUXCMD
\DIFdel{in }\texttt{\DIFdel{Arguments}} %DIFAUXCMD
\DIFdel{to toggle the
...
\item
  \DIFaddbegin \texttt{\DIFadd{DetectCable}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enable serial port cable detection.
}

  \DIFadd{This option will override the value of }\texttt{\DIFadd{gEfiMdeModulePkgTokenSpaceGuid.PcdSerialDetectCable}}
...
\item
  \DIFaddend \texttt{ExtendedTxFifoSize}\\
  \textbf{Type}: \texttt{plist\ integer}\\
...
& HFS file system driver with bless support. This driver is an alternative to
  ...
  a security audit. \\
\DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{\DIFadd{ResetNvramEntry}}}\textbf{\DIFadd{*}}
& \hyperref[uefiresetnvram]{OpenCore plugin} \DIFadd{implementing }\texttt{\DIFadd{OC\_BOOT\_ENTRY\_PROTOCOL}}
  \DIFadd{to add a configurable }\texttt{\DIFadd{Reset NVRAM}} \DIFadd{entry to the boot picker menu. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{\DIFadd{ToggleSipEntry}}}\textbf{\DIFadd{*}}
& \hyperref[uefitogglesip]{OpenCore plugin} \DIFadd{implementing }\texttt{\DIFadd{OC\_BOOT\_ENTRY\_PROTOCOL}}
  \DIFadd{to add a configurable }\texttt{\DIFadd{Toggle SIP}} \DIFadd{entry to the boot picker menu. }\\
\DIFaddend \href{https://github.com/acidanthera/audk}{\texttt{UsbMouseDxe}}\textbf{*}
...
\tightlist
  ...
  \item \texttt{SIPDisabled} --- \DIFdelbegin \DIFdel{Toogle }\DIFdelend \DIFaddbegin \DIFadd{Toggle }\DIFaddend SIP tool with SIP disabled.
  \item \texttt{SIPEnabled} --- \DIFdelbegin \DIFdel{Toogle }\DIFdelend \DIFaddbegin \DIFadd{Toggle }\DIFaddend SIP tool with SIP enabled.
...
\DIFaddbegin \subsection{\DIFadd{Other Boot Entry Protocol drivers}}

\DIFadd{In addition to the }\hyperref[uefilinux]{OpenLinuxBoot} \DIFadd{plugin, the following }\texttt{\DIFadd{OC\_BOOT\_ENTRY\_PROTOCOL}}
\DIFadd{plugins are made available to add optional, configurable boot entries to the OpenCore boot picker.
}

\subsubsection{\DIFadd{ResetNvramEntry}}\label{uefiresetnvram}

\DIFadd{Adds a menu entry which resets NVRAM and immediately restarts. Additionally adds support for hotkey
}\texttt{\DIFadd{CMD+OPT+P+R}} \DIFadd{to perform the same action. Note that on some combinations of firmware and
drivers, the }\texttt{\DIFadd{TakeoffDelay}} \DIFadd{option must be configured in order for this
and other builtin hotkeys to be reliably detected.
}

\emph{\DIFadd{Note 1}}\DIFadd{: It is known that some Lenovo laptops have a firmware
bug, which makes them unbootable after performing NVRAM reset. Refer to
}\href{https://github.com/acidanthera/bugtracker/issues/995}{\DIFadd{acidanthera/bugtracker\#995}}
\DIFadd{for details.
}

\emph{\DIFadd{Note 2}}\DIFadd{: If }\texttt{\DIFadd{LauncherOption}} \DIFadd{is set to }\texttt{\DIFadd{Full}} \DIFadd{or }\texttt{\DIFadd{Short}} \DIFadd{then the
OpenCore boot entry is protected. Resetting NVRAM will normally erase any other boot options
not specified via }\texttt{\DIFadd{BlessOverride}}\DIFadd{, for example Linux installations to custom locations
and not using the }\texttt{\DIFadd{OpenLinuxBoot}} \DIFadd{driver, or user-specified UEFI boot menu entries.
To obtain reset NVRAM functionality which does not remove other boot options, it is possible
to use the }\texttt{\DIFadd{-}{}\DIFadd{-preserve-boot}} \DIFadd{option (though see the warning specified).
}
...
\begin{itemize}
  \item \texttt{\DIFadd{-}{}\DIFadd{-preserve-boot}} \DIFadd{- Boolean flag, enabled if present. }\medskip

  \DIFadd{If enabled, BIOS boot entries are not cleared during NVRAM reset. This option should
  be used with caution, as some boot problems can be fixed by clearing these entries. }\medskip

  \item \texttt{\DIFadd{-}{}\DIFadd{-apple}} \DIFadd{- Boolean flag, enabled if present. }\medskip

  \DIFadd{On Apple firmware only, this performs a system NVRAM reset. This can result in
  additional, desirable operations such as NVRAM garbage collection. This is achieved by
  setting the }\texttt{\DIFadd{ResetNVRam}} \DIFadd{NVRAM variable.
  Where available, this has the same effect as pressing }\texttt{\DIFadd{CMD+OPT+P+R}} \DIFadd{during native boot,
  although note that if accessed from the menu entry only one boot chime will be heard. }\medskip

  \emph{\DIFadd{Note 1}}\DIFadd{: Due to using system NVRAM reset, this option is not compatible with the }\texttt{\DIFadd{-}{}\DIFadd{-preserve-boot}}
  \DIFadd{option and will override it, therefore all BIOS boot entries will be removed. }\medskip

  \emph{\DIFadd{Note 2}}\DIFadd{: Due to using system NVRAM reset, the OpenCore boot option
  cannot be preserved and OpenCore will have to either be reselected in the native boot picker or
  re-blessed. }\medskip

  \emph{\DIFadd{Note 3}}\DIFadd{: On non-Apple hardware, this option will still set this variable but the
  variable will not be recognised by the firmware and no NVRAM reset will happen. }\medskip
\end{itemize}

\subsubsection{\DIFadd{ToggleSipEntry}}\label{uefitogglesip}

\DIFadd{Provides a boot entry for enabling and disabling System Integrity Protection (SIP) in OpenCore picker.
}

\DIFadd{While macOS is running, SIP involves multiple configured software protection systems, however
all the information about which of these protections to enable is stored in the single Apple NVRAM variable
}\texttt{\DIFadd{csr-active-config}}\DIFadd{. As long as this variable is set before macOS startup,
...
}\texttt{\DIFadd{csrutil}} \DIFadd{command in macOS Recovery.
}

\texttt{\DIFadd{csr-active-config}}  \DIFadd{will be toggled between }\texttt{\DIFadd{0}} \DIFadd{for enabled, and a user-specified or
default value for disabled. The default value is }\texttt{\DIFadd{0x27F}} \DIFadd{(see below).
Any other required value can be specified as a single number in the }\texttt{\DIFadd{Arguments}} \DIFadd{for this driver.
This can be specified as hexadecimal, beginning with }\texttt{\DIFadd{0x}}\DIFadd{, or as decimal.
}

\emph{\DIFadd{Note 1}}\DIFadd{: It is recommended not to run macOS with
SIP disabled. Use of this boot option may make it easier to quickly disable SIP
protection when genuinely needed - it should be re-enabled again afterwards.
}

\emph{\DIFadd{Note 2}}\DIFadd{: The default value for disabling SIP with this boot entry is }\texttt{\DIFadd{0x27F}}\DIFadd{.
For comparison, }\texttt{\DIFadd{csrutil disable}} \DIFadd{with no other arguments on macOS Big Sur and Monterey
sets }\texttt{\DIFadd{0x7F}}\DIFadd{, and on Catalina it sets }\texttt{\DIFadd{0x77}}\DIFadd{. The OpenCore default value of }\texttt{\DIFadd{0x27F}}
\DIFadd{is a variant of the Big Sur and Monterey value, chosen as follows:
}\begin{itemize}
\tightlist
\item \texttt{\DIFadd{CSR\_ALLOW\_UNAPPROVED\_KEXTS}} \DIFadd{(}\texttt{\DIFadd{0x200}}\DIFadd{) is included in the default value,
...
}\item \texttt{\DIFadd{CSR\_ALLOW\_UNAUTHENTICATED\_ROOT}} \DIFadd{(}\texttt{\DIFadd{0x800}}\DIFadd{) is not included in the default
value, as it is very easy when using it to inadvertently break OS seal and prevent incremental OTA updates.
}\item \DIFadd{If unsupported bits from a later OS are specified in }\texttt{\DIFadd{csr-active-config}}
\DIFadd{(e.g. specifying }\texttt{\DIFadd{0x7F}} \DIFadd{on Catalina) then }\texttt{\DIFadd{csrutil status}}
\DIFadd{will report that SIP has a non-standard value, however protection will be functionally the same.
}\end{itemize}

\DIFaddend \subsection{AudioDxe}\label{uefiaudio}
...
\begin{itemize}
  ...
  in decimal, e.g. \texttt{-{}-gpio-pins=0x12} or \texttt{-{}-gpio-pins=18}. \DIFaddbegin \medskip
\DIFaddend 
...
\item
  ...
  is \DIFdelbegin \DIFdel{specific to }\DIFdelend \DIFaddbegin \DIFadd{only provided natively by }\DIFaddend Gibraltar machines (T2)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, however versions of macOS
  which support VoiceOver will see and use the implementation provided by OpenCore,
  on screens such as FileVault 2 unlock. VoiceOver }\DIFaddend is not supported before
