...
%DIF DEL PreviousConfiguration.tex   Tue Apr  6 17:30:22 2021
%DIF ADD ../Configuration.tex        Tue Apr  6 22:57:35 2021
...
\begin{enumerate}
  ...
  fast boot times, this actually means that the OpenCore re-implementation will be found and usedexcept in the case that OpenCore was been explicitly selected and started (not just auto-booted) from
  ...
  One some Macs (e.g. Mac Pro) this will find the Apple implementation. On both older and
  ...
  }\DIFaddend \item \texttt{Builtin} ---  Use OpenCore's updated re-implementation of the Apple Event protocol.
  \DIFdelbegin \DIFdel{Recommended}\DIFdelend \DIFaddbegin \DIFadd{Using this setting is recommended even on Apple hardware where the OEM version can be used, due to
  improvements (better fine mouse control, configurable key delays) made in the OC re-implementation
  of the protocol}\DIFaddend .
  \item \texttt{OEM} --- Assume Apple's protocol will be available at driver connection. This results in Apple's
  implementation being \DIFdelbegin \DIFdel{reliably used on }\DIFdelend \DIFaddbegin \DIFadd{used on most (except the oldest) }\DIFaddend Apple systems. It results in no keyboard or mouse support
  otherwise. \DIFaddbegin \DIFadd{On all Apple hardware where a recent enough version of the OEM protocol is available -- whether
  connected automatically by Apple's firmware or not -- this option will work reliably; nevertheless for
  the reasons stated }\texttt{\DIFadd{Builtin}} \DIFadd{is recommended in preference to this option in most use cases.
...
\item
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{Auto} --- Treated as \texttt{Enabled} when \texttt{KeySupport} is \texttt{true} and \texttt{Disabled} otherwise.
  \item \texttt{Enabled} --- The values \texttt{KeyInitialDelay} and \texttt{KeySubsequentDelay} are used.
  \item \texttt{Disabled} --- Apple default values of 500ms (\texttt{50}) and 50ms (\texttt{5}) are used.
  \end{itemize}

\item
  \texttt{KeyInitialDelay}\\
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0} (no initial delay, immediate repeat)\\
  ...
  Configures the initial delay before key repeat. The Apple OEM default value is \texttt{50}
  (500ms).

  \emph{Note \DIFaddbegin \DIFadd{1}\DIFaddend }: When using \texttt{KeySupport}, you may find that you get an additional slow key
  repeat before normal speed key repeat starts. If so, \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend initial key repeat delay is being
...
\DIFdelbegin \DIFdel{The instructions in this note only apply on }\DIFdelend \DIFaddbegin 

  \emph{\DIFadd{Note 2}}\DIFadd{: On }\DIFaddend systems using \texttt{KeySupport}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, the recommended value is }\texttt{\DIFadd{0}}\DIFadd{.
  On other systems the recommended value is }\texttt{\DIFadd{50}}\DIFadd{, which may optionally
  be tuned up or down according to user preference.
}\DIFaddend 
...
\item
  ...
  \emph{Note \DIFaddbegin \DIFadd{1}\DIFaddend }: When using \texttt{KeySupport}, you may find that you get one additional slow key repeat
  before normal speed key repeat starts. If so, set \texttt{KeyInitialDelay} to \texttt{0}
  and set \texttt{KeySubsequentDelay} to at least the value of your \texttt{KeyForgetThreshold}
  setting. The reason for this is that the key-smoothing parameter \texttt{KeyForgetThreshold}
  ...
  The instructions in this note only apply on systems }\DIFdelend \DIFaddbegin \DIFadd{.)
}

  \emph{\DIFadd{Note 2}}\DIFadd{: On systems using }\texttt{\DIFadd{KeySupport}}\DIFadd{, the recommended setting is to use the same
  value as }\texttt{\DIFadd{KeyForgetThreshold}}\DIFadd{. This value may then be tuned upwards for slower key repeat
  if desired, but not downwards.
  On systems not }\DIFaddend using \texttt{KeySupport} \DIFaddbegin \DIFadd{the recommended starting value is }\texttt{\DIFadd{5}}\DIFadd{,
  and this may optionally be tuned up or down according to user preference}\DIFaddend .
...
\DIFaddend \item
  ...
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0}\DIFdelbegin \DIFdel{)}\DIFdelend \\
...
\item
  ...
  version\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\texttt{\DIFdel{AppleEvent}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Replaces the Apple Event protocol with a builtin
  version. This may be used to ensure FileVault 2 compatibility on VMs and legacy Macs}\DIFdelend .
