...
%DIF DEL PreviousConfiguration.tex   Fri Jun  4 23:44:14 2021
%DIF ADD ../Configuration.tex        Fri Jun  4 23:44:14 2021
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (\DIFdelbegin \DIFdel{0.6.9}\DIFdelend \DIFaddbegin \DIFadd{0.7.0}\DIFaddend )
...
\item
  ...
  \begin{itemize}
    ...
    \texttt{Cpuid1Mask}: \texttt{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}
    \DIFaddbegin \item \DIFadd{Comet Lake U62 (}\texttt{\DIFadd{0x0A0660}}\DIFadd{) to Comet Lake U42 (}\texttt{\DIFadd{0x0806EC}}\DIFadd{):}\\
    \texttt{\DIFadd{Cpuid1Data}}\DIFadd{: }\texttt{\DIFadd{EC 06 08 00 00 00 00 00 00 00 00 00 00 00 00 00}}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{ProvideCurrentCpuInfo}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Requirement}}\DIFadd{: 10.8}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Provides current CPU info to the kernel.
}

  \DIFadd{This quirk currently provides the correct TSC and FSB values to the kernel, as well
  as disables CPU topology validation.
}

  \emph{\DIFadd{Note}}\DIFadd{: These patches currently target Microsoft Hyper-V and may need to be extended for
  other purposes.
}

\item
  \DIFaddend \texttt{SetApfsTrimTimeout}\\
  \textbf{Type}: \texttt{plist\ integer}\\
...
\item
  ...
  \item \texttt{0x0001} --- \texttt{OC\_ATTR\_USE\_VOLUME\_ICON}, provides custom icons
    ...
    \DIFdelbegin \DIFdel{For }\texttt{\DIFdel{Tools}}%DIFAUXCMD
\DIFdel{, }\DIFdelend OpenCore will attempt loading a \DIFdelbegin \DIFdel{custom icon and fallback to a default icon on failure:
    }%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD <     \tightlist
%DIFDELCMD <       \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{ResetNVRAM}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{Resources\textbackslash Image\textbackslash ResetNVRAM.icns}}
        %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{ResetNVRAM.icns}} %DIFAUXCMD
\DIFdel{from icons directory.
      }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{Tools\textbackslash <TOOL\_RELATIVE\_PATH>.icns}}
        %DIFAUXCMD
\DIFdel{--- icon near the tool file with appended }\texttt{\DIFdel{.icns}} %DIFAUXCMD
\DIFdel{extension.
    }
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize} \medskip
%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{For custom boot }\texttt{\DIFdel{Entries}}%DIFAUXCMD
\DIFdel{, OpenCore will attempt loading a custom icon and fallback
...
%DIFDELCMD <       \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{<ENTRY\_PATH>.icns}} %DIFAUXCMD
\DIFdel{--- icon near the entry file with appended }\texttt{\DIFdel{.icns}} %DIFAUXCMD
\DIFdel{extension.
    }
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize} \medskip
...
%DIFDELCMD <     %%%
\DIFdel{For all other entries, OpenCore will attempt loading a volume icon }\DIFdelend \DIFaddbegin \DIFadd{volume icon }\DIFaddend by searching
  ...
  \item \texttt{0x0002} --- \texttt{OC\_ATTR\_USE\_DISK\_LABEL\_FILE}, provides custom
    \DIFdelbegin \DIFdel{rendered }\DIFdelend \DIFaddbegin \DIFadd{prerendered }\DIFaddend titles for boot entries \DIFdelbegin \DIFdel{:
    }%DIFDELCMD < \begin{itemize}
%DIFDELCMD <     \tightlist
%DIFDELCMD <       \item %%%
\item %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend \texttt{.disk\_label} (\texttt{.disk\_label\_2x})
    file \DIFdelbegin \DIFdel{near }\DIFdelend \DIFaddbegin \DIFadd{next to the }\DIFaddend bootloader for all filesystems.
    \DIFdelbegin %DIFDELCMD < \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{<TOOL\_NAME>.lbl}} %DIFAUXCMD
\DIFdel{(}\texttt{\DIFdel{<TOOL\_NAME>.l2x}}%DIFAUXCMD
\DIFdel{) file near tool for }\texttt{\DIFdel{Tools}}%DIFAUXCMD
\DIFdel{.
    }%DIFDELCMD < \end{itemize}
%DIFDELCMD <     %%%
\DIFdelend Prerendered labels can be generated via the \texttt{disklabel} utility or the \texttt{bless} command.
    When disabled or missing\DIFdelbegin \DIFdel{text labels, }\DIFdelend \DIFaddbegin \DIFadd{, label text in }\DIFaddend (\texttt{.contentDetails}
    or \texttt{.disk\_label.contentDetails}) \DIFdelbegin \DIFdel{are to be rendered instead}\DIFdelend \DIFaddbegin \DIFadd{will be rendered if present instead, otherwise
    the entry name itself will be rendered}\DIFaddend .
  ...
  content flavour the following algorithm is used:
    ...
    \begin{itemize}
    \tightlist
      \item \DIFadd{For a Tool the value is read from }\texttt{\DIFadd{Flavour}} \DIFadd{field.
      }\item \DIFadd{For an automatically discovered entry it is read from the }\texttt{\DIFadd{.contentFlavour}}
      \DIFadd{file next to the bootloader, if present.
      }\item \DIFadd{For a custom entry it is read from the }\texttt{\DIFadd{.contentFlavour}} \DIFadd{file next to the
      bootloader if }\texttt{\DIFadd{Flavour}} \DIFadd{is }\texttt{\DIFadd{Auto}}\DIFadd{, otherwise specified via the }\texttt{\DIFadd{Flavour}}
      \DIFadd{value itself.
      }\item \DIFadd{If read flavour is }\texttt{\DIFadd{Auto}} \DIFadd{or there is no }\texttt{\DIFadd{.contentFlavour}}\DIFadd{, entry
      flavour is chosen based on the entry type (e.g. Windows automatically gets Windows flavour).
    }\DIFaddend \end{itemize}
    \DIFaddbegin \medskip
\DIFaddend 

  \DIFaddbegin \DIFadd{The Flavour value is a sequence of }\texttt{\DIFadd{:}} \DIFadd{separated names limited to 64 characters of
  ...
  Example flavour values: }\texttt{\DIFadd{BigSur:Apple}}\DIFadd{, }\texttt{\DIFadd{Windows10:Windows}}\DIFadd{. }\texttt{\DIFadd{OpenShell:UEFIShell:Shell}}\DIFadd{.
  }\medskip

  \DIFadd{Using flavours means that you can switch between icon sets easily, with the flavour selecting
  the best available icons from each set. E.g. specifying icon flavour }\texttt{\DIFadd{Debian:Linux}}
  \DIFadd{will use the icon }\texttt{\DIFadd{Debian.icns}} \DIFadd{if provided, then will try }\texttt{\DIFadd{Linux.icns}}\DIFadd{, then will
  fall back to the default for an OS, which is }\texttt{\DIFadd{HardDrive.icns}}\DIFadd{.
  }\medskip

  \DIFadd{Things to keep in mind:
}

    \begin{itemize}
    \tightlist
      \DIFaddend \item \DIFaddbegin \DIFadd{For security reasons }\texttt{\DIFadd{Ext<Flavour>.icns}} \DIFadd{and }\texttt{\DIFadd{<Flavour>.icns}} \DIFadd{are both
      supported, and only }\texttt{\DIFadd{Ext<Flavour>.icns}} \DIFadd{will be used if the entry is on an external drive
      (followed by default fallback }\texttt{\DIFadd{ExtHardDrive.icns}}\DIFadd{).
      }\item \DIFadd{Where both apply }\texttt{\DIFadd{.VolumeIcon.icns}} \DIFadd{takes precence over }\texttt{\DIFadd{.contentFlavour}}\DIFadd{.
      }\item \DIFadd{In order to allow icons and audio assist to work correctly for tools (e.g. for UEFI Shell),
      system default boot entry icons (see }\texttt{\DIFadd{Docs/Flavours.md}}\DIFadd{) specified in the }\texttt{\DIFadd{Flavour}}
      \DIFadd{setting for }\texttt{\DIFadd{Tools}} \DIFadd{or }\texttt{\DIFadd{Entries}} \DIFadd{will continue to apply even when flavour is disabled.
      Non-system icons will be ignored in this  case. In addition, the flavours }\texttt{\DIFadd{UEFIShell}} \DIFadd{and
      }\texttt{\DIFadd{NVRAMReset}} \DIFadd{are given special processing, identifying their respective tools to apply correct
      audio-assist, default builtin labels, etc.
      }\item \DIFadd{A list of recommended flavours is provided in }\texttt{\DIFadd{Docs/Flavours.md}}\DIFadd{.
    }\end{itemize}
    \medskip
  \end{itemize}

\item
  \DIFaddend \texttt{PickerAudioAssist}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  ...
  \DIFdelbegin \DIFdel{The following values are supported}\DIFdelend \DIFaddbegin \DIFadd{An icon set is a directory path relative to }\texttt{\DIFadd{Resources\textbackslash Image}}\DIFadd{,
  where the icons and an optional manifest are located. It is recommended for
  the artists to use provide their sets in the }\texttt{\DIFadd{Vendor\textbackslash Set}} \DIFadd{format,
  e.g. }\texttt{\DIFadd{Acidanthera\textbackslash GoldenGate}}\DIFadd{.
}

  \DIFadd{Sample resources provided as a part of
  }\href{https://github.com/acidanthera/OcBinaryData}{\DIFadd{OcBinaryData repository}}
  \DIFadd{provide the following icon set}\DIFaddend :
  \begin{itemize}
    \tightlist
    \item \texttt{\DIFdelbegin \DIFdel{Auto}\DIFdelend \DIFaddbegin \DIFadd{Acidanthera\textbackslash GoldenGate}\DIFaddend } --- \DIFdelbegin \DIFdel{Automatically select one setof icons based on the }\texttt{\DIFdel{DefaultBackground}}
  %DIFAUXCMD
\DIFdel{colour}\DIFdelend \DIFaddbegin \DIFadd{macOS 11 styled icon set}\DIFaddend .
    \item \texttt{\DIFdelbegin \DIFdel{Default}\DIFdelend \DIFaddbegin \DIFadd{Acidanthera\textbackslash Syrah}\DIFaddend } --- \DIFdelbegin \DIFdel{Normal icon set(without prefix)}\DIFdelend \DIFaddbegin \DIFadd{macOS 10.10 styled icon set}\DIFaddend .
    \item \texttt{\DIFdelbegin \DIFdel{Old}\DIFdelend \DIFaddbegin \DIFadd{Acidanthera\textbackslash Chardonnay}\DIFaddend } --- \DIFdelbegin \DIFdel{Vintage icon set(}\texttt{\DIFdel{Old}} %DIFAUXCMD
\DIFdel{filename prefix).
  }\DIFdelend \DIFaddbegin \DIFadd{macOS 10.4 styled icon set.
  }\end{itemize}

  \DIFadd{For convenience purposes there also are predefined aliases:
  }\begin{itemize}
    \tightlist
    \DIFaddend \item \texttt{\DIFdelbegin \DIFdel{Modern}\DIFdelend \DIFaddbegin \DIFadd{Auto}\DIFaddend } --- \DIFdelbegin \DIFdel{Nouveau icon set (}\DIFdelend \DIFaddbegin \DIFadd{Automatically select one set of icons based on the }\DIFaddend \texttt{\DIFdelbegin \DIFdel{Modern}\DIFdelend \DIFaddbegin \DIFadd{DefaultBackground}\DIFaddend }
      \DIFdelbegin \DIFdel{filename prefix)}\DIFdelend \DIFaddbegin \DIFadd{colour: }\texttt{\DIFadd{Acidanthera\textbackslash GoldenGate}} \DIFadd{for Syrah Black and
      }\texttt{\DIFadd{Acidanthera\textbackslash Chardonnay}} \DIFadd{for Light Gray}\DIFaddend .
    \item \DIFdelbegin \DIFdel{Other value }\DIFdelend \DIFaddbegin \texttt{\DIFadd{Default}} \DIFaddend --- \DIFdelbegin \DIFdel{Custom icon set if supported by installed resources}\DIFdelend \DIFaddbegin \texttt{\DIFadd{Acidanthera\textbackslash GoldenGate}}\DIFaddend .
  \end{itemize}
...
\DIFaddend \item
  \DIFaddbegin \texttt{\DIFadd{AllowToggleSip}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enable
...
\item
  \texttt{\DIFdelbegin \DIFdel{AdviseWindows}\DIFdelend \DIFaddbegin \DIFadd{AdviseFeatures}\DIFaddend }\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{Forces Windows support in }\DIFdelend \DIFaddbegin \DIFadd{Updates }\DIFaddend \texttt{FirmwareFeatures} \DIFaddbegin \DIFadd{with supported bits}\DIFaddend .
  ...
  \begin{itemize}
    ...
    \DIFaddbegin \item \texttt{\DIFadd{FW\_FEATURE\_SUPPORTS\_APFS}} \DIFadd{(}\texttt{\DIFadd{0x00080000}}\DIFadd{)
    - Without this bit, it is not possible to install macOS on an APFS disk.
  }\DIFaddend \end{itemize}

  \DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: On most newer firmwares these bits are already set, the option
...
\DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{\DIFadd{CsrUtil}}}\textbf{\DIFadd{*}}
& \DIFadd{Simple implementation of SIP-related features of Apple }\texttt{\DIFadd{csrutil}}\DIFadd{. }\\
...
builtin icon set. The \DIFdelbegin \DIFdel{default }\DIFdelend chosen icon set \DIFdelbegin \DIFdel{depends }\DIFdelend \DIFaddbegin \DIFadd{may depend }\DIFaddend on the \texttt{DefaultBackgroundColor}
variable value. \DIFdelbegin \DIFdel{For Light Gray }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend \texttt{\DIFdelbegin \DIFdel{Old}%DIFDELCMD < \MBLOCKRIGHTBRACE %%%
\DIFdel{icon set will be used, for other colours ---
the one without a prefix}\DIFdelend \DIFaddbegin \DIFadd{PickerVariant}} \DIFadd{for more details}\DIFaddend .

Predefined icons are saved in the \DIFaddbegin \texttt{\DIFadd{PickerVariant}}\DIFadd{-derived subdirectory of the
...
\tightlist
  ...
  \item \DIFaddbegin \texttt{\DIFadd{SIPDisabled}} \DIFadd{--- Toogle SIP tool with SIP disabled.
  }\item \texttt{\DIFadd{SIPEnabled}} \DIFadd{--- Toogle SIP tool with SIP enabled.
  }\item \DIFaddend \texttt{Shell} --- Entry with UEFI Shell name (e.g. \texttt{OpenShell}).
...
\item
  ...
  startup and during OpenCore picker \DIFdelbegin \DIFdel{dusplay}\DIFdelend \DIFaddbegin \DIFadd{display}\DIFaddend . Disable if not required.
