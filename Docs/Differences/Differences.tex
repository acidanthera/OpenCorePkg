...
%DIF ADD ../Configuration.tex        Sun Jun 11 05:44:24 2023
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.9\DIFdelbegin \DIFdel{.2}\DIFdelend \DIFaddbegin \DIFadd{.3}\DIFaddend )
...
& Screenshot making driver saving images to the root of OpenCore partition (ESP) or
  ...
  \DIFaddbegin \DIFadd{Accepts optional driver argument }\texttt{\DIFadd{-}{}\DIFadd{-enable-mouse-click}} \DIFadd{to additionally take
  ...
  }\DIFaddend This is a modified version of \href{https://github.com/LongSoft/CrScreenshotDxe}{\texttt{CrScreenshotDxe}}
...
\DIFaddbegin 
  ...
  custom setting of }\texttt{\DIFadd{AudioOutMask}} \DIFadd{may need to be changed to match the new channel list.
  }\medskip
...
\item
  \DIFaddbegin \texttt{\DIFadd{ConsoleFont}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty (use OpenCore builtin console font)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Specify the console font to use for OpenCore }\texttt{\DIFadd{Builtin}} \DIFadd{text renderer.
}

  \DIFadd{The font file must be located in }\texttt{\DIFadd{EFI/OC/Resources/Font/\{font-name\}.hex}} \DIFadd{and must
  be 8x16 resolution. Various console fonts can be found online in either }\texttt{\DIFadd{.bdf}} \DIFadd{or
  }\texttt{\DIFadd{.hex}} \DIFadd{format. }\texttt{\DIFadd{.bdf}} \DIFadd{can be converted to }\texttt{\DIFadd{.hex}} \DIFadd{format using }\texttt{\DIFadd{gbdfed}}
  \DIFadd{(available for Linux or macOS).
}

  \DIFadd{There is often no need to change console font, the main use-case being to
  provide an extended character set for those relatively rare EFI applications which
  have multi-lingual support (e.g. }\texttt{\DIFadd{memtest86}}\DIFadd{).
}

  \DIFadd{The }\href{https://github.com/acidanthera/OcBinaryData}{\DIFadd{OcBinaryData repository}} \DIFadd{includes:
  }\begin{itemize}
    \tightlist
    \item \href{https://terminus-font.sourceforge.net/}{\DIFadd{Terminus}} \DIFadd{--- A font with extensive character
    support suitable for applications such as the above.
    }\item \DIFadd{TerminusCore --- A lightly modified version of the Terminus font, making some glyphs
    (}\texttt{\DIFadd{@KMRSTVWimrsw}}\DIFadd{) more similar to the free ISO Latin font used in XNU and OpenCore.
  }\end{itemize}

  \texttt{\DIFadd{Terminus}} \DIFadd{and }\texttt{\DIFadd{TerminusCore}} \DIFadd{are provided under the SIL Open Font License, Version 1.1.
  Some additional GPL licensed fonts from the EPTO Fonts library, converted
  to the required }\texttt{\DIFadd{.hex}} \DIFadd{format, can be found }\href{https://github.com/mikebeaton/epto-fonts}{\DIFadd{here}}\DIFadd{.
}

  \emph{\DIFadd{Note 1}}\DIFadd{: On many newer systems the }\texttt{\DIFadd{System}} \DIFadd{text renderer already provides a full
  set of international characters, in which case this can be used without requiring the }\texttt{\DIFadd{Builtin}}
  \DIFadd{renderer and a custom font.
}

  \emph{\DIFadd{Note 2}}\DIFadd{: This option only affects the }\texttt{\DIFadd{Builtin}} \DIFadd{text renderer and only takes effect from
  the point at which the }\texttt{\DIFadd{Builtin}} \DIFadd{renderer is configured. When console output is visible before
...
\item
  ...
  applications to receive notifications from the macOS bootloader\DIFaddbegin \DIFadd{.
}
...
\item
  ...
  \textbf{\DIFadd{Description}}\DIFadd{: Replaces functions in CpuIo and PciRootBridgeIo with
  64-bit MMIO compatible ones to fix }\texttt{\DIFadd{Invalid Parameter}} \DIFadd{when using 4G Decoding.
  This affects UEFI drivers such as }\texttt{\DIFadd{AudioDxe}} \DIFadd{which access 64-bit MMIO devices.
  Platforms earlier than APTIO V (Haswell and older) are typically affected}\DIFaddend .
