...
%DIF ADD ../Configuration.tex        Sat Oct  3 09:38:10 2020
...
\item
  ...
  \begin{itemize}
    ...
    }\DIFaddend \item Enabling \DIFaddbegin \DIFadd{support of a CPU model that is not yet supported by a specific version of macOS which usually are old.
...
\item
  ...
  \textbf{\DIFadd{Description}}\DIFadd{: Emulates CPUID and apply }\texttt{\DIFadd{DummyPowerManagement}} \DIFadd{on specified macOS version or older.
}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty string}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Emulates CPUID and apply }\texttt{\DIFadd{DummyPowerManagement}} \DIFadd{on specified macOS version or newer.
}
...
\item \label{secureapplesb}
  ...
  \begin{enumerate}
    \tightlist
    \item Just like on T2 Macs you will not be able to install any unsigned kernel
  ...
  is your system volume identifier. \DIFaddbegin \DIFadd{On HFS+ installations the manifests should be
...
OpenCore is designed to provide a secure boot chain between your firmware
...
\begin{enumerate}
\item Enable Apple Secure Boot by setting \texttt{SecureBootModel} if you need to
...
\item Disable DMG loading by setting \texttt{DmgLoading} to \texttt{Disabled}
  if you are concerned of loading old vulnerable DMG recoveries. This is
...
\item Make sure that you do not need \texttt{Force} driver loading and can
...
\item Make sure that \texttt{ScanPolicy} restricts loading from undesired
  ...
  filesystems.
\item Sign all the installed drivers and tools with your private key. Do not sign
  tools that provide administrative access to your computer, like UEFI Shell.
\item Vault your configuration as explained \hyperref[securevaulting]{Vaulting}
  section.
...
\item Enable UEFI Secure Boot in your firmware preferences and install the
  ...
  \href{http://go.microsoft.com/fwlink/?LinkId=321194}{Microsoft UEFI Driver Signing CA}.
...
\item
  ...
  \item Consider using serial port to inspect early kernel boot failures. For this you may
    need \texttt{debug=0x108}, \texttt{serial=5}, and \texttt{msgbuf=1048576} \DIFaddbegin \DIFadd{boot }\DIFaddend arguments.
