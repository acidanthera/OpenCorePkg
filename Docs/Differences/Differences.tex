...
%DIF DEL PreviousConfiguration.tex   Wed Mar 31 14:49:52 2021
%DIF ADD ../Configuration.tex        Wed Mar 31 14:49:52 2021
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.6\DIFdelbegin \DIFdel{.7}\DIFdelend \DIFaddbegin \DIFadd{.8}\DIFaddend )
...
This document provides information on the \DIFaddbegin \DIFadd{format of the
}\DIFaddend \href{https://github.com/acidanthera/OpenCorePkg}{OpenCore} user
configuration file \DIFdelbegin \DIFdel{format }\DIFdelend used to set up the correct functioning of the macOS
...
\DIFdelbegin \DIFdel{tastes, }\DIFdelend misinterpretations of this document, and unavoidable obsolescence.
...
please refer back to this document on every decision made and re-evaluate potential \DIFdelbegin \DIFdel{consequences}\DIFdelend \DIFaddbegin \DIFadd{implications}\DIFaddend .
...
\DIFdelbegin \DIFdel{Depending on }\DIFdelend \DIFaddbegin \DIFadd{Subject to the }\DIFaddend OpenCore bootstrapping mechanism, the presence of multiple
...
The recommended \DIFdelbegin \DIFdel{practice for }\DIFdelend \DIFaddbegin \DIFadd{approach to }\DIFaddend interpreting \texttt{invalid\ values}
...
\item
  ...
  Refer to the \texttt{LauncherOption} property for \DIFdelbegin \DIFdel{more }\DIFdelend details.
\item
  \texttt{boot} \\
...
\item
  ...
  for screen reader support. \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[uefiaudioprops]{\texttt{UEFI Audio Properties}}
  section for \DIFdelbegin \DIFdel{more }\DIFdelend details. This directory also contains image files
  for graphical user interface. \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[ueficanopy]{OpenCanopy} section for \DIFdelbegin \DIFdel{more }\DIFdelend details.
\item
  \texttt{Tools} \\
...
%DIF > -I/UefiPackages/OpenCorePkg/Include/Nvidia
...
The \href{https://uefi.org/specifications}{ACPI specification} \DIFdelbegin \DIFdel{define the
}\DIFdelend \DIFaddbegin \DIFadd{defines
}\DIFaddend standard tables (e.g.~\texttt{DSDT}, \texttt{SSDT}, \texttt{FACS}, \texttt{DMAR})
and various methods (e.g. \texttt{\_DSM}, \texttt{\_PRW}) for implementation.
Modern hardware needs \DIFdelbegin \DIFdel{little }\DIFdelend \DIFaddbegin \DIFadd{few }\DIFaddend changes to maintain ACPI compatibility \DIFdelbegin \DIFdel{, yet
some
of those }\DIFdelend \DIFaddbegin \DIFadd{and some
options for such changes }\DIFaddend are provided as \DIFdelbegin \DIFdel{a }\DIFdelend part of OpenCore.
...
\href{https://github.com/daliansky/OC-little}{ACPI sample collection}. Note however that the
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values, describing each add entry.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[acpipropsadd]{Add Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values, describing each delete entry.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[acpipropsdelete]{Delete Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dictionary} values describing each patch entry.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[acpipropspatch]{Patch Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  \DIFaddbegin \texttt{\DIFadd{Base}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: Empty (Ignored)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Selects ACPI path base for patch lookup (or immediate
  replacement) by obtaining the offset to the provided path.
}

  \DIFadd{Only fully-qualified absolute paths are supported (e.g. }\texttt{\DIFadd{\textbackslash \_SB.PCI0.LPCB.HPET}}\DIFadd{).
  Currently supported object types are: }\texttt{\DIFadd{Device}}\DIFadd{, }\texttt{\DIFadd{Field}}\DIFadd{, }\texttt{\DIFadd{Method}}\DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: Use with care, not all OEM tables can be parsed. Use }\texttt{\DIFadd{ACPIe}}
  \DIFadd{utility to debug. }\texttt{\DIFadd{ACPIe}} \DIFadd{compiled  with }\texttt{\DIFadd{DEBUG=1 make}} \DIFadd{command
  produces helpful ACPI lookup tracing.
}

\item
  \texttt{\DIFadd{BaseSkip}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}} \DIFadd{(Do not skip any occurrences)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Number of found }\texttt{\DIFadd{Base}} \DIFadd{occurrences to skip before
...
\item
  ...
  \DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: Can be empty, when }\texttt{\DIFadd{Base}} \DIFadd{is specified, immediate replacement
  after }\texttt{\DIFadd{Base}} \DIFadd{lookup happens in this case.
}
...
\item
  ...
  firmware, it typically results in a need for additional patches. \DIFdelbegin \DIFdel{Modern
  firmware generally does not need this }\DIFdelend \DIFaddbegin \DIFadd{These are
  not usually needed on modern firmware }\DIFaddend and smaller patches work well on
...
%DIFDELCMD <   %%%
\DIFdel{notes for more }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\hyperref[kernelpropsquirks]{ControlMsrE2}
  \DIFadd{notes for }\DIFaddend details.
...
The specific issues may vary, but \DIFdelbegin \DIFdel{generally }\DIFdelend ACPI tables should \DIFaddbegin \DIFadd{typically }\DIFaddend be looked at first.

Here is an example of a defect found \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend some
...
\item
  \texttt{MmioWhitelist}\\
  \textbf{Type}: \texttt{plist\ array}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values,
  describing addresses critical for particular firmware functioning when
  \texttt{DevirtualiseMmio} quirk is in use.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[booterpropsmmio]{MmioWhitelist Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dictionary} values, describing each
  patch. \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[booterpropspatch]{Patch Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \textbf{Description}: \texttt{Apple} for macOS booter (\DIFdelbegin \DIFdel{generally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend \texttt{boot.efi});
...
\item
  ...
  This quirk \DIFdelbegin \DIFdel{tries to update }\DIFdelend \DIFaddbegin \DIFadd{attempts to update the }\DIFaddend memory map and memory attributes table to correct this.
...
For all kernel \DIFdelbegin \DIFdel{drivers }\DIFdelend \DIFaddbegin \DIFadd{extensions }\DIFaddend that may inspect the \texttt{IODeviceTree} plane without probing,
...
\item
  ...
  \textbf{Type}: \texttt{plist\ array}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Load selected kernel \DIFdelbegin \DIFdel{drivers from }\DIFdelend \DIFaddbegin \DIFadd{extensions (kexts) from the }\DIFaddend \texttt{OC/Kexts} directory.

  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values, describing each \DIFdelbegin \DIFdel{driver. See }\DIFdelend \DIFaddbegin \DIFadd{kext. Refer to
  }\DIFaddend the \hyperref[kernelpropsadd]{Add Properties} section below \DIFaddbegin \DIFadd{for details.
}

  \emph{\DIFadd{Note 1}}\DIFadd{: The load order is based on the order in which the kexts appear in
  the array. Hence, dependencies must appear before kexts that depend on them}\DIFaddend .
\DIFdelbegin \DIFdel{Kernel driver load order follows the item order in the array, thus the dependencies should be written
  prior to their consumers.
}\DIFdelend 

  \DIFaddbegin \emph{\DIFadd{Note 2}}\DIFadd{: }\DIFaddend To track the dependency order, inspect the \texttt{OSBundleLibraries}
  key in the \texttt{Info.plist} \DIFaddbegin \DIFadd{file }\DIFaddend of the kext \DIFaddbegin \DIFadd{being added}\DIFaddend . Any kext \DIFdelbegin \DIFdel{mentioned in the }\texttt{\DIFdel{OSBundleLibraries}} %DIFAUXCMD
\DIFdel{of the other kext must precede this kext}\DIFdelend \DIFaddbegin \DIFadd{included
  under the key is a dependency that must appear before the kext being added}\DIFaddend .

  \emph{Note \DIFaddbegin \DIFadd{3}\DIFaddend }: Kexts may have inner kexts (\texttt{\DIFdelbegin \DIFdel{Plug-Ins}\DIFdelend \DIFaddbegin \DIFadd{Plugins}\DIFaddend }) \DIFdelbegin \DIFdel{in their bundle.
  Each
  inner kext }\DIFdelend \DIFaddbegin \DIFadd{included in the bundle.
  Such }\texttt{\DIFadd{Plugins}} \DIFaddend must be added separately \DIFaddbegin \DIFadd{and follow the same global ordering
  rules as other kexts}\DIFaddend .

\item
  \texttt{Block}\\
  \textbf{Type}: \texttt{plist\ array}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Remove selected kernel \DIFdelbegin \DIFdel{drivers from }\DIFdelend \DIFaddbegin \DIFadd{extensions (kexts) from the }\DIFaddend prelinked kernel.

  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dictionary} values, describing each blocked \DIFdelbegin \DIFdel{driver.
  See }\DIFdelend \DIFaddbegin \DIFadd{kext.
  Refer to }\DIFaddend the \hyperref[kernelpropsblock]{Block Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \textbf{Type}: \texttt{plist\ array}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Load kernel \DIFdelbegin \DIFdel{drivers from }\DIFdelend \DIFaddbegin \DIFadd{extensions (kexts) from the }\DIFaddend system volume if they are not cached.

  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values, describing each \DIFdelbegin \DIFdel{driver.
  See }\DIFdelend \DIFaddbegin \DIFadd{kext.
  Refer to }\DIFaddend the \hyperref[kernelpropsforce]{Force Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
  This section resolves the problem of injecting \DIFdelbegin \DIFdel{drivers }\DIFdelend \DIFaddbegin \DIFadd{kexts }\DIFaddend that depend on other
  \DIFdelbegin \DIFdel{drivers}\DIFdelend \DIFaddbegin \DIFadd{kexts}\DIFaddend , which are not \DIFdelbegin \DIFdel{cached otherwise }\DIFdelend \DIFaddbegin \DIFadd{otherwise cached}\DIFaddend . The issue \DIFdelbegin \DIFdel{normally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend affects older
...
\DIFdelbegin \DIFdel{The kernel driver load order follows the item order in the array, thus the dependencies should be written prior to their consumers.
  ...
  \emph{\DIFadd{Note 2}}\DIFadd{: }\DIFaddend \texttt{Force} happens before \texttt{Add}.

  \emph{Note \DIFaddbegin \DIFadd{3}\DIFaddend }: The signature of the ``forced'' \DIFdelbegin \DIFdel{kernel drivers }\DIFdelend \DIFaddbegin \DIFadd{kext }\DIFaddend is not checked \DIFdelbegin \DIFdel{anyhow,
  making the use of }\DIFdelend \DIFaddbegin \DIFadd{in any way.
  This makes using }\DIFaddend this feature extremely dangerous and \DIFdelbegin \DIFdel{undesired }\DIFdelend \DIFaddbegin \DIFadd{undesirable }\DIFaddend for secure boot.
\DIFaddbegin 

  \emph{\DIFadd{Note 4}}\DIFadd{: }\DIFaddend This feature may not work on encrypted partitions in newer operating systems.
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dictionary} values, describing each patch.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[kernelpropspatch]{Patch Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{This kernel driver will not be added unless set }\DIFdelend \DIFaddbegin \DIFadd{Set }\DIFaddend to \texttt{true} \DIFaddbegin \DIFadd{to add this kernel extension}\DIFaddend .
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Adds kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend on specified macOS version or older.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Adds kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend on specified macOS version or newer.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{This kernel driver will not be blocked unless set }\DIFdelend \DIFaddbegin \DIFadd{Set }\DIFaddend to \texttt{true} \DIFaddbegin \DIFadd{to block this kernel extension}\DIFaddend .
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Blocks kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend on specified macOS version or older.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: Empty\\
  \textbf{Description}: Blocks kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend on specified macOS version or newer.
...
\item
  ...
  \emph{Note 2}: \DIFdelbegin \DIFdel{Normally it is only }\DIFdelend \DIFaddbegin \DIFadd{Only }\DIFaddend the value of \texttt{EAX}\DIFdelbegin \DIFdel{that needs to be taken care of, since it }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend represents the full CPUID\DIFdelbegin \DIFdel{. The remaining bytes are to }\DIFdelend \DIFaddbegin \DIFadd{,
  typically needs to be accounted for and remaining bytes should }\DIFaddend be left as zeroes.
  \DIFdelbegin \DIFdel{Byte }\DIFdelend \DIFaddbegin \DIFadd{The byte }\DIFaddend order is Little Endian\DIFdelbegin \DIFdel{, so for }\DIFdelend \DIFaddbegin \DIFadd{. For }\DIFaddend example, \texttt{C3 06 03 00} stands for CPUID
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{This kernel driver will not be added }\DIFdelend \DIFaddbegin \DIFadd{Set to }\texttt{\DIFadd{true}} \DIFadd{to load this kernel extension from the
  system volume }\DIFaddend when not present \DIFdelbegin \DIFdel{unless set to }\texttt{\DIFdel{true}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{in the kernel cache}\DIFaddend .
...
\DIFaddend \item
  ...
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\DIFaddend \texttt{AppleCpuPmCfgLock} description for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item
  ...
  This is \DIFdelbegin \DIFdel{normally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend used in conjunction with \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Emulate} section on Haswell-E,
...
\item
  ...
  for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item
  ...
  \DIFdelbegin \DIFdel{defects }\DIFdelend \DIFaddbegin \DIFadd{flaws }\DIFaddend resulting in a failure to properly execute the server detection in the first place.
...
\item
  ...
  if slower variants are available for debugging and stability reasons. \DIFdelbegin \DIFdel{I.e.}\DIFdelend \DIFaddbegin \DIFadd{That is}\DIFaddend , by
...
Refer to the \texttt{ScanPolicy} property for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\begin{itemize}
\tightlist
  \item Refer to the \texttt{HideAuxiliary} section for \DIFdelbegin \DIFdel{more }\DIFdelend details.
\end{itemize} \medskip
\end{itemize}
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ string} entries containing
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values, describing each load entry.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[miscentryprops]{Entry Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \emph{Note 1}: Some types of firmware may have \DIFdelbegin \DIFdel{defective NVRAM implementation }\DIFdelend \DIFaddbegin \DIFadd{NVRAM implementation flaws}\DIFaddend , no boot option
  support, or other incompatibilities. While unlikely, the use of this option may \DIFdelbegin \DIFdel{cause }\DIFdelend \DIFaddbegin \DIFadd{result in
  }\DIFaddend boot failures and should only be used exclusively on \DIFdelbegin \DIFdel{the }\DIFdelend boards known to be compatible. Refer to
  \href{https://github.com/acidanthera/bugtracker/issues/1222}{acidanthera/bugtracker\#1222}
  for some known issues \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{affecting }\DIFaddend Haswell and other boards.
...
\item
  ...
  \item \texttt{0x0001} --- \texttt{OC\_ATTR\_USE\_VOLUME\_ICON}, provides custom icons
    ...
    For \texttt{Tools}, OpenCore will attempt loading a custom icon and fallback to a default icon on failure:
    \begin{itemize}
    \tightlist
    ...
    For custom boot \texttt{Entries}, OpenCore will attempt loading a custom icon and fallback
    to the volume icon or the default icon on failure:
    \begin{itemize}
    \tightlist
      \item \texttt{<ENTRY\_PATH>.icns} --- icon near the entry file with appended \texttt{.icns} extension.
    \end{itemize} \medskip

    For all other entries, OpenCore will attempt loading a volume icon by searching
  ...
  control UI elements.
  \DIFaddbegin \item \texttt{\DIFadd{0x0020}} \DIFadd{--- }\texttt{\DIFadd{OC\_ATTR\_DEBUG\_DISPLAY}}\DIFadd{, enable display of additional
  timing and debug information, in Builtin picker in }\texttt{\DIFadd{DEBUG}} \DIFadd{and }\texttt{\DIFadd{NOOPT}}
  \DIFadd{builds only.
...
\item
  ...
  \hyperref[uefiaudioprops]{\texttt{UEFI Audio Properties}} section for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item
  ...
  In addition to \texttt{action hotkeys}, which are partially described in \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{PickerMode}
  section and are \DIFdelbegin \DIFdel{normally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend handled by Apple BDS, modifier keys handled by the operating system
  ...
  manner from within the OpenCore picker. Such extensions include support for tapping on \DIFdelbegin \DIFdel{keys in
  addition to holding and pressing }\texttt{\DIFdel{Shift}} %DIFAUXCMD
\DIFdel{along with other keys instead of only
  pressing }\DIFdelend \DIFaddbegin \DIFadd{key
  combinations before selecting the boot item, and for reliable detection of }\DIFaddend the \texttt{Shift} key
  \DIFdelbegin \DIFdel{, which is not detectable }\DIFdelend \DIFaddbegin \DIFadd{when selecting the boot item, in order to work around the fact that hotkeys which are continuously
  held during boot cannot be reliably detected }\DIFaddend on many PS/2 keyboards.
  ...
  \item \texttt{Shift\DIFaddbegin \DIFadd{+Enter}\DIFaddend }\DIFaddbegin \DIFadd{, }\texttt{\DIFadd{Shift+Index}} \DIFaddend --- safe mode\DIFaddbegin \DIFadd{, may be used in
  combination with }\texttt{\DIFadd{CTRL+Enter}}\DIFadd{, }\texttt{\DIFadd{CTRL+Index}}\DIFaddend .
  \end{itemize}
...
\item
  ...
  \item Other value --- Custom icon set if supported by installed resources.
  \end{itemize}
...
\item
  ...
  Refer to the \hyperref[troubleshootingdebug]{\texttt{Debugging}} section for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item
  ...
  bug, which makes them unbootable after performing NVRAM reset. \DIFdelbegin \DIFdel{See
  }\DIFdelend \DIFaddbegin \DIFadd{Refer to
  }\DIFaddend \href{https://github.com/acidanthera/bugtracker/issues/995}{acidanthera/bugtracker\#995}
  for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item \label{secureapplesb}
  ...
  \begin{enumerate}
    \tightlist
    \item As with T2 Macs, \DIFdelbegin \DIFdel{unsigned kernel drivers and }\DIFdelend \DIFaddbegin \DIFadd{all unsigned kernel extensions as well as }\DIFaddend several
      signed kernel \DIFdelbegin \DIFdel{drivers}\DIFdelend \DIFaddbegin \DIFadd{extensions}\DIFaddend , including NVIDIA Web Drivers, cannot be installed.
    \item The list of cached \DIFdelbegin \DIFdel{drivers }\DIFdelend \DIFaddbegin \DIFadd{kernel extensions }\DIFaddend may be different, resulting in a need
      to change the list of \texttt{Added} or \texttt{Forced} kernel \DIFdelbegin \DIFdel{drivers}\DIFdelend \DIFaddbegin \DIFadd{extensions}\DIFaddend .
...
\DIFdelbegin \DIFdel{Has }\texttt{\DIFdel{plist\ dict}} %DIFAUXCMD
\DIFdel{type and allows to set volatile }\DIFdelend \DIFaddbegin \DIFadd{This section allows setting non-volatile }\DIFaddend UEFI variables commonly \DIFdelbegin \DIFdel{referred }\DIFdelend \DIFaddbegin \DIFadd{described
}\DIFaddend as NVRAM variables. Refer to \texttt{man\ nvram} for \DIFdelbegin \DIFdel{more details.
macOS }\DIFdelend \DIFaddbegin \DIFadd{details.
The macOS operating system }\DIFaddend extensively uses NVRAM variables for OS --- Bootloader
--- Firmware intercommunication\DIFdelbegin \DIFdel{, and thus supplying several NVRAM }\DIFdelend \DIFaddbegin \DIFadd{. Hence, the supply of several NVRAM variables
}\DIFaddend is required for \DIFdelbegin \DIFdel{proper macOS functioning }\DIFdelend \DIFaddbegin \DIFadd{the proper functioning of macOS}\DIFaddend .
...
representing which `section' \DIFaddbegin \DIFadd{the }\DIFaddend NVRAM variable belongs to. \DIFdelbegin \DIFdel{macOS uses
}\DIFdelend \DIFaddbegin \DIFadd{The macOS
operating system makes use of }\DIFaddend several GUIDs, including but not limited to:
...
\emph{Note}: Some of the variables may be added by \DIFaddbegin \DIFadd{the
}\DIFaddend \hyperref[platforminfonvram]{PlatformNVRAM} or
\hyperref[platforminfogeneric]{Generic} subsections of \DIFaddbegin \DIFadd{the
}\DIFaddend \hyperref[platforminfo]{PlatformInfo} section.
Please ensure that variables \DIFdelbegin \DIFdel{of this section never collide with them,
as }\DIFdelend \DIFaddbegin \DIFadd{set in this section do not conflict with items
in those subsections as the implementation }\DIFaddend behaviour is undefined otherwise.

\DIFdelbegin \DIFdel{For proper macOS functioning it is often required to use }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{OC\_FIRMWARE\_RUNTIME} protocol implementation\DIFaddbegin \DIFadd{, }\DIFaddend currently offered
as a part of \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{OpenRuntime} driver\DIFdelbegin \DIFdel{.
While it brings any }\DIFdelend \DIFaddbegin \DIFadd{, is often required for macOS to
function properly. While this brings many }\DIFaddend benefits, there are \DIFdelbegin \DIFdel{certain limitations which arise depending on the
use }\DIFdelend \DIFaddbegin \DIFadd{some
limitations that should be considered for certain use cases}\DIFaddend .

\begin{enumerate}
\item Not all tools may be aware of protected namespaces.\\
  When \texttt{RequestBootVarRouting} is used\DIFaddbegin \DIFadd{, }\DIFaddend \texttt{Boot}-prefixed variable access
  is restricted and protected in a separate namespace. To access the original variables\DIFdelbegin \DIFdel{tools have to }\DIFdelend \DIFaddbegin \DIFadd{,
  tools must }\DIFaddend be aware of \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{OC\_FIRMWARE\_RUNTIME} logic.
\end{enumerate}
...
\item
  ...
  \DIFdelbegin \DIFdel{Created variables get }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{EFI\_VARIABLE\_BOOTSERVICE\_ACCESS} and \texttt{EFI\_VARIABLE\_RUNTIME\_ACCESS}
  attributes \DIFaddbegin \DIFadd{of created variables are }\DIFaddend set. Variables will only be set if not present or deleted.
  \DIFdelbegin \DIFdel{I.e. }\DIFdelend \DIFaddbegin \DIFadd{That is, }\DIFaddend to overwrite an existing variable value\DIFaddbegin \DIFadd{, }\DIFaddend add the variable name to the \texttt{Delete} section.
  This approach enables \DIFdelbegin \DIFdel{to provide default values till }\DIFdelend \DIFaddbegin \DIFadd{the provision of default values until }\DIFaddend the operating system takes the lead.

  \emph{Note}: \DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{The implementation behaviour is undefined when the }\DIFaddend \texttt{plist\ key}
  does not conform to \DIFdelbegin \DIFdel{GUID format,
  behaviour is undefined}\DIFdelend \DIFaddbegin \DIFadd{the GUID format}\DIFaddend .
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Enables loading \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend NVRAM variable file named \texttt{nvram.plist}
  from EFI volume root.

  This file must have \DIFaddbegin \DIFadd{a }\DIFaddend root \texttt{plist\ dictionary} type and contain two fields:
  \begin{itemize}
  \tightlist
  ...
  Variable loading happens prior to \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Delete} (and \texttt{Add}) phases. Unless
...
\DIFaddbegin 
  ...
  \DIFaddend Third-party scripts may be used to create \texttt{nvram.plist}
  ...
  \texttt{boot-path} variable with \DIFaddbegin \DIFadd{the }\DIFaddend OpenCore EFI partition UUID.

  \textbf{Warning}: This feature \DIFdelbegin \DIFdel{is very dangerous}\DIFdelend \DIFaddbegin \DIFadd{can be dangerous, }\DIFaddend as it passes unprotected data to
  firmware variable services. \DIFdelbegin \DIFdel{Use it only }\DIFdelend \DIFaddbegin \DIFadd{Only use }\DIFaddend when no hardware NVRAM implementation is
  provided by the firmware or \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{when the NVRAM implementation }\DIFaddend is incompatible.
...
\item
  ...
  \textbf{WARNING}: Choose variables \DIFdelbegin \DIFdel{very }\DIFdelend carefully, as \DIFaddbegin \DIFadd{the }\DIFaddend nvram.plist \DIFaddbegin \DIFadd{file }\DIFaddend is not vaulted.
  For instance, do not \DIFdelbegin \DIFdel{put }\DIFdelend \DIFaddbegin \DIFadd{include }\DIFaddend \texttt{boot-args} or \texttt{csr-active-config}, as
  \DIFdelbegin \DIFdel{this can }\DIFdelend \DIFaddbegin \DIFadd{these can be used to }\DIFaddend bypass SIP.
...
\item
  ...
  \emph{Note}: \DIFdelbegin \DIFdel{It is recommended to have this value }\DIFdelend \DIFaddbegin \DIFadd{This value should be }\DIFaddend enabled on most types of firmware but \DIFdelbegin \DIFdel{it }\DIFdelend is
  left configurable \DIFaddbegin \DIFadd{to account }\DIFaddend for firmware that may have issues with NVRAM
  variable storage garbage collection or similar.

\end{enumerate}

\DIFdelbegin \DIFdel{To }\DIFdelend \DIFaddbegin \DIFadd{The }\texttt{\DIFadd{nvram}} \DIFadd{command can be used to }\DIFaddend read NVRAM variable \DIFdelbegin \DIFdel{value from macOS
, }\texttt{\DIFdel{nvram}} %DIFAUXCMD
\DIFdel{could be used
by concatenating }\DIFdelend \DIFaddbegin \DIFadd{values from macOS
by concatenating the }\DIFaddend GUID and name variables separated by a \texttt{:} symbol.
...
\textbf{Warning}: These variables may be added by \DIFaddbegin \DIFadd{the
}\DIFaddend \hyperref[platforminfonvram]{PlatformNVRAM} or
\hyperref[platforminfogeneric]{Generic} subsections of \DIFaddbegin \DIFadd{the
}\DIFaddend \hyperref[platforminfo]{PlatformInfo} section.
...
\item
  ...
  \emph{Note}: \DIFdelbegin \DIFdel{To see }\DIFdelend \DIFaddbegin \DIFadd{Enable the }\texttt{\DIFadd{AppleDebug}} \DIFadd{option to display }\DIFaddend verbose output from \texttt{boot.efi}
  on modern macOS versions\DIFdelbegin \DIFdel{enable }\texttt{\DIFdel{AppleDebug}} %DIFAUXCMD
\DIFdel{option}\DIFdelend . This will save the log to \DIFaddbegin \DIFadd{the }\DIFaddend general OpenCore log \DIFaddbegin \DIFadd{file}\DIFaddend . For versions before
  10.15.4\DIFaddbegin \DIFadd{, }\DIFaddend set \texttt{bootercfg} to \texttt{log=1}. This will print verbose output onscreen.
...
\item
  ...
  memory and is pointed \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{to by the }\DIFaddend \texttt{efiboot-perf-record-data} and \texttt{efiboot-perf-record-size}
  variables. Starting from 10.15.4\DIFaddbegin \DIFadd{, }\DIFaddend it can also be saved to \DIFaddbegin \DIFadd{the }\DIFaddend OpenCore log by \DIFaddbegin \DIFadd{setting the
  }\DIFaddend \texttt{AppleDebug} option.
\item
...
Platform information \DIFdelbegin \DIFdel{is comprised }\DIFdelend \DIFaddbegin \DIFadd{consists }\DIFaddend of several identification fields
...
\DIFdelbegin \DIFdel{To inspect SMBIOS contents }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend \href{http://www.nongnu.org/dmidecode}{dmidecode} utility can be used \DIFdelbegin \DIFdel{. Version }\DIFdelend \DIFaddbegin \DIFadd{to inspect
SMBIOS contents and a version }\DIFaddend with macOS specific enhancements can be downloaded
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Generate PlatformInfo based on \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Generic}
  section instead of using values from \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{DataHub}, \texttt{NVRAM},
  ...
  \textbf{Warning}: \DIFdelbegin \DIFdel{It is strongly discouraged to set }\DIFdelend \DIFaddbegin \DIFadd{Setting }\DIFaddend this option to \texttt{false} \DIFaddbegin \DIFadd{is strongly discouraged }\DIFaddend when
  intending to update platform information. \DIFdelbegin \DIFdel{The only reason to do so is if
  making }\DIFdelend \DIFaddbegin \DIFadd{A }\texttt{\DIFadd{false}} \DIFadd{setting is typically only
  valid for }\DIFaddend minor corrections to SMBIOS values on legacy Apple hardware. In all other
  cases\DIFaddbegin \DIFadd{, }\DIFaddend setting \texttt{Automatic} to \texttt{false} may lead to \DIFdelbegin \DIFdel{hard to debug errors
  ,
  due to }\DIFdelend \DIFaddbegin \DIFadd{hard-to-debug errors
  resulting from }\DIFaddend inconsistent or invalid settings.
...
\item
  ...
  from \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Generic} or \texttt{DataHub} sections depending on
  \DIFaddbegin \DIFadd{the setting of the }\DIFaddend \texttt{Automatic} \DIFdelbegin \DIFdel{value}\DIFdelend \DIFaddbegin \DIFadd{property}\DIFaddend .

  \emph{Note}: The implementation of the Data Hub protocol in EFI firmware on
  \DIFdelbegin \DIFdel{essentially }\DIFdelend \DIFaddbegin \DIFadd{virtually }\DIFaddend all systems, including Apple hardware, means that existing Data Hub
  entries cannot be overridden\DIFdelbegin \DIFdel{, while new }\DIFdelend \DIFaddbegin \DIFadd{. New }\DIFaddend entries are added to the end \DIFaddbegin \DIFadd{of the Data Hub
  instead, }\DIFaddend with macOS ignoring \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{old entries}\DIFaddend . This can be worked around by
  \DIFdelbegin \DIFdel{reinstalling }\DIFdelend \DIFaddbegin \DIFadd{replacing }\DIFaddend the Data Hub protocol using the \texttt{ProtocolOverrides} section.
...
\item
  ...
  These fields are read from \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Generic} or \texttt{PlatformNVRAM}
  sections depending on \DIFaddbegin \DIFadd{the setting of the }\DIFaddend \texttt{Automatic} \DIFdelbegin \DIFdel{value}\DIFdelend \DIFaddbegin \DIFadd{property}\DIFaddend .
  All the other fields are to be specified with \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{NVRAM} section.

  If \texttt{UpdateNVRAM} is set to \texttt{false}\DIFaddbegin \DIFadd{, }\DIFaddend the aforementioned
  variables can be updated with \DIFaddbegin \DIFadd{the }\DIFaddend \hyperref[nvram]{\texttt{NVRAM}}
  section. If \texttt{UpdateNVRAM} is set to \texttt{true}\DIFaddbegin \DIFadd{, }\DIFaddend the behaviour is
  undefined when any of the fields are present in \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{NVRAM} section.
\item
  \texttt{UpdateSMBIOS}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Update SMBIOS fields. These fields are read from \DIFaddbegin \DIFadd{the
  }\DIFaddend \texttt{Generic} or \texttt{SMBIOS} sections depending on
  \DIFaddbegin \DIFadd{the setting of the }\DIFaddend \texttt{Automatic} \DIFdelbegin \DIFdel{value}\DIFdelend \DIFaddbegin \DIFadd{property}\DIFaddend .
\item
  ...
  \emph{Note}: A side effect of using \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Custom} approach \DIFdelbegin \DIFdel{is making
  }\DIFdelend \DIFaddbegin \DIFadd{that it
  makes }\DIFaddend SMBIOS updates exclusive to macOS, avoiding a collision with existing
...
\item
  ...
  \DIFaddbegin \DIFadd{The }\DIFaddend SMBIOS specification did not explicitly specify the encoding format for the
  ...
  \begin{itemize}
    \tightlist
    \item Apple uses \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Big Endian} format everywhere but it ignores SMBIOS UUID within macOS.
    \item \texttt{dmidecode} uses \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Big Endian} format for SMBIOS 2.5.x or lower
    and \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Little Endian} \DIFaddbegin \DIFadd{format }\DIFaddend for 2.6 and newer. Acidanthera
    \href{https://github.com/acidanthera/dmidecode}{dmidecode} prints all \DIFdelbegin \DIFdel{the }\DIFdelend three.
    \item Windows uses \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Little Endian} format everywhere, but \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend only affects
    the visual representation of the values.
  \end{itemize}

  OpenCore always sets a recent SMBIOS version (currently 3.2) when generating
  the modified DMI tables. If \texttt{UseRawUuidEncoding} is enabled, \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Big Endian}
  format is used to store the \texttt{SystemUUID} data. Otherwise, \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{Little Endian}
  \DIFaddbegin \DIFadd{format }\DIFaddend is used.

  \emph{Note}: \DIFdelbegin \DIFdel{Since }\DIFdelend \DIFaddbegin \DIFadd{This preference does not affect }\DIFaddend UUIDs used in DataHub and NVRAM
  \DIFaddbegin \DIFadd{as they }\DIFaddend are not standardised and are added by Apple\DIFdelbegin \DIFdel{, this preference does not affect them}\DIFdelend . Unlike SMBIOS\DIFaddbegin \DIFadd{, }\DIFaddend they are
...
\item
  ...
  It \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend dangerous to use \DIFdelbegin \DIFdel{Apple }\DIFdelend \DIFaddbegin \DIFadd{``Apple'' }\DIFaddend in SMBIOS vendor fields for reasons \DIFdelbegin \DIFdel{given }\DIFdelend \DIFaddbegin \DIFadd{outlined }\DIFaddend in the
...
\item
  ...
  \begin{itemize}
    ...
    - Without this bit\DIFaddbegin \DIFadd{, }\DIFaddend it is not possible to reboot to Windows installed on
      a drive with \DIFdelbegin \DIFdel{EFI partition being }\DIFdelend \DIFaddbegin \DIFadd{an EFI partition that is }\DIFaddend not the first partition on the disk.
    \item \texttt{FW\_FEATURE\_SUPPORTS\_UEFI\_WINDOWS\_BOOT} (\texttt{0x20000000})
    - Without this bit\DIFaddbegin \DIFadd{, }\DIFaddend it is not possible to reboot to Windows installed on
      a drive with \DIFdelbegin \DIFdel{EFI partition being }\DIFdelend \DIFaddbegin \DIFadd{an EFI partition that is }\DIFaddend the first partition on the disk.
  \end{itemize}
...
\item
  ...
  Macs when using \texttt{Automatic} PlatformInfo\DIFaddbegin \DIFadd{, }\DIFaddend to avoid BIOS updates in unofficially supported macOS
  versions.
...
\item
  ...
  This controls the visibility of the Memory tab in \DIFaddbegin \DIFadd{``}\DIFaddend About This Mac\DIFaddbegin \DIFadd{''}\DIFaddend .
...
\item
  ...
  in Hz, and is normally 24 MHz for \DIFaddbegin \DIFadd{the }\DIFaddend client Intel segment, 25 MHz for \DIFaddbegin \DIFadd{the }\DIFaddend server Intel segment,
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dictionary} values, describing each
  memory device. \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[platforminfomemorydevice]{Memory Devices Properties}
...
\item
  ...
  \DIFaddbegin \DIFadd{When }\texttt{\DIFadd{Automatic}} \DIFadd{is }\texttt{\DIFadd{true}}\DIFadd{, the original value from the the corresponding Mac model will be set if available.
  Otherwise, the value from }\texttt{\DIFadd{OcMacInfoLib}} \DIFadd{will be set.
  When }\texttt{\DIFadd{Automatic}} \DIFadd{is }\texttt{\DIFadd{false}}\DIFadd{, a user-specified value will be set if available.
  Otherwise, the original value from the firmware will be set.
  If no value is provided, the fallback value (}\texttt{\DIFadd{zero}}\DIFadd{) will be set.
}
...
\item
  ...
  processor, memory, and I/O) or \texttt{0xB} (Processor/Memory Module)\DIFdelbegin \DIFdel{,
  refer }\DIFdelend \DIFaddbegin \DIFadd{.
  Refer }\DIFaddend to Table 15 -- Baseboard: Board Type for \DIFdelbegin \DIFdel{more }\DIFdelend details.
\item
...
\item
  ...
  \textbf{Description}: Chassis type\DIFdelbegin \DIFdel{, refer }\DIFdelend \DIFaddbegin \DIFadd{. Refer }\DIFaddend to Table 17 --- System
  Enclosure or Chassis Types for \DIFdelbegin \DIFdel{more }\DIFdelend details.
\item
...
\item
  ...
  \texttt{PlatformFeature}\\
  \textbf{Description}: Platform features bitmask \DIFaddbegin \DIFadd{(Missing on older Macs)}\DIFaddend . Refer to
  \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Apple/IndustryStandard/AppleFeatures.h}{AppleFeatures.h} for \DIFdelbegin \DIFdel{more details.
Missing on older Macs.
}\DIFdelend \DIFaddbegin \DIFadd{details.
}\DIFaddend \item
  \texttt{SmcVersion}\\
  \textbf{Type}: \texttt{plist\ data}, 16 bytes\\
...
\item
  ...
  for \DIFdelbegin \DIFdel{more }\DIFdelend details. Lower 32 bits match \texttt{FirmwareFeatures}. Upper
...
\item
  ...
  bitmask. Refer to
  \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Apple/IndustryStandard/AppleFeatures.h}{AppleFeatures.h}
  for \DIFdelbegin \DIFdel{more }\DIFdelend details. Lower 32 bits match \texttt{FirmwareFeaturesMask}.
...
\item
  ...
  Automatic value generation \DIFdelbegin \DIFdel{tries to provide }\DIFdelend \DIFaddbegin \DIFadd{attempts to provide the }\DIFaddend most accurate value for
  the currently installed CPU. When this fails\DIFdelbegin \DIFdel{please make sure to create
  }\DIFdelend \DIFaddbegin \DIFadd{, please raise }\DIFaddend an
  ...
  only apply if the CPU core count matches)\DIFdelbegin \DIFdel{refer to }\DIFdelend \DIFaddbegin \DIFadd{, refer to the }\DIFaddend Apple SMBIOS definitions header
...
platform firmware. This section allows \DIFdelbegin \DIFdel{to load }\DIFdelend \DIFaddbegin \DIFadd{loading }\DIFaddend additional UEFI modules \DIFdelbegin \DIFdel{and/or apply
tweaks for }\DIFdelend \DIFaddbegin \DIFadd{as well as applying
tweaks to }\DIFaddend the onboard firmware. To inspect firmware contents, apply modifications
...
Depending on the firmware\DIFaddbegin \DIFadd{, }\DIFaddend a different set of drivers may be required.
...
While some tools can be launched from within OpenCore \DIFdelbegin \DIFdel{, see more details in
}\DIFdelend \DIFaddbegin \DIFadd{(Refer to }\DIFaddend the \hyperref[misctools]{Tools} subsection
\DIFdelbegin \DIFdel{of the configuration}\DIFdelend \DIFaddbegin \DIFadd{for more details)}\DIFaddend , most should be run separately either directly or from \texttt{Shell}.
...
Predefined icons are saved in the
...
\item
  ...
  \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with string filenames meant to be loaded as UEFI drivers.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ array}\\
  \textbf{Description}: \DIFdelbegin \DIFdel{Designed to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend be filled with \texttt{plist\ dict} values,
  describing memory areas \DIFdelbegin \DIFdel{exquisite to particular }\DIFdelend \DIFaddbegin \DIFadd{exclusive to specific }\DIFaddend firmware and hardware functioning,
  which should not be used by the operating system. \DIFdelbegin \DIFdel{An example }\DIFdelend \DIFaddbegin \DIFadd{Examples }\DIFaddend of such memory \DIFdelbegin \DIFdel{region
  could be }\DIFdelend \DIFaddbegin \DIFadd{regions
  could be the }\DIFaddend second 256 MB corrupted by \DIFaddbegin \DIFadd{the }\DIFaddend Intel HD 3000 or an area with faulty RAM.
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to }\DIFaddend the \hyperref[uefirsvdprops]{ReservedMemory Properties} section below \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \DIFaddbegin \DIFadd{An }\DIFaddend APFS EFI driver is bundled in all bootable APFS containers. This
  option performs \DIFaddbegin \DIFadd{the }\DIFaddend loading of signed APFS drivers \DIFdelbegin \DIFdel{with respect to
  }\DIFdelend \DIFaddbegin \DIFadd{(consistent with the
  }\DIFaddend \texttt{ScanPolicy}\DIFdelbegin \DIFdel{. See more details in }\DIFdelend \DIFaddbegin \DIFadd{). Refer to the }\DIFaddend ``EFI Jumpstart'' section of \DIFaddbegin \DIFadd{the
  }\DIFaddend \href{https://developer.apple.com/support/apple-file-system/Apple-File-System-Reference.pdf}{Apple File System Reference} \DIFaddbegin \DIFadd{for details}\DIFaddend .
...
\item
  ...
  \DIFdelbegin \DIFdel{Instead of }\DIFdelend \DIFaddbegin \DIFadd{Every handle is connected recursively instead of the }\DIFaddend partition handle connection
  \DIFdelbegin \DIFdel{normally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend used for APFS driver loading\DIFdelbegin \DIFdel{every handle is connected recursively}\DIFdelend . This may \DIFdelbegin \DIFdel{take more time than usual
  but can }\DIFdelend \DIFaddbegin \DIFadd{result in additional time being
  taken but can sometimes }\DIFaddend be the only way to access APFS partitions on \DIFdelbegin \DIFdel{some types of firmware}\DIFdelend \DIFaddbegin \DIFadd{certain
  firmware, }\DIFaddend such as those on older HP laptops.
...
\item
  ...
  \DIFdelbegin \DIFdel{Performs APFS driver loading not only }\DIFdelend \DIFaddbegin \DIFadd{Permits APFS USB hot plug which enables loading APFS drivers, both }\DIFaddend at OpenCore
  startup \DIFdelbegin \DIFdel{but also
  during the OpenCore picker . This permits APFS USB hot plug. }\DIFdelend \DIFaddbegin \DIFadd{and during OpenCore picker dusplay. }\DIFaddend Disable if not required.
...
\item
  ...
  \DIFaddbegin \DIFadd{The }\DIFaddend APFS driver date connects \DIFaddbegin \DIFadd{the }\DIFaddend APFS driver with the calendar release date.
  \DIFdelbegin \DIFdel{Older versions of APFS drivers may contain unpatched
  vulnerabilities , which }\DIFdelend \DIFaddbegin \DIFadd{Apple ultimately drops support for older macOS releases and APFS drivers from
  such releases may contain vulnerabilities that }\DIFaddend can be used to \DIFdelbegin \DIFdel{inflict harm to the computer }\DIFdelend \DIFaddbegin \DIFadd{compromise
  a computer if such drivers are used after support ends}\DIFaddend . This option
  permits restricting APFS drivers to \DIFdelbegin \DIFdel{only recent releases}\DIFdelend \DIFaddbegin \DIFadd{current macOS versions}\DIFaddend .
...
\item
  ...
  \DIFaddbegin \DIFadd{The }\DIFaddend APFS driver version connects \DIFaddbegin \DIFadd{the }\DIFaddend APFS driver with the macOS release.
  \DIFdelbegin \DIFdel{APFS drivers from }\DIFdelend \DIFaddbegin \DIFadd{Apple ultimately drops support for }\DIFaddend older macOS releases \DIFdelbegin \DIFdel{will become
  unsupported and thus may contain unpatched vulnerabilities , which
  }\DIFdelend \DIFaddbegin \DIFadd{and APFS drivers from
  such releases may contain vulnerabilities that }\DIFaddend can be used to \DIFdelbegin \DIFdel{inflict harm to the computer }\DIFdelend \DIFaddbegin \DIFadd{compromise
  a computer if such drivers are used after support ends}\DIFaddend . This option
  permits restricting APFS drivers to \DIFdelbegin \DIFdel{only modern }\DIFdelend \DIFaddbegin \DIFadd{current }\DIFaddend macOS versions.
...
\item
  ...
  \DIFdelbegin \DIFdel{Normally this contains }\DIFdelend \DIFaddbegin \DIFadd{This typically contains the }\DIFaddend first audio codec address on the builtin analog audio controller (\texttt{HDEF}).
  ...
  As an alternative\DIFaddbegin \DIFadd{, }\DIFaddend this value can be obtained from \texttt{IOHDACodecDevice} class in I/O Registry
...
\item
  ...
  \DIFdelbegin \DIFdel{Normally this }\DIFdelend \DIFaddbegin \DIFadd{This typically }\DIFaddend contains builtin analog audio controller (\texttt{HDEF}) device path,
  ...
  As an alternative\DIFaddbegin \DIFadd{, }\DIFaddend \texttt{gfxutil -f HDEF} command can be used in macOS. Specifying \DIFaddbegin \DIFadd{an }\DIFaddend empty device
  path will result in the first available audio controller \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{being }\DIFaddend used.
...
\item
  ...
  \DIFdelbegin \DIFdel{Normally this }\DIFdelend \DIFaddbegin \DIFadd{This typically }\DIFaddend contains the index of the green out of the builtin analog audio controller (\texttt{HDEF}).
...
\item
  ...
  \DIFdelbegin \DIFdel{Screen }\DIFdelend \DIFaddbegin \DIFadd{The screen }\DIFaddend reader will use this volume level \DIFdelbegin \DIFdel{, }\DIFdelend when the calculated volume level is \DIFdelbegin \DIFdel{less }\DIFdelend \DIFaddbegin \DIFadd{lower
  }\DIFaddend than \texttt{MinimumVolume} \DIFdelbegin \DIFdel{. Boot chime sound }\DIFdelend \DIFaddbegin \DIFadd{and the boot chime }\DIFaddend will not play if the calculated
  volume level is \DIFdelbegin \DIFdel{less }\DIFdelend \DIFaddbegin \DIFadd{lower }\DIFaddend than \texttt{MinimumVolume}.
...
\item
  ...
  Enabling this setting plays \DIFdelbegin \DIFdel{boot chime through }\DIFdelend \DIFaddbegin \DIFadd{the boot chime using the }\DIFaddend builtin audio support. \DIFdelbegin \DIFdel{Volume }\DIFdelend \DIFaddbegin \DIFadd{The volume
  }\DIFaddend level is determined by \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{MinimumVolume} and \texttt{VolumeAmplifier} settings
  \DIFdelbegin \DIFdel{and
  }\DIFdelend \DIFaddbegin \DIFadd{as well as the }\DIFaddend \texttt{SystemAudioVolume} NVRAM variable. Possible values include:
  ...
  NVRAM variable to avoid conflicts when the firmware is able to play \DIFaddbegin \DIFadd{the }\DIFaddend boot chime.
...
\item
  \DIFaddbegin \texttt{\DIFadd{KeyInitialDelay}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}} \DIFadd{(acts as }\texttt{\DIFadd{50}}\DIFadd{, or 500ms)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Configure initial keyboard repeat delay in OpenCore implementation
  of Apple Event protocol, in units of 10ms.
}

  \DIFadd{In order to fine tune keyboard behaviour three values are relavent:
  }\texttt{\DIFadd{KeyForgetThreshold}}\DIFadd{, }\texttt{\DIFadd{KeyInitialDelay}} \DIFadd{and }\texttt{\DIFadd{KeySubsequentDelay}}\DIFadd{.
}

  \DIFadd{The tuning procedure is as follows:
}

  \begin{itemize}
    \tightlist
    \item \DIFadd{On systems which are not using }\texttt{\DIFadd{KeySupport}}\DIFadd{, no tuning
    is needed.
    }\item \DIFadd{On systems which are using }\texttt{\DIFadd{KeySupport}}\DIFadd{, it is important to tune
    }\texttt{\DIFadd{KeyForgetThreshold}} \DIFadd{as documented, in order to get reliable key-held
    behaviour - including reliable responses to boot hotkeys - within OC.
    }\item \DIFadd{After this, no further tuning is required for reliable operation.
    However for fine tuning, proceed as follows:
 }\end{itemize}

  \DIFadd{Apple Event protocol is used for key input in OpenCore, since we have to
  provide it for Apple's own pre-boot systems to use anyway. However when combining
  OpenCore }\texttt{\DIFadd{KeySupport}} \DIFadd{with Apple Event key handling,
  ...
  this issue, configure the value of }\texttt{\DIFadd{KeyInitialDelay}} \DIFadd{and }\texttt{\DIFadd{KeySubsequentDelay}}
  \DIFadd{to be the same as the value you have already configured for }\texttt{\DIFadd{KeyForgetThreshold}}\DIFadd{. (However,
  to avoid over-fast key repeats, if you have been able to configure }\texttt{\DIFadd{KeyForgetThreshold}}
  \DIFadd{to a value less than }\texttt{\DIFadd{5}}\DIFadd{, then you are recommended to configure }\texttt{\DIFadd{KeyInitialDelay}}
  \DIFadd{and }\texttt{\DIFadd{KeySubsequentDelay}} \DIFadd{to }\texttt{\DIFadd{5}}\DIFadd{.) Configure and test this. You should find the
  ...
  to increment the value of both settings by one or two. E.g. if your }\texttt{\DIFadd{KeyForgetThreshold}}
  \DIFadd{was }\texttt{\DIFadd{7}} \DIFadd{and you configured }\texttt{\DIFadd{KeyInitialDelay}} \DIFadd{and }\texttt{\DIFadd{KeySubsequentDelay}}
  \DIFadd{to }\texttt{\DIFadd{7}}\DIFadd{, then if you get double key press responses, try configuring values
  of }\texttt{\DIFadd{8}} \DIFadd{and }\texttt{\DIFadd{8}} \DIFadd{or }\texttt{\DIFadd{9}} \DIFadd{and }\texttt{\DIFadd{9}} \DIFadd{instead. Use the lowest values
  which work.
}

  \emph{\DIFadd{Note}}\DIFadd{: When tuning keyboard behaviour, double key responses can make it impossible to
  ...
  }\texttt{\DIFadd{Misc/Boot/Timeout}} \DIFadd{to }\texttt{\DIFadd{5}} \DIFadd{so that your normal OS will boot after a short time
  anyway.
}

\item
  \texttt{\DIFadd{KeySubsequentDelay}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}} \DIFadd{(acts as }\texttt{\DIFadd{5}}\DIFadd{, or 50ms)}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Configure subsequent keyboard repeat delay in OpenCore implementation
  of Apple Event protocol, in units of 10ms.
}

  \DIFadd{See documentation of }\texttt{\DIFadd{KeyInitialDelay}} \DIFadd{for fine tuning. Even if not fine tuning,
  this value should ideally not be set to less than the value of }\texttt{\DIFadd{KeyForgetThreshold}} \DIFadd{in
  ...
  \emph{\DIFadd{Note}}\DIFadd{: No changes to this value are ever required on systems which are not using }\texttt{\DIFadd{KeySupport}}\DIFadd{.
}

\item
  \DIFaddend \texttt{KeySwap}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  ...
  This option allows \DIFdelbegin \DIFdel{to update }\DIFdelend \DIFaddbegin \DIFadd{updating the }\DIFaddend firmware architecture timer period with the specified value
  in \texttt{100} nanosecond units. Setting a lower value \DIFdelbegin \DIFdel{generally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend improves performance
...
\item
  ...
  UEFI firmware \DIFdelbegin \DIFdel{generally }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend supports \texttt{ConsoleControl} with two
  ...
  The use of \texttt{BuiltinGraphics} is \DIFdelbegin \DIFdel{generally }\DIFdelend straightforward.
  For most platforms\DIFaddbegin \DIFadd{, }\DIFaddend it is necessary to enable \texttt{ProvideConsoleGop}
  \DIFdelbegin \DIFdel{,
  }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend set \texttt{Resolution} to \texttt{Max}. \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{BuiltinText} variant is
...
\item
  ...
  section for \DIFdelbegin \DIFdel{more }\DIFdelend details.
...
\item
  ...
  Apple audio protocols allow \DIFdelbegin \DIFdel{macOS bootloader and OpenCore }\DIFdelend \DIFaddbegin \DIFadd{OpenCore and the macOS bootloader }\DIFaddend to play
  sounds and signals for screen reading or audible error reporting.
  Supported protocols are beep generation and VoiceOver. \DIFaddbegin \DIFadd{The }\DIFaddend VoiceOver protocol
  is specific to Gibraltar machines (T2) and is not supported before
  macOS High Sierra (10.13). \DIFdelbegin \DIFdel{Instead older }\DIFdelend \DIFaddbegin \DIFadd{Older }\DIFaddend macOS versions use \DIFdelbegin \DIFdel{AppleHDA protocol,
  which is currently not implemented}\DIFdelend \DIFaddbegin \DIFadd{the AppleHDA protocol
  (which is not currently implemented) instead}\DIFaddend .

  Only one set of audio protocols can be available at a time, so \DIFaddbegin \DIFadd{this setting should
  be enabled }\DIFaddend in order to \DIFdelbegin \DIFdel{get }\DIFdelend \DIFaddbegin \DIFadd{enable }\DIFaddend audio playback in \DIFaddbegin \DIFadd{the }\DIFaddend OpenCore user interface on Mac
  \DIFdelbegin \DIFdel{system }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend implementing some of these protocols\DIFdelbegin \DIFdel{this setting should be enabled}\DIFdelend .

  \emph{Note}: \DIFdelbegin \DIFdel{Backend }\DIFdelend \DIFaddbegin \DIFadd{The backend }\DIFaddend audio driver needs to be configured in \texttt{UEFI Audio}
...
\item
  ...
  version. This may be used to ensure FileVault 2 compatibility on VMs and legacy Macs.
...
\item
  ...
  This protocol replaces \DIFaddbegin \DIFadd{the }\DIFaddend legacy \texttt{VirtualSmc} UEFI driver, and is compatible
  with any SMC kernel extension. However, in case \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{FakeSMC} kernel extension
...
\item
  ...
  installed, so all properties required for \DIFaddbegin \DIFadd{the }\DIFaddend safe operation of the system must
  be specified in \DIFdelbegin \DIFdel{your configuration }\DIFdelend \DIFaddbegin \DIFadd{the configuration file}\DIFaddend .
...
\item
  ...
  specified in \DIFdelbegin \DIFdel{your configuration }\DIFdelend \DIFaddbegin \DIFadd{the configuration file}\DIFaddend .

\item
  \texttt{FirmwareVolume}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Wraps Firmware Volume protocols\DIFaddbegin \DIFadd{, }\DIFaddend or installs a new version\DIFaddbegin \DIFadd{,
  }\DIFaddend to support custom cursor images for FileVault 2. Set to \texttt{true} to ensure
  FileVault 2 compatibility on anything other than on VMs and legacy Macs.

  \emph{Note}: Several virtual machines\DIFdelbegin \DIFdel{including VMware }\DIFdelend \DIFaddbegin \DIFadd{, including VMware, }\DIFaddend may have corrupted
...
\item
  ...
  The recommended solution in the other cases is to install a kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend such as
  ...
  \emph{Note}: This quirk cannot replace the kernel \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend because it cannot operate in
...
\item
  ...
  \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\DIFaddend \texttt{Sample.plist} \DIFdelbegin \DIFdel{for more }\DIFdelend \DIFaddbegin \DIFadd{file for }\DIFaddend details.
...
Older operating systems may be more complicated to install, but \DIFdelbegin \DIFdel{sometimes can
be necessary to use for all kinds of }\DIFdelend \DIFaddbegin \DIFadd{are sometimes
necessary for various }\DIFaddend reasons. While a compatible board identifier
...
This section \DIFdelbegin \DIFdel{tries to cover }\DIFdelend \DIFaddbegin \DIFadd{covers }\DIFaddend a common set of issues relevant to installing
...
\begin{itemize}
  \item Disk images on these systems use \DIFaddbegin \DIFadd{the }\DIFaddend Apple Partitioning Scheme
    and require \DIFaddbegin \DIFadd{the }\DIFaddend \texttt{OpenPartitionDxe} driver to run DMG recovery
...
\begin{itemize}
  ...
  \item Last released installer images for macOS~10.6 are macOS~10.6.7
    ...
    assuming macOS~10.6 is legally owned. \DIFdelbegin \DIFdel{Read }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\DIFaddend \texttt{DIGEST.txt} \DIFdelbegin \DIFdel{for more }\DIFdelend \DIFaddbegin \DIFadd{file
    for }\DIFaddend details. Note that these are the earliest tested
...
\begin{itemize}
  ...
  \item Last released installer image for macOS~10.5 is macOS~10.5.7
    ...
    assuming macOS~10.5 is legally owned. \DIFdelbegin \DIFdel{Read }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\DIFaddend \texttt{DIGEST.txt} \DIFdelbegin \DIFdel{for more }\DIFdelend \DIFaddbegin \DIFadd{file
    for }\DIFaddend details. Note that this is the earliest tested
...
\begin{itemize}
  ...
  \item Last released installer images for macOS~10.4 are macOS~10.4.10
    ...
    assuming macOS~10.4 is legally owned. \DIFdelbegin \DIFdel{Read }\DIFdelend \DIFaddbegin \DIFadd{Refer to the }\DIFaddend \texttt{DIGEST.txt} \DIFdelbegin \DIFdel{for more }\DIFdelend \DIFaddbegin \DIFadd{file
    for }\DIFaddend details. Note that these are the earliest tested
...
\subsection{Windows support}\label{troubleshootingwin}
  ...
  \textbf{Why do I see \texttt{Basic data partition} in \DIFaddbegin \DIFadd{the }\DIFaddend Boot Camp Startup Disk control panel?}

  \DIFaddbegin \DIFadd{The }\DIFaddend Boot Camp control panel uses \DIFaddbegin \DIFadd{the }\DIFaddend GPT partition table to obtain each boot option name.
  After installing Windows separately\DIFdelbegin \DIFdel{the partition will have }\DIFdelend \DIFaddbegin \DIFadd{, the partition has }\DIFaddend to be relabelled manually.
  This can be done with many utilities including \DIFaddbegin \DIFadd{the }\DIFaddend open-source
...
\end{lstlisting}
  ...
  \textbf{How \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{do I }\DIFaddend choose Windows BOOTCAMP with custom NTFS drivers?}
  ...
  disrupt certain macOS functionality, including \DIFaddbegin \DIFadd{the
  }\DIFaddend \href{https://support.apple.com/HT202796}{Startup Disk} preference
...
\begin{enumerate}
\item
  \textbf{How \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{do I }\DIFaddend debug boot \DIFdelbegin \DIFdel{failure}\DIFdelend \DIFaddbegin \DIFadd{failures}\DIFaddend ?}

  \DIFdelbegin \DIFdel{Normally it is enough to obtain }\DIFdelend \DIFaddbegin \DIFadd{Obtaining }\DIFaddend the actual error message \DIFaddbegin \DIFadd{is usually adequate}\DIFaddend . For this\DIFaddbegin \DIFadd{, }\DIFaddend ensure that:
  \begin{itemize}
  \tightlist
...
\item
  \textbf{How \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{do I }\DIFaddend debug macOS boot \DIFdelbegin \DIFdel{failure}\DIFdelend \DIFaddbegin \DIFadd{failures}\DIFaddend ?}
...
\item
  \textbf{How \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{do I }\DIFaddend customise boot entries?}
...
\item
  \textbf{How \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{do I }\DIFaddend choose the default boot entry?}
...
\item
  ...
  descriptions in this document for \DIFdelbegin \DIFdel{more }\DIFdelend details.
