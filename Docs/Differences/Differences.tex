...
%DIF DEL PreviousConfiguration.tex   Tue Mar  1 19:05:37 2022
%DIF ADD ../Configuration.tex        Tue Mar  8 17:48:18 2022
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (\DIFdelbegin \DIFdel{0.7.8}\DIFdelend \DIFaddbegin \DIFadd{0.8.0}\DIFaddend )
...
\item
  ...
  \DIFaddbegin \DIFadd{On macOS 12+, it is no longer possible to set trim timeout for APFS filesystems.
...
\end{lstlisting}
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{AAPL} --- \DIFdelbegin \DIFdel{OcDebugLogLib}\DIFdelend \DIFaddbegin \DIFadd{OcLogAggregatorLib}\DIFaddend , Apple EfiBoot logging
  ...
  \item \texttt{OCL} --- \DIFdelbegin \DIFdel{OcDebugLogLib
  }\DIFdelend \DIFaddbegin \DIFadd{OcLogAggregatorLib
  }\DIFaddend \item \texttt{OCM} --- OcMiscLib
...
\item
  ...
  reinstalled or personalised. \DIFdelbegin \DIFdel{Unless }\DIFdelend \DIFaddbegin \DIFadd{Until }\DIFaddend the operating system is personalised,
  \DIFaddbegin \DIFadd{only }\DIFaddend macOS DMG recovery \DIFdelbegin \DIFdel{cannot }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend be loaded. In cases where DMG recovery is missing,
...
\DIFdel{are set . }\DIFdelend \DIFaddbegin \item \texttt{\DIFadd{LINUX\_BOOT\_ADD\_DEBUG\_INFO}}\DIFadd{.
  ...
  Allows \DIFaddbegin \DIFadd{manually }\DIFaddend specifying kernel options \DIFaddbegin \DIFadd{to use in autodetect mode }\DIFaddend for a given partition only.
  \DIFaddbegin \DIFadd{Replace the text }\texttt{\DIFadd{\{PARTUUID\}}} \DIFadd{with the specific partition UUID on which the kernels are stored
  (in normal layout, the partition which contains }\texttt{\DIFadd{/boot}}\DIFadd{),
  e.g. }\texttt{\DIFadd{autoopts:11223344-5566-7788-99aa-bbccddeeff00+="vt.handoff=7"}}\DIFadd{.
  }\DIFaddend If specified with \texttt{+=} then these \DIFaddbegin \DIFadd{options }\DIFaddend are used in addition to \DIFaddbegin \DIFadd{any }\DIFaddend autodetected options, if specified
  with \texttt{=} they are used instead. Used for autodetected Linux only \DIFdelbegin \DIFdel{. Values }\DIFdelend \DIFaddbegin \DIFadd{-- values }\DIFaddend specified here are never
  used for entries created from \texttt{/loader/entries/*.conf} files.
  \medskip

  \emph{Note}: The \texttt{\DIFdelbegin \DIFdel{partuuid}\DIFdelend \DIFaddbegin \DIFadd{PARTUUID}\DIFaddend } value to be specified here is typically the same as the \texttt{PARTUUID}
  seen in \texttt{root=PARTUUID=...} in the Linux kernel boot options (view using
  \texttt{cat /proc/cmdline})\DIFdelbegin \DIFdel{for autodetected Debian-style distros, but is not the same for
  Fedora-style distros booted from }\texttt{\DIFdel{/loader/entries/*.conf}} %DIFAUXCMD
\DIFdel{files. }%DIFDELCMD < \medskip
%DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdel{Typically this option should not be needed in the latter case, but in case it is, to find out the unique
  partition uuid to use look for }\texttt{\DIFdel{LNX:}} %DIFAUXCMD
\DIFdel{entries in the OpenCore debug log file. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend Alternatively, and
  ...
  output of \texttt{ls -l /dev/disk/by-partuuid}. \medskip
	...
	\item \texttt{autoopts[+]="\{options\}"} - Default: None specified. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \medskip

  \DIFadd{Allows manually specifying }\DIFaddend kernel options to use \DIFdelbegin \DIFdel{for autodetected Linux only. The value here is never used for entries created from
  }\texttt{\DIFdel{/loader/entries/*.conf}} %DIFAUXCMD
\DIFdel{files. }\texttt{\DIFdel{partuuidopts}} %DIFAUXCMD
\DIFdel{may be }\DIFdelend \DIFaddbegin \DIFadd{in autodetect mode. The alternative format }\texttt{\DIFadd{autoopts:\{PARTUUID\}}}
  \DIFadd{is }\DIFaddend more suitable where there are multiple distros, but \texttt{autoopts} with no PARTUUID required \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{may be }\DIFaddend more
  ...
  with \texttt{=} they are used instead. \DIFaddbegin \DIFadd{Used for autodetected Linux only -- values specified here are never
  used for entries created from }\texttt{\DIFadd{/loader/entries/*.conf}} \DIFadd{files.
  }\medskip

  \DIFaddend As example usage, it is possible to use \texttt{+=} format to add
...
\DIFaddbegin 
	...
	\item \texttt{\DIFadd{-}{}\DIFadd{-restore-nosnoop}} \DIFadd{- Boolean flag, enabled if present. }\medskip
...
%DIFAUXCMD
\DIFdel{for known issues. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Properties}\label{uefiprops}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Enable AVX vector acceleration of SHA-512 and SHA-384 hashing algorithms\DIFaddbegin \DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: This option may cause issues on certain laptop firmwares, including Lenovo}\DIFaddend .
