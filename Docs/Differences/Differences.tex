...
%DIF ADD ../Configuration.tex        Wed Mar  3 14:39:18 2021
...
Here is an example of a defect found in some
...
\item
  ...
  defects resulting in a failure to properly execute the server detection in the first place.
...
\item
  ...
  \emph{Note 1}: Some types of firmware may have defective NVRAM implementation, no boot option
  support, or other incompatibilities. While unlikely, the use of this option may cause boot
  failures and should only be used exclusively on the boards known to be compatible. Refer to
...
\item
  ...
  section and are normally handled by Apple BDS, modifier keys handled by the operating system
...
\item
  ...
  protocol using the \texttt{ProtocolOverrides} section. Refer to the
  \texttt{DataHub} protocol override description for details.
\item
  \texttt{UpdateNVRAM}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  ...
  format is used to store the \texttt{SystemUUID} data. Otherwise, \texttt{Little Endian} is used.
...
\item
  ...
  It is dangerous to use Apple in SMBIOS vendor fields for reasons given in the
...
\item
  ...
  based on Mac product name\DIFaddbegin \DIFadd{.
}

  \DIFadd{When }\texttt{\DIFadd{Automatic}} \DIFadd{is }\texttt{\DIFadd{true}}\DIFadd{, the original value from the the corresponding Mac model will be set if available.
  ...
  If no value is provided, the fallback value (}\texttt{\DIFadd{zero}}\DIFadd{) will be set}\DIFaddend .
...
\item
  ...
  during the OpenCore picker. This permits APFS USB hot plug. Disable
  if not required.
...
\end{lstlisting}
  ...
  disrupt certain macOS functionality, including
