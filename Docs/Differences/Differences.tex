...
%DIF ADD ../Configuration.tex        Tue Mar 22 16:40:30 2022
...
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, while }\texttt{\DIFadd{0}} \DIFadd{entirely disables it. }\DIFaddend Refer to this
  ...
  On macOS 12+, it is no longer possible to \DIFdelbegin \DIFdel{set }\DIFdelend \DIFaddbegin \DIFadd{specify }\DIFaddend trim timeout for APFS filesystems.
  However, \DIFdelbegin \DIFdel{trim }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend can be disabled \DIFdelbegin \DIFdel{when the timeout value is }\DIFdelend \DIFaddbegin \DIFadd{by setting }\texttt{\DIFadd{0}}\DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: System Information may still report }\texttt{\DIFadd{TRIM Support: Yes}} \DIFadd{with this
  option }\DIFaddend set to \texttt{0}. \DIFaddbegin \DIFadd{Yet, this is false positive and trim is de facto disabled.
}\DIFaddend 
