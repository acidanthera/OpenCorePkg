...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.5\DIFdelbegin \DIFdel{.3}\DIFdelend \DIFaddbegin \DIFadd{.4}\DIFaddend )
...
\item
  ...
  \emph{Note}: This \DIFdelbegin \DIFdel{option is deprecated and will be removed in the newer OpenCore
  releases. There is no known hardwarethat needs this option,
  and its entire existence
  appears to be a programmer error. Please report on
  }%DIFDELCMD < \href{https://github.com/acidanthera/bugtracker}{Acidanthera Bugtracker} %%%
\DIFdel{if you need this option}\DIFdelend \DIFaddbegin \DIFadd{may be used to workaround buggy memory maps on older hardware,
...
\DIFaddbegin \item
  \texttt{\DIFadd{SignalAppleOS}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Report macOS being loaded through OS Info for any OS.
}

  \DIFadd{This quirk is useful on Mac firmwares, which behave differently in different OS.
...
\item
  ...
  \texttt{EDX} values \DIFdelbegin \DIFdel{in Little Endian order }\DIFdelend to replace \texttt{CPUID (1)} call in XNU kernel.
\DIFaddbegin 

  \DIFadd{This property serves for two needs:
}

  \begin{itemize}
    \tightlist
    \item \DIFadd{Enabling support of an unsupported CPU model.
    }\item \DIFadd{Enabling XCPM support for an unsupported CPU variant.
  }\end{itemize}

  \DIFaddend Normally it is only the value of \texttt{EAX} that needs to be taken care of,
  \DIFdelbegin \DIFdel{which represents the exact CPUID. And the remainders }\DIFdelend \DIFaddbegin \DIFadd{since it represents the full CPUID. The remaining bytes }\DIFaddend are to be left as zeroes.
  \DIFdelbegin \DIFdel{For instance, }\DIFdelend \DIFaddbegin \DIFadd{Byte order is Little Endian, so for example, }\DIFaddend \texttt{A9 06 03 00} stands for CPUID
  \texttt{0x0306A9} (Ivy Bridge).
\DIFdelbegin \DIFdel{A good example }\DIFdelend \DIFaddbegin 

  \DIFadd{For XCPM support it is recommended to use the following combinations.
}

  \begin{itemize}
    \tightlist
    \item \DIFadd{Haswell-E (}\texttt{\DIFadd{0x306F2}}\DIFadd{) to Haswell (}\texttt{\DIFadd{0x0306C3}}\DIFadd{):}\\
    \texttt{\DIFadd{Cpuid1Data}}\DIFadd{: }\texttt{\DIFadd{C3 06 03 00 00 00 00 00 00 00 00 00 00 00 00 00}}\\
    \texttt{\DIFadd{Cpuid1Mask}}\DIFadd{: }\texttt{\DIFadd{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}}
    \item \DIFadd{Broadwell-E (}\texttt{\DIFadd{0x0406F1}}\DIFadd{) to Broadwell (}\texttt{\DIFadd{0x0306D4}}\DIFadd{):}\\
    \texttt{\DIFadd{Cpuid1Data}}\DIFadd{: }\texttt{\DIFadd{D4 06 03 00 00 00 00 00 00 00 00 00 00 00 00 00}}\\
    \texttt{\DIFadd{Cpuid1Mask}}\DIFadd{: }\texttt{\DIFadd{FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00}}
  \end{itemize}

  \DIFadd{Further explanations }\DIFaddend can be found at
  \href{https://github.com/acidanthera/bugtracker/issues/365}{acidanthera/bugtracker\#365}.
  \DIFdelbegin \DIFdel{(See }\DIFdelend \DIFaddbegin \DIFadd{See }\DIFaddend \texttt{Special NOTES\DIFdelbegin \DIFdel{for Haswell}\DIFdelend \DIFaddbegin } \DIFadd{for Haswell}\DIFaddend + low-end\DIFdelbegin %DIFDELCMD < \MBLOCKRIGHTBRACE%%%
\DIFdel{)  
...
\DIFaddbegin 
  ...
  \DIFaddend When each \texttt{Cpuid1Mask} bit is set to 0, the original CPU bit is used,
...
\DIFdel{: This option should }\DIFdelend \DIFaddbegin \DIFadd{Certain firmwares lock }\texttt{\DIFadd{PKG\_CST\_CONFIG\_CONTROL}} \DIFadd{MSR register. To check its
  state one can use bundled }\texttt{\DIFadd{VerifyMsrE2}} \DIFadd{tool. Select firmwares have this
  register locked on some cores only.
}

  \DIFadd{As modern firmwares provide }\texttt{\DIFadd{CFG Lock}} \DIFadd{setting, which allows configuring
  }\texttt{\DIFadd{PKG\_CST\_CONFIG\_CONTROL}} \DIFadd{MSR register lock, this option should be }\DIFaddend avoided
  whenever possible. \DIFdelbegin \DIFdel{Modern firmwares provide
  }\DIFdelend \DIFaddbegin \DIFadd{For several APTIO firmwares not displaying }\DIFaddend \texttt{CFG Lock} setting
  \DIFdelbegin \DIFdel{, disabling which is much cleaner.
    More details
  about the issue can be foundin
  }%DIFDELCMD < \href{https://github.com/acidanthera/AppleSupportPkg#verifymsre2}{VerifyMsrE2} %%%
\DIFdel{notes.}\DIFdelend \DIFaddbegin \DIFadd{in the GUI it is possible to access the option directly:
}\DIFaddend 

  \DIFaddbegin \begin{enumerate}
    \tightlist
    \DIFaddend \item \DIFaddbegin \DIFadd{Download }\href{https://github.com/LongSoft/UEFITool/releases}{UEFITool} \DIFadd{and
      }\href{https://github.com/LongSoft/Universal-IFR-Extractor/releases}{IFR-Extractor}\DIFadd{.
    }\item \DIFadd{Open your firmware image in UEFITool and find }\texttt{\DIFadd{CFG Lock}} \DIFadd{unicode string.
      If it is not present, your firmware may not have this option and you should stop.
    }\item \DIFadd{Extract the }\texttt{\DIFadd{Setup.bin}} \DIFadd{PE32 Image Section (the one UEFITool found) through
      }\texttt{\DIFadd{Extract Body}} \DIFadd{menu option.
    }\item \DIFadd{Run IFR-Extractor on the extracted file (e.g. }\texttt{\DIFadd{./ifrextract Setup.bin Setup.txt}}\DIFadd{).
    }\item \DIFadd{Find }\texttt{\DIFadd{CFG Lock, VarStoreInfo (VarOffset/VarName):}} \DIFadd{in }\texttt{\DIFadd{Setup.txt}} \DIFadd{and
      remember the offset right after it (e.g. }\texttt{\DIFadd{0x123}}\DIFadd{).
    }\item \DIFadd{Download and run }\href{http://brains.by/posts/bootx64.7z}{Modified GRUB Shell} \DIFadd{compiled by
      }\href{https://geektimes.com/post/258090}{brainsucker} \DIFadd{or use
      }\href{https://github.com/datasone/grub-mod-setup_var}{a newer version} \DIFadd{by
      }\href{https://github.com/datasone}{datasone}\DIFadd{.
    }\item \DIFadd{Enter }\texttt{\DIFadd{setup\_var 0x123 0x00}} \DIFadd{command, where }\texttt{\DIFadd{0x123}} \DIFadd{should be replaced by
      your actual offset, and reboot.
  }\end{enumerate}

  \textbf{\DIFadd{WARNING}}\DIFadd{: Variable offsets are unique not only to each motherboard but even to its firmware
  version. Never ever try to use an offset without checking.
}

\item
  \DIFaddend \texttt{AppleXcpmCfgLock}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Disables \texttt{PKG\_CST\_CONFIG\_CONTROL} (\texttt{0xE2})
  ...
  \emph{Note}: This option should \DIFaddbegin \DIFadd{be }\DIFaddend avoided whenever possible. \DIFdelbegin \DIFdel{Modern firmwares provide
  }\DIFdelend \DIFaddbegin \DIFadd{See }\DIFaddend \texttt{\DIFdelbegin \DIFdel{CFG Lock}\DIFdelend \DIFaddbegin \DIFadd{AppleCpuPmCfgLock}\DIFaddend }
  \DIFdelbegin \DIFdel{setting, disabling which is much cleaner. More detailsabout the issue can be found in
  }%DIFDELCMD < \href{https://github.com/acidanthera/AppleSupportPkg#verifymsre2}{VerifyMsrE2} %%%
\DIFdel{notes}\DIFdelend \DIFaddbegin \DIFadd{description for more details}\DIFaddend .
...
\item
  \DIFaddbegin \texttt{\DIFadd{AppleXcpmForceBoost}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Forces maximum performance in XCPM mode.
}

  \DIFadd{This patch writes }\texttt{\DIFadd{0xFF00}} \DIFadd{to }\texttt{\DIFadd{MSR\_IA32\_PERF\_CONTROL}} \DIFadd{(}\texttt{\DIFadd{0x199}}\DIFadd{),
  effectively setting maximum multiplier for all the time.
}

  \emph{\DIFadd{Note}}\DIFadd{: While this may increase the performance, this patch is strongly discouraged
...
\item
  ...
  \emph{Note}: activated \texttt{KeySupport}, \texttt{\DIFdelbegin \DIFdel{UsbKbDxe}\DIFdelend \DIFaddbegin \DIFadd{AppleUsbKbDxe}\DIFaddend }, or similar driver is required
...
\item
  \DIFaddbegin \texttt{\DIFadd{AllowSetDefault}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Allow }\texttt{\DIFadd{CTRL+Enter}} \DIFadd{and }\texttt{\DIFadd{CTRL+Index}} \DIFadd{handling
  to set the default boot option in boot picker.
}

\item
  \texttt{\DIFadd{AuthRestart}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
...
\DIFaddbegin \DIFadd{For proper macOS functioning it is often required to use }\texttt{\DIFadd{OC\_FIRMWARE\_RUNTIME}}
\DIFadd{protocol implementation currently offered as a part of }\texttt{\DIFadd{FwRuntimeServices}} \DIFadd{driver.
While it brings any benefits, there are certain limitations which arise depending on the
use.
}

\begin{enumerate}
\item \DIFadd{Not all tools may be aware of protected namespaces.}\\
  \DIFadd{When }\texttt{\DIFadd{RequestBootVarRouting}} \DIFadd{is used }\texttt{\DIFadd{Boot}}\DIFadd{-prefixed variable access
  is restricted and protected in a separate namespace. To access the original variables
  tools have to be aware of }\texttt{\DIFadd{OC\_FIRMWARE\_RUNTIME}} \DIFadd{logic.
}\item \DIFadd{Assigned NVRAM variables are not always allowed to exceed 512 bytes.}\\
  \DIFadd{This is true for }\texttt{\DIFadd{Boot}}\DIFadd{-prefixed variables when }\texttt{\DIFadd{RequestBootVarFallback}}
  \DIFadd{is used, and for overwriting volatile variables with non-volatile on UEFI 2.8
  non-conformant firmwares.
}\end{enumerate}

\DIFaddend \subsection{Properties}\label{nvramprops}
...
\item
  ...
  Variable loading happens prior to \texttt{Block} (and \texttt{Add}) phases\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. Unless
  }\texttt{\DIFadd{LegacyOverwrite}} \DIFadd{is enabled, it }\DIFaddend will not overwrite any existing variable.
...
\item
  \DIFaddbegin \texttt{\DIFadd{LegacyOverwrite}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Permits overwriting firmware variables from }\texttt{\DIFadd{nvram.plist}}\DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: Only variables accessible from the operating system will be overwritten.
}

\item
  \DIFaddend \texttt{LegacySchema}\\
  \textbf{Type}: \texttt{plist\ dict}\\
...
\DIFaddbegin \item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enables writing to flash memory for all added variables.
  }
...
\item
  \DIFaddbegin \texttt{\DIFadd{SupportsCsm}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Forces CSM support in }\texttt{\DIFadd{FirmwareFeatures}}\DIFadd{.
}

  \DIFadd{Without this bit it is not possible to reboot to Windows installed on
  a drive with EFI partition being not the first partition on the disk.
  }

  \emph{\DIFadd{Note}}\DIFadd{: This was enabled unconditionally in older OpenCore versions.
}\item
...
\item
  ...
  While effective, this option \DIFdelbegin \DIFdel{is not necessary with e.g. APFS loader
  driver}\DIFdelend \DIFaddbegin \DIFadd{may not be necessary for drivers performing
  automatic connection}\DIFaddend , and may slightly slowdown the boot.
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OcSupportPkg}{\texttt{FwRuntimeServices}}
  ...
  \item \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/AppleSupportPkg}{\texttt{UsbKbDxe}}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OcSupportPkg}{\texttt{AppleUsbKbDxe}}
  ...
  \item \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/VirtualSMC}{\texttt{VirtualSmc}}
%DIFDELCMD <   %%%
\DIFdel{--- UEFI SMC driver, required for proper FileVault 2 functionality and potentially
  other macOS specifics. An alternative, named }\texttt{\DIFdel{SMCHelper}}%DIFAUXCMD
\DIFdel{, is not compatible
  with }\texttt{\DIFdel{VirtualSmc}} %DIFAUXCMD
\DIFdel{and OpenCore, which is unaware of its specific interfaces.
...
\item
  ...
  a separate driver is used, such as \texttt{\DIFdelbegin \DIFdel{UsbKbDxe}\DIFdelend \DIFaddbegin \DIFadd{AppleUsbKbDxe}\DIFaddend }, this option
...
\item
  \DIFaddbegin \texttt{\DIFadd{AppleSmcIo}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
...
\item
  ...
  prior to APTIO V (Haswell and older) are affected\DIFaddbegin \DIFadd{.
}
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Forcibly reinstalls OS Info protocol with builtin
  versions. This protocol is generally used to receive notifications from macOS
  bootloader, by the firmware or by other applications}\DIFaddend .
