...
%DIF ADD ../Configuration.tex        Sat Oct  3 09:51:46 2020
...
In case \DIFdelbegin \DIFdel{you use }\DIFdelend \DIFaddbegin \DIFadd{one uses }\DIFaddend these sources, for example, \href{https://dortania.github.io}{Dortania}'s
...
Be warned that regardless of the sources used \DIFdelbegin \DIFdel{you are }\DIFdelend \DIFaddbegin \DIFadd{one is }\DIFaddend required to fully understand every
...
for legacy systems. To run OpenCore on such a legacy system \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend can install
...
\item \texttt{Above 4G Decoding} or similar enabled in firmware
  ...
  with the same issue are known, consider this option to be first to check if \DIFdelbegin \DIFdel{you
  have }\DIFdelend \DIFaddbegin \DIFadd{one
  encounters }\DIFaddend erratic boot failures.
...
\href{https://github.com/LongSoft/UEFITool/blob/master/UEFIPatch/patches.txt}{patching it}
  if \DIFdelbegin \DIFdel{you have enough skills and }\DIFdelend no option is available \DIFaddbegin \DIFadd{(for sophisticated users only)}\DIFaddend . See
...
\item
  ...
  of the firmware. Do not use this unless \DIFdelbegin \DIFdel{you fully understand the consequences }\DIFdelend \DIFaddbegin \DIFadd{a complete understanding of the consequences can be ensured}\DIFaddend .
...
\item
  ...
  \begin{enumerate}
    ...
    \item Open \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware image in UEFITool and find \texttt{CFG Lock} unicode string.
      If it is not present, \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend firmware may not have this option and \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend should stop.
...
\item
  ...
  boot performance. This setting allows to prevent using faster kernel caching variants
  if slower variants are available for debugging and stability reasons. I.e. by
  specifying \texttt{Mkext} one will disable \texttt{Prelinked} for e.g. 10.6 but not 10.7.
...
\item
  ...
  To start using personalised Apple Secure Boot \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend will have to reinstall the
  operating system or personalise it. Until \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend operating system is personalised
  \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend will only be able to load macOS DMG recovery. If \DIFdelbegin \DIFdel{you do not have DMG recovery you could always download it }\DIFdelend \DIFaddbegin \DIFadd{DMG recovery is missing,
...
\end{lstlisting}
  ...
  \emph{Note 1}: While it may appear obvious, but \DIFdelbegin \DIFdel{you have to use }\DIFdelend an external
  method \DIFaddbegin \DIFadd{is required }\DIFaddend to verify \texttt{OpenCore.efi} and \texttt{BOOTx64.efi} for
  secure boot path. For this \DIFdelbegin \DIFdel{you are }\DIFdelend \DIFaddbegin \DIFadd{one is }\DIFaddend recommended to at least enable UEFI SecureBoot
  with a custom certificate, and sign \texttt{OpenCore.efi} and \texttt{BOOTx64.efi}
  with \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{user's }\DIFaddend custom key. More details on customising secure boot on modern firmwares
...
\item \label{secureapplesb}
  ...
  of Apple Secure Boot. To achieve \texttt{Full Security} one will need to
  also specify \texttt{ApECID} value.

  Enabling Apple Secure Boot is more demanding to incorrect configurations,
  buggy macOS installations, and unsupported setups. Things to keep in mind:

  \begin{enumerate}
    \tightlist
    \item Just like on T2 Macs \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend will not be able to install any unsigned kernel
      drivers and several signed kernel drivers including NVIDIA Web Drivers.
  ...
  it is likely the case. When this happens \DIFdelbegin \DIFdel{you }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend can either reinstall the operating
...
\item
  ...
  Simply said, \DIFdelbegin \DIFdel{you are }\DIFdelend \DIFaddbegin \DIFadd{one is }\DIFaddend required to enable this quirk to be able to reliably
...
\begin{itemize}
  ...
  \item Last released installer images for macOS~10.6 are macOS~10.6.7
    ...
    assuming that \DIFdelbegin \DIFdel{you legally own }\DIFdelend \DIFaddbegin \DIFadd{one legally owns }\DIFaddend macOS~10.6. Read \texttt{DIGEST.txt}
...
\begin{enumerate}
\item Enable Apple Secure Boot by setting \texttt{SecureBootModel} if \DIFdelbegin \DIFdel{you need }\DIFdelend \DIFaddbegin \DIFadd{one needs }\DIFaddend to
...
\item Enable UEFI Secure Boot in \DIFdelbegin \DIFdel{your }\DIFdelend firmware preferences and install the
  certificate with a private key \DIFdelbegin \DIFdel{you own}\DIFdelend \DIFaddbegin \DIFadd{one owns}\DIFaddend . Details on how to generate a certificate
  ...
  If \DIFdelbegin \DIFdel{you need }\DIFdelend \DIFaddbegin \DIFadd{one needs }\DIFaddend to launch option ROMs or decided to use signed Linux drivers
  \DIFdelbegin \DIFdel{you will also need the
  }\href{http://go.microsoft.com/fwlink/?LinkId=321194}{\DIFdel{Microsoft UEFI Driver Signing CA}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \href{http://go.microsoft.com/fwlink/?LinkId=321194}{\DIFadd{Microsoft UEFI Driver Signing CA}} \DIFadd{will also be needed}\DIFaddend .
...
}\DIFdelend \DIFaddbegin \DIFadd{one has
}\DIFaddend to connect motherboard ``\texttt{TX}'' to USB UART \texttt{GND}, and motherboard ``\texttt{GND}''
