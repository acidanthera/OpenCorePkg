...
%DIF ADD ../Configuration.tex        Sat Oct  3 13:08:09 2020
...
\item \texttt{CSM} (Compatibility Support Module) disabled in firmware settings
  if present. \DIFdelbegin \DIFdel{You may need to flash GOP ROM on }\DIFdelend \DIFaddbegin \DIFadd{On }\DIFaddend NVIDIA 6xx/AMD 2xx or older\DIFaddbegin \DIFadd{, GOP ROM might be flashed first}\DIFaddend . Use
...
When debugging sleep issues \DIFdelbegin \DIFdel{you may want to (temporarily) disable }\DIFdelend Power Nap and automatic power off \DIFaddbegin \DIFadd{may be disabled}\DIFaddend ,
...
\item
  ...
  \begin{itemize}
    ...
    \item Consumer Ivy Bridge (\texttt{0x0306A9}) as Apple disabled XCPM for Ivy Bridge
      ...
      \DIFdelbegin \DIFdel{to force }\DIFdelend \DIFaddbegin \DIFadd{should be patched to enforce }\DIFaddend XCPM on these CPUs instead of \DIFdelbegin \DIFdel{using }\DIFdelend this option.
...
overwrite OpenCore, make sure to enable it if \DIFdelbegin \DIFdel{you plan }\DIFdelend \DIFaddbegin \DIFadd{one plans }\DIFaddend to use them.
...
\item
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{None} --- Avoid hibernation\DIFdelbegin \DIFdel{for your own good}\DIFdelend .
...
\item
  ...
  Use at \DIFdelbegin \DIFdel{your }\DIFdelend \DIFaddbegin \DIFadd{user's }\DIFaddend own risk on boards known to be compatible.
...
\item Make sure that \texttt{ScanPolicy} restricts loading from undesired
  ...
  filesystems.
\item Sign all the installed drivers and tools with \DIFdelbegin \DIFdel{your }\DIFdelend private key. Do not sign
