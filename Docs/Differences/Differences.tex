...
%DIF DEL PreviousConfiguration.tex   Fri Mar 17 21:49:13 2023
%DIF ADD ../Configuration.tex        Tue Mar 28 00:23:38 2023
...
\item
  ...
  also be used, i.e. it may need to be overridden on Mac EFI\DIFaddbegin \DIFadd{.
}

\item
  \texttt{\DIFadd{GopBurstMode}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enable write-combining (WC) caching for GOP memory, if system firmware has not
  already enabled it.
}

  \DIFadd{Some older firmware (e.g. EFI-era Macs) fails to set write-combining caching (aka burst mode)
  for GOP memory, even though the CPU supports it. Setting this can give a considerable
  speed-up for GOP operations, especially on systems which require }\texttt{\DIFadd{DirectGopRendering}}\DIFadd{.
}

  \emph{\DIFadd{Note}}\DIFadd{: This takes effect whether or not }\texttt{\DIFadd{DirectGopRendering}} \DIFadd{is set, and may
  give some speed-up to GOP operations even when }\texttt{\DIFadd{DirectGopRendering}} \DIFadd{is }\texttt{\DIFadd{false}}\DIFaddend .
