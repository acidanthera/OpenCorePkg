...
%DIF ADD ../Configuration.tex        Mon Jun  1 03:16:00 2020
...
\lstdefinestyle{ocbash}{
  ...
  keepspaces=true,
%DIF 47c47
%DIF <   morekeywords={git, make, build, ioreg, grep, nvram, sort, sudo, diskutil, gfxutil, strings, dd, cut},
%DIF -------
  morekeywords={git, make, build, ioreg, grep, nvram, sort, sudo, diskutil, gfxutil, strings, dd, cut, python}, %DIF > 
%DIF -------
  literate =
    {"}{{\textquotedbl}}1
    {'}{{\textquotesingle}}1
...
\DIFdelbegin \DIFdel{All other sources or translations of this document are unofficial and may contain errors.
}\DIFdelend 

This document is structured as a specification, and is not meant to provide a step by
step algorithm for configuring end-user board support package (BSP). \DIFdelbegin \DIFdel{Any third-party
articles, tools}\DIFdelend \DIFaddbegin \DIFadd{The intended audience
...
\DIFadd{Third-party articles, utilities}\DIFaddend , books, \DIFdelbegin \DIFdel{etc. , providing such material }\DIFdelend \DIFaddbegin \DIFadd{and alike may be more useful for a wider audience as
...
%DIFDELCMD < %%%
\DIFdel{(}%DIFDELCMD < \href{https://dortania.github.io/getting-started}{parent link}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \href{https://dortania.github.io}{Dortania}\DIFadd{'s
}\href{https://dortania.github.io/OpenCore-Desktop-Guide}{OpenCore Desktop Guide}
\DIFadd{and }\href{https://dortania.github.io/getting-started}{related material}\DIFaddend ,
please ensure \DIFdelbegin \DIFdel{following }\DIFdelend \DIFaddbegin \DIFadd{to follow }\DIFaddend this document for every made decision and \DIFdelbegin \DIFdel{judging }\DIFdelend \DIFaddbegin \DIFadd{judge }\DIFaddend its consequences.
...
\item
  \DIFaddbegin \texttt{\DIFadd{config.plist}} \\
  \texttt{\DIFadd{OC Config}}\DIFadd{.
}\item
  \DIFaddend \texttt{vault.plist} \DIFdelbegin %DIFDELCMD < \break
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \\
  \DIFaddend Hashes for all files potentially loadable by \texttt{OC Config}.
\item
  \DIFdelbegin \texttt{\DIFdel{config.plist}}
  %DIFAUXCMD
%DIFDELCMD < \break
%DIFDELCMD <   %%%
\texttt{\DIFdel{OC Config}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend \texttt{vault.sig} \DIFdelbegin %DIFDELCMD < \break
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \\
  \DIFaddend Signature for \texttt{vault.plist}.
\item
  \DIFaddbegin \texttt{\DIFadd{SysReport}} \\
  \DIFadd{Directory containing system reports generated by }\texttt{\DIFadd{SysReport}} \DIFadd{option.
}\item
  \DIFaddend \texttt{nvram.plist} \DIFdelbegin %DIFDELCMD < \break
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \\
...
\DIFaddbegin \item
  \texttt{\DIFadd{panic-YYYY-MM-DD-HHMMSS.txt}} \\
  \DIFadd{Kernel panic log file.
...
\texttt{\DIFdelbegin \DIFdel{DuetPkg}\DIFdelend \DIFaddbegin \DIFadd{OpenDuetPkg}\DIFaddend } with a dedicated tool \DIFaddbegin \DIFadd{--- }\DIFaddend BootInstall (bundled with OpenCore).
\DIFaddbegin \href{https://github.com/corpnewt/gibMacOS}{Third-party utilities} \DIFadd{can be used to
...
\DIFdelbegin \textbf{\DIFdel{Debugging}}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{When trying to find the problematic change it is useful to rely on
...
\DIFaddbegin \DIFadd{ACPI changes apply globally (to every operating system) with the following effective order:
}
...
\DIFadd{There are many places providing ACPI tables and workarounds. Commonly used
...
\DIFadd{and }\href{https://applelife.ru/forums/dsdt.129}{DSDT} \DIFadd{subforums
(e.g. }\href{https://applelife.ru/posts/498967}{Battery register splitting} \DIFadd{guide).
A slightly more user-friendly explanation of some tables included with OpenCore
can also be found in }\href{https://dortania.github.io}{Dortania}\DIFadd{'s
}\href{https://dortania.github.io/Getting-Started-With-ACPI}{Getting started with ACPI} \DIFadd{guide.
For more exotic cases there also are several other places including
}\href{https://github.com/daliansky}{daliansky}\DIFadd{'s
}\href{https://github.com/daliansky/OC-little}{ACPI sample collection}\DIFadd{, but the quality
...
\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: Patches of different }\texttt{\DIFadd{Find}} \DIFadd{and }\texttt{\DIFadd{Replace}} \DIFadd{lengths
...
\item \texttt{CFG Lock} (MSR \texttt{0xE2} write protection) disabled in
  firmware settings if present. \DIFdelbegin \DIFdel{Cconsider
}\DIFdelend \DIFaddbegin \DIFadd{Consider
}\DIFaddend \href{https://github.com/LongSoft/UEFITool/blob/master/UEFIPatch/patches.txt}{patching it}
...
\item
  ...
  like \texttt{DevirtualiseMmio}, \texttt{\DIFdelbegin \DIFdel{ProtectCsmRegion}\DIFdelend \DIFaddbegin \DIFadd{ProtectMemoryRegions}\DIFaddend }, or \texttt{\DIFdelbegin \DIFdel{ShrinkMemoryMap}\DIFdelend \DIFaddbegin \DIFadd{RebuildAppleMemoryMap}\DIFaddend },
...
\item
  ...
  Broadwell-E, \DIFdelbegin \DIFdel{Skylake-X}\DIFdelend \DIFaddbegin \DIFadd{Skylake-SP}\DIFaddend , and similar CPUs. More details on the XCPM patches are outlined in
...
This section contains miscellaneous configuration \DIFdelbegin \DIFdel{entries for OpenCore behaviour that does not go to any other sections
}\DIFdelend \DIFaddbegin \DIFadd{affecting OpenCore operating system loading behaviour
as well as other entries, which do not go to any other section.
}

\DIFadd{OpenCore tries to follow ``bless'' model also known as ``Apple Boot Policy''. The primary specialty of
...
\DIFaddbegin \DIFadd{Each partition will only be used for booting when it corresponds to ``Scan policy'': a set of restrictions
to only use partitions with specific file systems and from specific device types. Scan policy behaviour is
discussed in }\texttt{\DIFadd{ScanPolicy}} \DIFadd{property description.
}

\DIFadd{Scan process starts with obtaining all the partitions filtered with ``Scan policy''. Each partition may
...
Each primary and alternate option can be an auxiliary option or not, refer to }\texttt{\DIFadd{HideAuxiliary}}
\DIFadd{for more details. Algorithm to determine boot options behaves as follows:
}

\begin{enumerate}
\tightlist
\item \DIFadd{Obtain all available partition handles filtered by ``Scan policy'' (and driver availability).
}\item \DIFadd{Obtain all available boot options from }\texttt{\DIFadd{BootOrder}} \DIFadd{UEFI variable.
}\item \DIFadd{For each found boot option:
  }\begin{itemize}
  \item \DIFadd{Retrieve device path of the boot option.
  %DIF >  Scan policy restrictions are actually checked here as we want the function to be self-contained
  %DIF >  for non-scan based startup.
  }\item \DIFadd{Perform fixups (e.g. NVMe subtype correction) and expansion (e.g. for Boot Camp) of the device path.
  }\item \DIFadd{Obtain device handle by locating device path of the resulting device path (ignore it on failure).
  }\item \DIFadd{Find device handle in the list of partition handles (ignore it if missing).
  ...
  }\item \DIFadd{For disk device paths (not specifying a bootloader) execute ``bless'' (may return > 1 entry).
  }\item \DIFadd{For file device paths check presence on the file system directly.
  }\item \DIFadd{Exclude options with blacklisted filenames (refer to }\texttt{\DIFadd{BlacklistAppleUpdate}} \DIFadd{option).
  %DIF >  Just kill all \EFI\APPLE\ paths.
  }\item \DIFadd{On OpenCore boot partition exclude all OpenCore bootstrap files by header checks.
  }\item \DIFadd{Mark device handle as }\textit{\DIFadd{used}} \DIFadd{in the list of partition handles if any.
  %DIF >  Each partition handle will basically have a list of boot option entries for later quick lookup.
  }\item \DIFadd{Register the resulting entries as primary options and determine their types. }\\
  \DIFadd{The option will become auxiliary for some types (e.g. Apple HFS recovery).
  }\end{itemize}
\item \DIFadd{For each partition handle:
  }\begin{itemize}
  \item \DIFadd{If partition handle is marked as }\textit{\DIFadd{unused}} \DIFadd{execute ``bless'' primary option list retrieval. }\\
    \DIFadd{In case }\texttt{\DIFadd{BlessOverride}} \DIFadd{list is set, not only standard ``bless'' paths will be found but
    also custom ones.
  }\item \DIFadd{Exclude options with blacklisted filenames (refer to }\texttt{\DIFadd{BlacklistAppleUpdate}} \DIFadd{option).
  }\item \DIFadd{On OpenCore boot partition exclude all OpenCore bootstrap files by header checks.
  }\item \DIFadd{Register the resulting entries as primary options and determine their types if found. }\\
  \DIFadd{The option will become auxiliary for some types (e.g. Apple HFS recovery).
  %DIF >  Looking up primary and alternate handles could be done per handle to make sure the list is ordered.
  }\item \DIFadd{If partition already has primary options of ``Apple Recovery'' type proceed to next handle.
  }\item \DIFadd{Lookup alternate entries by ``bless'' recovery option list retrieval and predefined paths.
  }\item \DIFadd{Register the resulting entries as alternate auxiliary options and determine their types if found.
  }\end{itemize}
\item \DIFadd{Custom entries and tools are added as primary options without any checks with respect to }\texttt{\DIFadd{Auxiliary}}\DIFadd{.
}\item \DIFadd{System entries (e.g. }\texttt{\DIFadd{Reset NVRAM}}\DIFadd{) are added as primary auxiliary options.
}\end{enumerate}

\DIFadd{The display order of the boot options in the picker and the boot process are determined separately from the scanning
algorithm. The display order as follows:
}

\begin{itemize}
\tightlist
\item \DIFadd{Alternate options follow corresponding primary options, i.e. Apple recovery will be following the
  relevant macOS option whenever possible.
}\item \DIFadd{Options will be listed in file system handle firmware order to maintain an established order across
  the reboots regardless of the chosen operating system for loading.
}\item \DIFadd{Custom entries, tools, and system entries will be added after all other options.
}\item \DIFadd{Auxiliary options will only show upon entering ``Advanced Mode'' in the picker (usually by pressing ``Space'').
}\end{itemize}

\DIFadd{The boot process is as follows:
}\begin{itemize}
\tightlist
\item \DIFadd{Try looking up first valid primary option through }\texttt{\DIFadd{BootNext}} \DIFadd{UEFI variable.
}\item \DIFadd{On failure looking up first valid primary option through }\texttt{\DIFadd{BootOrder}} \DIFadd{UEFI variable.
}\item \DIFadd{Mark the option as the default option to boot.
}\item \DIFadd{Boot option through the picker or without it depending on the }\texttt{\DIFadd{ShowPicker}} \DIFadd{option.
}\item \DIFadd{Show picker on failure otherwise.
}\end{itemize}
...
\item
  \DIFdelbegin \texttt{\DIFdel{HideSelf}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Hides own boot entry from boot picker. This
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend \texttt{PickerAttributes}\\
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{ApplePanic}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Save macOS kernel panic to OpenCore root partition.
}

  \DIFadd{The file is saved as }\texttt{\DIFadd{panic-YYYY-MM-DD-HHMMSS.txt}}\DIFadd{. It is strongly
  recommended to have }\texttt{\DIFadd{keepsyms=1}} \DIFadd{boot argument to see debug symbols
  in the panic log. In case it was not present }\texttt{\DIFadd{kpdescribe.sh}} \DIFadd{utility
  (bundled with OpenCore) may be used to partially recover the stacktrace.
}

  \DIFadd{Development and debug kernels produce more helpful kernel panics.
  Consider downloading and installing }\texttt{\DIFadd{KernelDebugKit}} \DIFadd{from
  }\href{https://developer.apple.com}{developer.apple.com} \DIFadd{when debugging a problem.
  To activate a development kernel you will need to add a }\texttt{\DIFadd{kcsuffix=development}}
  \DIFadd{boot argument. Use }\texttt{\DIFadd{uname -a}} \DIFadd{command to ensure that your current loaded
  kernel is a development (or a debug) kernel.
}

  \DIFadd{In case OpenCore kernel panic saving mechanism was not used, kernel panics may
  still be found in }\texttt{\DIFadd{/Library/Logs/DiagnosticReports}} \DIFadd{directory.
  Starting with macOS Catalina kernel panics are stored in JSON format, so they
  need to be preprocessed before passing to }\texttt{\DIFadd{kpdescribe.sh}}\DIFadd{:
}

\DIFmodbegin
\begin{lstlisting}[label=kpanic, style=ocbash,alsolanguage=DIFcode]
%DIF > cat Kernel.panic | grep macOSProcessedStackshotData |
%DIF >   python -c 'import json,sys;print(json.load(sys.stdin)["macOSPanicString"])'
\end{lstlisting}
\DIFmodend
...
\item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Produce system report on ESP folder.
}

  \DIFadd{This option will create a }\texttt{\DIFadd{SysReport}} \DIFadd{directory on ESP partition
  unless it is already present. The directory will contain ACPI and SMBIOS dumps.
}

  \emph{\DIFadd{Note}}\DIFadd{: For security reasons }\texttt{\DIFadd{SysReport}} \DIFadd{option is }\textbf{\DIFadd{not}} \DIFadd{available
  in }\texttt{\DIFadd{RELEASE}} \DIFadd{builds. Use a }\texttt{\DIFadd{DEBUG}} \DIFadd{build if you need this option.
}

\item
  \DIFaddend \texttt{Target}\\
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0}\\
...
\end{lstlisting}
  ...
  }\item \texttt{\DIFadd{VMOPT}} \DIFadd{--- VerifyMemOpt
  }\end{itemize}
...
\item
  ...
  \emph{Note 2}: Be warned that \DIFdelbegin \DIFdel{NVRAM reset will also }\DIFdelend \DIFaddbegin \DIFadd{while NVRAM reset executed from OpenCore should not }\DIFaddend erase the boot
  option created in \texttt{Bootstrap}\DIFdelbegin \DIFdel{mode}\DIFdelend \DIFaddbegin \DIFadd{, executing NVRAM reset prior to loading OpenCore will remove it}\DIFaddend .
...
\item
  \texttt{ScanPolicy}\\
  \textbf{Type}: \texttt{plist\ integer}, 32 bit\\
  \textbf{Failsafe}: \texttt{\DIFdelbegin \DIFdel{0xF0103}\DIFdelend \DIFaddbegin \DIFadd{0x10F0103}\DIFaddend }\\
  ...
  \tightlist
    ...
    \DIFaddbegin \item \texttt{\DIFadd{0x01000000}} \DIFadd{(bit }\texttt{\DIFadd{24}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_PCI}}\DIFadd{, allow
...
\item
  ...
  enough\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{:
  }\begin{itemize}
  \tightlist
  \item \DIFaddend When enabled \texttt{SMBIOS}, \texttt{DataHub}, and
  \texttt{PlatformNVRAM} data is unused.
  \item \DIFaddbegin \DIFadd{When disabled }\texttt{\DIFadd{Generic}} \DIFadd{section is unused.
  }\end{itemize}
\item
...
\DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{CrScreenshotDxe}}\textbf{\DIFadd{*}}
& \DIFadd{Screenshot making driver saving images to the root of OpenCore partition (ESP) or
  any available writeable filesystem upon pressing }\texttt{\DIFadd{F10}}\DIFadd{.
  This is a modified version of }\href{https://github.com/LongSoft/CrScreenshotDxe}{\texttt{CrScreenshotDxe}}
  \DIFadd{driver by }\href{https://github.com/NikolajSchlej}{Nikolaj Schlej}\DIFadd{. }\\
...
\DIFaddbegin \href{https://github.com/acidanthera/OcBinaryData}{\texttt{PartitionDxe}}
& \DIFadd{Proprietary partition management driver with Apple Partitioning Scheme support
  ...
  For Sandy Bridge and earlier CPUs }\texttt{\DIFadd{PartitionDxeLegacy}} \DIFadd{driver should be
...
\DIFaddbegin \DIFadd{Font is Helvetica 12 pt times scale factor.
}\DIFaddend 

\DIFaddbegin \DIFadd{Font format corresponds to }\href{https://www.angelcode.com/products/bmfont}{AngelCode binary BMF}\DIFadd{.
While there are many utilities to generate font files, currently it is recommended to use
}\href{https://github.com/danpla/dpfontbaker}{dpFontBaker} \DIFadd{to generate bitmap font
(}\href{https://github.com/danpla/dpfontbaker/pull/1}{using CoreText produces best results}\DIFadd{)
and }\href{https://github.com/usr-sse2/fonverter}{fonverter} \DIFadd{to export it to binary format.
}

\DIFaddend \emph{WARNING}: OpenCanopy is currently considered experimental and is not recommended for
...
\item
  \DIFdelbegin \texttt{\DIFdel{DirectGopCacheMode}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
...
%DIFDELCMD <   %%%
\textbf{\DIFdel{Description}}%DIFAUXCMD
\DIFdel{: Cache mode for builtin graphics output protocol framebuffer.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{Tuning cache mode may provide better rendering performance on some firmwares.
  Providing empty string leaves cache control settings to the firmware.
  Valid non-empty values are: }\texttt{\DIFdel{Uncacheable}}%DIFAUXCMD
\DIFdel{, }\texttt{\DIFdel{WriteCombining}}%DIFAUXCMD
\DIFdel{, and
  }\texttt{\DIFdel{WriteThrough}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\emph{\DIFdel{Note}}%DIFAUXCMD
\DIFdel{: This option is not supported on most hardware (see
  }%DIFDELCMD < \href{https://github.com/acidanthera/bugtracker/issues/755}{acidanthera/bugtracker\#755}
%DIFDELCMD <   %%%
\DIFdel{for more details).
}%DIFDELCMD < 

%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend \texttt{DirectGopRendering}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{TscSyncTimeout}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Attempts to perform TSC synchronisation with a specified timeout.
}

  \DIFadd{The primary purpose of this quirk is to enable early bootstrap TSC synchronisation
  ...
  amount of cores present on the platform, the recommended starting value is }\texttt{\DIFadd{500000}}\DIFadd{.
}

  \DIFadd{This is an experimental quirk, which should only be used for the aforementioned problem.
  ...
  }\href{https://github.com/RehabMan/VoodooTSCSync}{VoodooTSCSync}\DIFadd{,
  }\href{https://github.com/interferenc/TSCAdjustReset}{TSCAdjustReset}\DIFadd{,
  or }\href{https://github.com/lvs1974/CpuTscSync}{CpuTscSync} \DIFadd{(a more specialised
  variant of VoodooTSCSync for newer laptops).
}

  \emph{\DIFadd{Note}}\DIFadd{: The reason this quirk cannot replace the kernel driver is
...
\item
  \DIFaddend \texttt{UnblockFsConnect}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\DIFdelend All the modifications applied (to ACPI, NVRAM, SMBIOS, etc.) are supposed
  ...
  be installed. Some of the known \DIFdelbegin \DIFdel{tools }\DIFdelend \DIFaddbegin \DIFadd{utilities }\DIFaddend are:
  ...
  This can be done with many \DIFdelbegin \DIFdel{tools }\DIFdelend \DIFaddbegin \DIFadd{utilities }\DIFaddend including open-source
...
\item
  ...
  using \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/OpenCoreShell}{UEFI Shell} %%%
\DIFdelend \DIFaddbegin \texttt{\DIFadd{UEFI Shell}} \DIFadd{(bundled with OpenCore) }\DIFaddend may help to see
  early debug messages.

\item
  \DIFaddbegin \textbf{\DIFadd{How to debug macOS boot failure?}}
...
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Utilities/macrecovery/macrecovery.py}{macrecovery.py}\DIFadd{,
  ...
  \DIFdelbegin %DIFDELCMD < \href{https://github.com/corpnewt/gibMacOS}{third-party tools} %%%
\DIFdelend \DIFaddbegin \href{https://github.com/corpnewt/gibMacOS}{third-party utilities} \DIFaddend to download an offline image.
...
\item
  ...
  Mac hardware, some ongoing instructions can be found \DIFdelbegin \DIFdel{in
  }%DIFDELCMD < \href{https://github.com/acidanthera/bugtracker/issues/377}{acidanthera/bugtracker\#377}%%%
...
\item
  ...
  \href{https://applelife.ru/posts/819790}{AppleLife.ru} \DIFaddbegin \DIFadd{or in the ACPI section of this document}\DIFaddend .

\item
  \textbf{How can I \DIFdelbegin \DIFdel{migrate from }\texttt{\DIFdel{AptioMemoryFix}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{decide which }\texttt{\DIFadd{Booter}} \DIFadd{quirks to use}\DIFaddend ?}

  \DIFdelbegin \DIFdel{Behaviour similar to that of }\DIFdelend \DIFaddbegin \DIFadd{These quirks originate from }\DIFaddend \texttt{AptioMemoryFix} \DIFdelbegin \DIFdel{can be obtained by
  installing }\texttt{\DIFdel{OpenRuntime}} %DIFAUXCMD
\DIFdel{driver and enabling the quirks listed below. Please note, that most of these are not necessary to be enabled. Refer to their
  individual descriptions in this document for more details.
  }\DIFdelend \DIFaddbegin \DIFadd{driver but provide a wider
  set of changes specific to modern systems. Note, that }\texttt{\DIFadd{OpenRuntime}}
  \DIFadd{driver is required for most configurations. To get a configuration similar
  to }\texttt{\DIFadd{AptioMemoryFix}} \DIFadd{you may try enabling the following set of quirks:
  }\DIFaddend \begin{itemize}
  \tightlist
  ...
  \item \texttt{\DIFdelbegin \DIFdel{SetupVirtualMap}\DIFdelend \DIFaddbegin \DIFadd{RebuildAppleMemoryMap}\DIFaddend }
  \item \texttt{\DIFdelbegin \DIFdel{ShrinkMemoryMap}\DIFdelend \DIFaddbegin \DIFadd{SetupVirtualMap}\DIFaddend }
  \end{itemize}
\DIFaddbegin 

  \DIFadd{However, as of today such set is strongly discouraged as some of these quirks
  are not necessary to be enabled or need additional quirks. For example,
  }\texttt{\DIFadd{DevirtualiseMmio}} \DIFadd{and }\texttt{\DIFadd{ProtectUefiServices}} \DIFadd{are often required,
  while }\texttt{\DIFadd{DiscardHibernateMap}} \DIFadd{and }\texttt{\DIFadd{ForceExitBootServices}} \DIFadd{are rarely
  necessary.
}
