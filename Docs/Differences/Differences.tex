...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.9\DIFdelbegin \DIFdel{.1}\DIFdelend \DIFaddbegin \DIFadd{.2}\DIFaddend )
...
\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: }\texttt{\DIFadd{RebaseRegions}} \DIFadd{and }\texttt{\DIFadd{SyncTableIds}} \DIFadd{quirks are special and are processed after all other ACPI changes since they can only be applied on the final ACPI configuration including all the patches and added tables.
}

\DIFaddend Applying the changes globally resolves the problems of incorrect operating system
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Requirement}: 10.4\DIFdelbegin \DIFdel{-12}\DIFdelend \\
  ...
  \DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: AppleIntelCPUPowerManagement.kext is removed as of macOS 13.
  However, a legacy version can be injected and thus get patched using this quirk.
}

  \DIFaddend Some types of firmware lock the \texttt{PKG\_CST\_CONFIG\_CONTROL} MSR
...
\item
  \DIFaddbegin \texttt{\DIFadd{DisableIoMapperMapping}}\\
  ...
  \emph{\DIFadd{Note 2}}\DIFadd{: This option is not needed on AMD systems.
}

\item
  \DIFaddend \texttt{DisableLinkeditJettison}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{InitialMode}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Auto}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Selects the internal }\texttt{\DIFadd{ConsoleControl}} \DIFadd{mode in
  which }\DIFaddend \texttt{TextRenderer} \DIFaddbegin \DIFadd{will operate.
}
  ...
  rendering modes: }\texttt{\DIFadd{Graphics}} \DIFadd{and }\texttt{\DIFadd{Text}}\DIFadd{. Some types of firmware
  do not provide a native }\texttt{\DIFadd{ConsoleControl}} \DIFadd{and rendering modes. OpenCore
  ...
  of the system }\texttt{\DIFadd{ConsoleControl}} \DIFadd{protocol, when it exists, may vary.
}

\item
  \texttt{\DIFadd{TextRenderer}}\DIFaddend \\
  \textbf{Type}: \texttt{plist\ string}\\
  ...
  \texttt{System}. \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{System} renderer uses firmware services
  for text rendering\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, however with additional options provided to
  sanitize the output. The }\DIFaddend \texttt{Builtin} \DIFdelbegin \DIFdel{bypassing }\DIFdelend \DIFaddbegin \DIFadd{renderer bypasses }\DIFaddend firmware
  services and performs text rendering on its own. \DIFdelbegin \DIFdel{Different renderers support
  }\DIFdelend \DIFaddbegin \DIFadd{Each renderer
  supports }\DIFaddend a different set of options. It is recommended to use \DIFaddbegin \DIFadd{the
  }\DIFaddend \texttt{Builtin} renderer, as it supports HiDPI mode and uses
  full screen resolution.

  \DIFdelbegin \DIFdel{UEFI firmware typically supports }\DIFdelend \DIFaddbegin \DIFadd{Each renderer provides its own }\DIFaddend \texttt{ConsoleControl} \DIFdelbegin \DIFdel{with two
  rendering modes: }\texttt{\DIFdel{Graphics}} %DIFAUXCMD
\DIFdel{and }\texttt{\DIFdel{Text}}%DIFAUXCMD
\DIFdel{. Some types of firmware
  do not support }\DIFdelend \DIFaddbegin \DIFadd{protocol
  (in the case of }\DIFaddend \texttt{\DIFdelbegin \DIFdel{ConsoleControl}%DIFDELCMD < \MBLOCKRIGHTBRACE %%%
\DIFdel{and rendering modes. OpenCore
  and macOS expect text to onlybe shown in }\texttt{\DIFdel{Graphics}} %DIFAUXCMD
\DIFdel{mode and
  graphics to be drawn in any mode. Since this is not required by UEFI
  specification,
  exact behaviour varies}\DIFdelend \DIFaddbegin \DIFadd{SystemGeneric}} \DIFadd{only, this passes some
  operations through to the system }\texttt{\DIFadd{ConsoleControl}} \DIFadd{protocol,
  if one exists)}\DIFaddend .

  Valid values \DIFaddbegin \DIFadd{of this option }\DIFaddend are combinations of \DIFdelbegin \DIFdel{text renderer and rendering mode :
}\DIFdelend \DIFaddbegin \DIFadd{the renderer to use
  ...
  \item \texttt{BuiltinGraphics} --- Switch to \texttt{Graphics}
    mode \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend use \texttt{Builtin} renderer with
    custom \texttt{ConsoleControl}.
  \item \texttt{BuiltinText} --- Switch to \texttt{Text}
    mode \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend use \texttt{Builtin} renderer with
    custom \texttt{ConsoleControl}.
  \item \texttt{SystemGraphics} --- Switch to \texttt{Graphics}
    mode \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend use \texttt{System} renderer with
    custom \texttt{ConsoleControl}.
  \item \texttt{SystemText} --- Switch to \texttt{Text}
    mode \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend use \texttt{System} renderer with
    custom \texttt{ConsoleControl}.
  \item \texttt{SystemGeneric} --- Use \texttt{System} renderer with
    \DIFaddbegin \DIFadd{custom a }\texttt{\DIFadd{ConsoleControl}} \DIFadd{protocol which passes its mode set and
    get operations through to }\DIFaddend system \texttt{ConsoleControl} \DIFdelbegin \DIFdel{assuming it behaves correctly}\DIFdelend \DIFaddbegin \DIFadd{when it exists}\DIFaddend .
  \end{itemize}
  ...
  an alternative \DIFaddbegin \DIFadd{to }\DIFaddend \texttt{BuiltinGraphics} for some very old and defective
...
\DIFdelbegin \DIFdel{This option is currently ignored as the }\texttt{\DIFdel{Builtin}} %DIFAUXCMD
...
\item
  ...
  \emph{Note \DIFaddbegin \DIFadd{1}\DIFaddend }: This \DIFaddbegin \DIFadd{quirk }\DIFaddend takes effect whether or not \texttt{DirectGopRendering} is set, and
  \DIFdelbegin \DIFdel{may
  give some }\DIFdelend \DIFaddbegin \DIFadd{in some cases may give a noticeable }\DIFaddend speed-up to GOP operations even when \texttt{DirectGopRendering}
  is \texttt{false}.

  \DIFaddbegin \emph{\DIFadd{Note 2}}\DIFadd{: On most systems from circa 2013 onwards, write-combining
  caching is already applied by the firmware to GOP memory, in which case }\texttt{\DIFadd{GopBurstMode}}
  \DIFadd{is unnecessary. On such systems enabling the quirk should normally be harmless, producing an
  }\texttt{\DIFadd{OCC:}} \DIFadd{debug log entry indicating that burst mode is already started.
}

  \emph{\DIFadd{Note 3}}\DIFadd{: Some caution should be taken when enabling this quirk, as it has been observed
...
\item
  ...
  discard all text output \DIFdelbegin \DIFdel{when }\DIFdelend \DIFaddbegin \DIFadd{if }\DIFaddend console control is \DIFdelbegin \DIFdel{in a different mode from }\DIFdelend \DIFaddbegin \DIFadd{not in }\DIFaddend \texttt{Text} \DIFaddbegin \DIFadd{mode}\DIFaddend .
