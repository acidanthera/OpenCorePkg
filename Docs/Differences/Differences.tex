...
%DIF ADD ../Configuration.tex        Mon Apr  6 04:46:56 2020
...
\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: It is not guaranteed that paths longer than
}\texttt{\DIFadd{OC\_STORAGE\_SAFE\_PATH\_MAX}} \DIFadd{(128 characters including
}\texttt{\\\DIFadd{0}}\DIFadd{-termnator) will be accessible within OpenCore.
}

\DIFaddend \subsection{Installation and Upgrade}\label{configuration-install}
...
\item
  ...
  in \texttt{\DIFdelbegin \DIFdel{FwRuntimeServices}\DIFdelend \DIFaddbegin \DIFadd{OpenRuntime}\DIFaddend .efi}.
...
\item
  ...
  }\texttt{\DIFadd{RebuildAppleMemoryMap}} \DIFadd{if your firmware supports memory attributes table (MAT)}\DIFaddend .
...
\item
  \texttt{\DIFdelbegin \DIFdel{ProtectCsmRegion}\DIFdelend \DIFaddbegin \DIFadd{ProtectMemoryRegions}\DIFaddend }\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Protect \DIFdelbegin \DIFdel{CSM region areas from relocation}\DIFdelend \DIFaddbegin \DIFadd{memory regions from incorrect access}\DIFaddend .

  \DIFdelbegin \DIFdel{Ensure that CSM memory regions are marked as ACPI NVS to prevent boot.
  efi or XNU from
  relocating or using them}\DIFdelend \DIFaddbegin \DIFadd{Some firmwares incorrectly map select memory regions:
}

  \begin{itemize}
    \tightlist
    \item \DIFadd{CSM region can be marked as boot services code or data, which
      leaves it as free memory for XNU kernel.
    }\item \DIFadd{MMIO regions can be marked as reserved memory and stay unmapped,
      but may be required to be accessible at runtime for NVRAM support.
  }\end{itemize}

  \DIFadd{This quirk attempts to fix types of these regions, e.g. ACPI NVS for
  CSM or MMIO for MMIO}\DIFaddend .

  \emph{Note}: The necessity of this quirk is determined by artifacts\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend sleep
  wake issues\DIFdelbegin \DIFdel{.
  As }\texttt{\DIFdel{AvoidRuntimeDefrag}} %DIFAUXCMD
\DIFdel{resolves a similar problem, no known firmwares should }\DIFdelend \DIFaddbegin \DIFadd{, and boot failures. In general only very old firmwares }\DIFaddend need
  this quirk.
\DIFdelbegin \DIFdel{Do not use this unless you fully understand the consequences.
}\DIFdelend 
...
\item
  ...
  \emph{\DIFadd{Note 1}}\DIFadd{: Since many firmwares come with incorrect memory protection
  table this quirk often comes in pair with }\texttt{\DIFadd{SyncRuntimePermissions}}\DIFadd{.
}

  \emph{\DIFadd{Note 2}}\DIFadd{: The necessity of this quirk is determined by early boot failures.
  This quirk replaces }\texttt{\DIFadd{EnableWriteUnprotector}} \DIFadd{on firmwares supporting
...
\DIFaddbegin \item
  ...
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Update memory permissions for runtime environment.
}

  \DIFadd{Some firmwares either fail to properly handle runtime permissions:
  }\begin{itemize}
    \tightlist
...
called \texttt{\DIFdelbegin \DIFdel{EfiDevicePropertyDatabase}\DIFdelend \DIFaddbegin \DIFadd{EfiDevicePathPropertyDatabase}\DIFaddend }. This buffer is a serialised
...
\item
  ...
  \emph{Note}: Select tools, for example, \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/OpenCoreShell}{UEFI Shell} %%%
\DIFdelend \DIFaddbegin \DIFadd{UEFI Shell, }\DIFaddend are very
...
\item
  \DIFaddbegin \texttt{\DIFadd{ConsoleAttributes}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ integer}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{0}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Sets specific attributes for console.
}

  \DIFadd{Text renderer supports colour arguments as a sum of foreground and background
  colors according to UEFI specification. The value of black background and
  black foreground (}\texttt{\DIFadd{0}}\DIFadd{) is reserved. List of colour names:
}

  \begin{itemize}
  \tightlist
  \item \texttt{\DIFadd{0x00}} \DIFadd{--- }\texttt{\DIFadd{EFI\_BLACK}}
  \item \texttt{\DIFadd{0x01}} \DIFadd{--- }\texttt{\DIFadd{EFI\_BLUE}}
  \item \texttt{\DIFadd{0x02}} \DIFadd{--- }\texttt{\DIFadd{EFI\_GREEN}}
  \item \texttt{\DIFadd{0x03}} \DIFadd{--- }\texttt{\DIFadd{EFI\_CYAN}}
  \item \texttt{\DIFadd{0x04}} \DIFadd{--- }\texttt{\DIFadd{EFI\_RED}}
  \item \texttt{\DIFadd{0x05}} \DIFadd{--- }\texttt{\DIFadd{EFI\_MAGENTA}}
  \item \texttt{\DIFadd{0x06}} \DIFadd{--- }\texttt{\DIFadd{EFI\_BROWN}}
  \item \texttt{\DIFadd{0x07}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTGRAY}}
  \item \texttt{\DIFadd{0x08}} \DIFadd{--- }\texttt{\DIFadd{EFI\_DARKGRAY}}
  \item \texttt{\DIFadd{0x09}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTBLUE}}
  \item \texttt{\DIFadd{0x0A}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTGREEN}}
  \item \texttt{\DIFadd{0x0B}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTCYAN}}
  \item \texttt{\DIFadd{0x0C}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTRED}}
  \item \texttt{\DIFadd{0x0D}} \DIFadd{--- }\texttt{\DIFadd{EFI\_LIGHTMAGENTA}}
  \item \texttt{\DIFadd{0x0E}} \DIFadd{--- }\texttt{\DIFadd{EFI\_YELLOW}}
...
\item
  ...
  \DIFdelbegin \DIFdel{Builtin picker supports colour arguments as a sum of foreground and background
  colors according to UEFI specification. The value of black background and
  black foreground (}\DIFdelend \DIFaddbegin \DIFadd{Different pickers may be configured through the attribute mask containing
  OpenCore-reserved (}\texttt{\DIFadd{BIT0}}\DIFadd{\textasciitilde}\texttt{\DIFadd{BIT15}}\DIFadd{) and OEM-specific
  (}\DIFaddend \texttt{\DIFdelbegin \DIFdel{0}%DIFDELCMD < \MBLOCKRIGHTBRACE%%%
\DIFdel{) is reserved.
List of colour names}\DIFdelend \DIFaddbegin \DIFadd{BIT16}}\DIFadd{\textasciitilde}\texttt{\DIFadd{BIT31}}\DIFadd{) values.
}

  \DIFadd{Current OpenCore values include}\DIFaddend :

  \begin{itemize}
  \tightlist
  \item \texttt{\DIFdelbegin \DIFdel{0x00}\DIFdelend \DIFaddbegin \DIFadd{0x0001}\DIFaddend } --- \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{OC}\DIFaddend \_\DIFdelbegin \DIFdel{BLACK}\DIFdelend \DIFaddbegin \DIFadd{ATTR\_USE\_VOLUME\_ICON}\DIFaddend }\DIFaddbegin \DIFadd{, provides custom icons
    for boot entries:
    }\begin{itemize}
      \tightlist
      \DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x01}\DIFdelend \DIFaddbegin \DIFadd{.VolumeIcon.icns}\DIFaddend } \DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \DIFadd{file at }\DIFaddend \texttt{\DIFdelbegin \DIFdel{EFI\_BLUE}\DIFdelend \DIFaddbegin \DIFadd{Preboot}\DIFaddend } \DIFaddbegin \DIFadd{root for APFS.
      }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x02}\DIFdelend \DIFaddbegin \DIFadd{.VolumeIcon.icns}\DIFaddend } \DIFdelbegin \DIFdel{--- }\texttt{\DIFdel{EFI\_GREEN}}
  %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{file at volume root for other filesystems.
      }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x03}\DIFdelend \DIFaddbegin \DIFadd{<TOOL\_NAME>.icns}\DIFaddend } \DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \DIFadd{file for }\DIFaddend \texttt{\DIFdelbegin \DIFdel{EFI\_CYAN}\DIFdelend \DIFaddbegin \DIFadd{Tools}\DIFaddend }\DIFaddbegin \DIFadd{.
    }\end{itemize}
    \DIFadd{Volume icons can be set in Finder.
  }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x04}\DIFdelend \DIFaddbegin \DIFadd{0x0002}\DIFaddend } --- \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{OC}\DIFaddend \_\DIFdelbegin \DIFdel{RED}\DIFdelend \DIFaddbegin \DIFadd{ATTR\_USE\_DISK\_LABEL\_FILE}\DIFaddend }\DIFaddbegin \DIFadd{, provides custom
    rendered titles for boot entries:
    }\begin{itemize}
      \tightlist
      \DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x05}\DIFdelend \DIFaddbegin \DIFadd{.disk\_label}\DIFaddend } \DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{.disk}\DIFaddend \_\DIFdelbegin \DIFdel{MAGENTA}\DIFdelend \DIFaddbegin \DIFadd{label\_2x}\DIFaddend }\DIFaddbegin \DIFadd{) file near bootloader for all filesystems.
      }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x06}\DIFdelend \DIFaddbegin \DIFadd{<TOOL\_NAME.lbl}\DIFaddend } \DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{<TOOL}\DIFaddend \_\DIFdelbegin \DIFdel{BROWN}\DIFdelend \DIFaddbegin \DIFadd{NAME.l2x}\DIFaddend }\DIFaddbegin \DIFadd{) file near tool for }\texttt{\DIFadd{Tools}}\DIFadd{.
    }\end{itemize}
    \DIFadd{Prerendered labels can be generated via }\texttt{\DIFadd{disklabel}} \DIFadd{utility or }\texttt{\DIFadd{bless}} \DIFadd{command.
    When disabled or missing text labels (}\texttt{\DIFadd{.contentDetails}} \DIFadd{or }\texttt{\DIFadd{.disk\_label.contentDetails}}\DIFadd{)
    are to be rendered instead.
  }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x07}\DIFdelend \DIFaddbegin \DIFadd{0x0004}\DIFaddend } --- \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{OC}\DIFaddend \_\DIFdelbegin \DIFdel{LIGHTGRAY}\DIFdelend \DIFaddbegin \DIFadd{ATTR\_USE\_GENERIC\_LABEL\_IMAGE}\DIFaddend }\DIFaddbegin \DIFadd{, provides predefined
    label images for boot entries without custom entries. May give less detail for the actual
    boot entry.
  }\DIFaddend \item \texttt{\DIFdelbegin \DIFdel{0x08}\DIFdelend \DIFaddbegin \DIFadd{0x0008}\DIFaddend } --- \texttt{\DIFdelbegin \DIFdel{EFI}\DIFdelend \DIFaddbegin \DIFadd{OC}\DIFaddend \_\DIFdelbegin \DIFdel{DARKGRAY}\DIFdelend \DIFaddbegin \DIFadd{ATTR\_USE\_ALTERNATE\_ICONS}\DIFaddend }\DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x09}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_LIGHTBLUE}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0A}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_LIGHTGREEN}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0B}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_LIGHTCYAN}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0C}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_LIGHTRED}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0D}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_LIGHTMAGENTA}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0E}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_YELLOW}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x0F}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_WHITE}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x00}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_BLACK}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x10}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_BLUE}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x20}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_GREEN}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x30}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_CYAN}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x40}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_RED}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x50}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_MAGENTA}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x60}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_BROWN}}
  %DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{0x70}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{EFI\_BACKGROUND\_LIGHTGRAY}}
  %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{, changes used icon set to
...
\DIFdel{: This option may not work well with }\texttt{\DIFdel{System}} %DIFAUXCMD
\DIFdel{text renderer. Setting a background different from black could help testing proper GOP functioning.
  }%DIFDELCMD < 
...
\item
  ...
  bypass policy enforcement. \DIFdelbegin \DIFdel{To implement }\texttt{\DIFdel{External}} %DIFAUXCMD
\DIFdel{mode }\DIFdelend \DIFaddbegin \DIFadd{See }\hyperref[ueficanopy]{OpenCanopy} \DIFadd{plugin
  for an example of }\DIFaddend a custom user interface\DIFdelbegin \DIFdel{may
  utilise }%DIFDELCMD < \href{https://github.com/acidanthera/OpenCorePkg}{OpenCorePkg}
%DIFDELCMD <   %%%
\texttt{\DIFdel{OcBootManagementLib}}%DIFAUXCMD
\DIFdel{.
Reference example of external graphics interface is provided in
  }%DIFDELCMD < \href{https://github.com/acidanthera/OpenCorePkg/tree/master/Tests/ExternalUi}{ExternalUi}
%DIFDELCMD <   %%%
\DIFdel{test driver.
  ...
  \emph{Note 1}: Activated \texttt{KeySupport}, \texttt{\DIFdelbegin \DIFdel{AppleUsbKbDxe}\DIFdelend \DIFaddbegin \DIFadd{OpenUsbKbDxe}\DIFaddend }, or similar driver is required
...
\item
  \DIFaddbegin \texttt{\DIFadd{AppleDebug}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Enable }\texttt{\DIFadd{boot.efi}} \DIFadd{debug log saving to OpenCore log.
}

  \emph{\DIFadd{Note}}\DIFadd{: This option only applies to 10.15.4 and newer.
}

\item
  \DIFaddend \texttt{DisableWatchDog}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  ...
  Example: \texttt{\DIFdelbegin \DIFdel{Shell}\DIFdelend \DIFaddbegin \DIFadd{OpenShell}\DIFaddend .efi}.
  \end{itemize}
...
protocol implementation currently offered as a part of \texttt{\DIFdelbegin \DIFdel{FwRuntimeServices}\DIFdelend \DIFaddbegin \DIFadd{OpenRuntime}\DIFaddend } driver.
...
\item
  ...
  of SMBIOS tables, especially in \DIFdelbegin \DIFdel{boot.
efi.
}\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}}\DIFadd{.
}\DIFaddend \item
  ...
  (2013+ at least) to avoid accessing special memory region, especially in \DIFdelbegin \DIFdel{boot.
efi.
}\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}}\DIFadd{.
}\DIFaddend \end{itemize}
...
\item
  ...
  real Macs, but used to avoid extra parsing of SMBIOS tables, especially in \DIFdelbegin \DIFdel{boot.
efi.
}\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}}\DIFadd{.
}\DIFaddend \item
...
\item
 ...
 \item
  ...
  \break
  One-byte data defining \DIFdelbegin \DIFdel{boot.efi }\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}} \DIFaddend user interface scaling. Should be \textbf{01} for normal
  screens and \textbf{02} for HiDPI screens.
 \item
  \texttt{4D1EDE05-38C7-4A6A-9CC6-4BCCA8B38C14:DefaultBackgroundColor}
  \break
  Four-byte \texttt{RGBA} data defining \DIFdelbegin \DIFdel{boot.efi }\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}} \DIFaddend user interface background colour.
...
\item
  ...
  \break
  Booter arguments, similar to \texttt{boot-args} but for \DIFdelbegin \DIFdel{boot. efi. }\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}}\DIFadd{. }\DIFaddend Accepts a set of
  arguments, which are hexadecimal 64-bit values with or without \texttt{0x}.
  At different stages \DIFdelbegin \DIFdel{boot.efi }\DIFdelend \DIFaddbegin \texttt{\DIFadd{boot.efi}} \DIFaddend will request different debugging (logging)
  ...
  \emph{Note}: To \DIFdelbegin \DIFdel{quickly }\DIFdelend see verbose output from \texttt{boot.efi} on \DIFdelbegin \DIFdel{versions }\DIFdelend \DIFaddbegin \DIFadd{modern macOS versions
  enable }\texttt{\DIFadd{AppleDebug}} \DIFadd{option. This will save the log to general OpenCore log.
  For versions }\DIFaddend before 10.15\DIFaddbegin \DIFadd{.4 }\DIFaddend set \texttt{bootercfg} to \texttt{log=1}. \DIFaddbegin \DIFadd{This will
...
\item
  \DIFaddbegin \texttt{\DIFadd{7C436110-AB2A-4BBB-A880-FE41995C9F82:efiboot-perf-record}}
  \break
  \DIFadd{Enable performance log saving in }\texttt{\DIFadd{boot.efi}}\DIFadd{. Performance log is saved to physical
  memory and is pointed by }\texttt{\DIFadd{efiboot-perf-record-data}} \DIFadd{and }\texttt{\DIFadd{efiboot-perf-record-size}}
  \DIFadd{variables. Starting from 10.15.4 it can also be saved to OpenCore log by }\texttt{\DIFadd{AppleDebug}} \DIFadd{option.
}\item
  \DIFaddend \texttt{7C436110-AB2A-4BBB-A880-FE41995C9F82:fmm-computer-name}
  \break
...
\subsection{\DIFdelbegin \DIFdel{Properties}\DIFdelend \DIFaddbegin \DIFadd{Drivers}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{uefiprops}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{uefidrivers}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{enumerate}
%DIFDELCMD < \item
%DIFDELCMD <   %%%
\texttt{\DIFdel{Audio}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
...
%DIFDELCMD <   %%%
\textbf{\DIFdel{Description}}%DIFAUXCMD
\DIFdel{: Configure audio backend support described
  in }%DIFDELCMD < \hyperref[uefiaudioprops]{Audio Properties} %%%
\DIFdel{section below.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{Audio support provides a way for upstream protocols to interact with the
  selected hardware and audio resources. All audio resources should reside
  in }\texttt{\DIFdel{\textbackslash EFI\textbackslash OC\textbackslash Resources\textbackslash Audio}}
  %DIFAUXCMD
\DIFdel{directory. Currently the only supported audio file format is WAVE PCM. While it is
...
%DIFDELCMD <   %%%
\DIFdel{Audio file path is determined by audio type, audio localisation, and audio path. Each filename
  looks as follows: }\texttt{%DIFDELCMD < [%%%
\DIFdel{audio type}%DIFDELCMD < ]%%%
\DIFdel{\_}%DIFDELCMD < [%%%
\DIFdel{audio localisation}%DIFDELCMD < ]%%%
\DIFdel{\_}%DIFDELCMD < [%%%
\DIFdel{audio path}%DIFDELCMD < ]%%%
\DIFdel{.wav}}%DIFAUXCMD
\DIFdel{. For unlocalised
...
%DIFDELCMD <   \item %%%
\item%DIFAUXCMD
\DIFdel{Audio type can be }\texttt{\DIFdel{OCEFIAudio}} %DIFAUXCMD
\DIFdel{for OpenCore audio files or
    }\texttt{\DIFdel{AXEFIAudio}} %DIFAUXCMD
\DIFdel{for macOS bootloader audio files.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Audio localisation is a two letter language code (e.g. }\texttt{\DIFdel{en}}%DIFAUXCMD
\DIFdel{)
  with an exception for Chinese, Spanish, and Portuguese. Refer to
  }%DIFDELCMD < \href{https://github.com/acidanthera/EfiPkg/blob/master/Include/Protocol/AppleVoiceOver.h}{\texttt{APPLE\_VOICE\_OVER\_LANGUAGE\_CODE} definition}
%DIFDELCMD <   %%%
\DIFdel{for the list of all supported localisations.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Audio path is the base filename corresponding to a file identifier. For macOS bootloader audio paths refer to
  }%DIFDELCMD < \href{https://github.com/acidanthera/EfiPkg/blob/master/Include/Protocol/AppleVoiceOver.h}{\texttt{APPLE\_VOICE\_OVER\_AUDIO\_FILE} definition}%%%
\DIFdel{.
  For OpenCore audio paths refer to
  }%DIFDELCMD < \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Protocol/OcAudio.h}{\texttt{OC\_VOICE\_OVER\_AUDIO\_FILE} definition}%%%
\DIFdel{.
  The only exception is OpenCore boot chime file, which is }\texttt{\DIFdel{OCEFIAudio\_VoiceOver\_Boot.wav}}%DIFAUXCMD
\DIFdel{.
  }
...
%DIFDELCMD <   %%%
\DIFdel{Audio localisation is determined separately for macOS bootloader and OpenCore.
  For macOS bootloader it is set in }\texttt{\DIFdel{preferences.efires}} %DIFAUXCMD
\DIFdel{archive in
  }\texttt{\DIFdel{systemLanguage.utf8}} %DIFAUXCMD
\DIFdel{file and is controlled by the operating system.
  For OpenCore the value of }\texttt{\DIFdel{prev-lang:kbd}} %DIFAUXCMD
\DIFdel{variable is used.
  When native audio localisation of a particular file is missing, English language
  (}\texttt{\DIFdel{en}}%DIFAUXCMD
\DIFdel{) localisation is used. Sample audio files can be found in
  }%DIFDELCMD < \href{https://github.com/acidanthera/OcBinaryData}{OcBinaryData repository}%%%
\DIFdel{.
}%DIFDELCMD < 
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\texttt{\DIFdel{ConnectDrivers}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
\DIFdel{: }\texttt{\DIFdel{plist\ boolean}}%DIFAUXCMD
...
%DIFDELCMD <   %%%
\textbf{\DIFdel{Description}}%DIFAUXCMD
\DIFdel{: Perform UEFI controller connection after driver loading.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{This option is useful for loading drivers following UEFI driver model
...
%DIFDELCMD <   %%%
\emph{\DIFdel{Note}}%DIFAUXCMD
\DIFdel{: Some firmwares, made by Apple in particular, only connect the boot
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\texttt{\DIFdel{Drivers}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
\DIFdel{: }\texttt{\DIFdel{plist\ array}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Failsafe}}%DIFAUXCMD
\DIFdel{: None}%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Description}}%DIFAUXCMD
\DIFdel{: Load selected drivers from }\texttt{\DIFdel{OC/Drivers}}
  %DIFAUXCMD
\DIFdel{directory.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{Designed to be filled with string filenames meant to be loaded as UEFI
  drivers. }\DIFdelend Depending on the firmware a different set of drivers may be required.
Loading an incompatible driver may lead your system to unbootable state or
even cause permanent firmware damage. Some of the known drivers \DIFdelbegin \DIFdel{include}\DIFdelend \DIFaddbegin \DIFadd{are listed below}\DIFaddend :

\DIFdelbegin %DIFDELCMD < \begin{itemize}
%DIFDELCMD <   \tightlist
%DIFDELCMD <   \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{tabular}{p{1.3in}p{5.55in}}
\DIFaddend \href{https://github.com/acidanthera/AppleSupportPkg}{\texttt{ApfsDriverLoader}}
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend APFS file system bootstrap driver adding the support of embedded APFS drivers
  in bootable APFS containers in UEFI firmwares \DIFdelbegin \DIFdel{.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/AppleSupportPkg}{\texttt{AudioDxe}}
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend HDA audio support driver in UEFI firmwares for most Intel and some other analog audio controllers.
  Refer to \href{https://github.com/acidanthera/bugtracker/issues/740}{acidanthera/bugtracker\#740}
  for known issues in AudioDxe. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/OcBinaryData}{\texttt{ExFatDxe}}
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend Proprietary ExFAT file system driver for Bootcamp support commonly found in Apple
  firmwares. For Sandy Bridge and earlier CPUs \texttt{ExFatDxeLegacy} driver should be
  used due to the lack of \texttt{RDRAND} instruction support. \DIFdelbegin %DIFDELCMD < \item \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{FwRuntimeServices}}
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{--- }\texttt{\DIFdel{OC\_FIRMWARE\_RUNTIME}} %DIFAUXCMD
\DIFdel{protocol implementation that increases the security
  of \mbox{OpenCore} and Lilu by supporting read-only and write-only NVRAM variables. Some
  commonly used quirks, e.g. }\texttt{\DIFdel{RequestBootVarRouting}}%DIFAUXCMD
\DIFdel{, require this driver for proper function.
...
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend Proprietary HFS file system driver with bless support commonly found in Apple
  firmwares. For Sandy Bridge and earlier CPUs \texttt{HfsPlusLegacy} driver should be
  used due to the lack of \texttt{RDRAND} instruction support. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/audk}{\texttt{HiiDatabase}}\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \textbf{\DIFadd{*}}
& \DIFaddend HII services support driver from \texttt{MdeModulePkg}. This driver is included in
  most firmwares starting with Ivy Bridge generation. Some applications with the GUI
  like UEFI Shell may need this driver to work properly. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/audk}{\texttt{EnhancedFatDxe}}
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend FAT filesystem driver from \texttt{FatPkg}. This driver is embedded in all
  ...
  writing to EFI partition is needed during the boot process. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
  \DIFaddend \href{https://github.com/acidanthera/audk}{\texttt{NvmExpressDxe}}\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \textbf{\DIFadd{*}}
& \DIFaddend NVMe support driver from \texttt{MdeModulePkg}. This driver is included in most
  firmwares starting with Broadwell generation. For Haswell and earlier embedding it
  within the firmware may be more favourable in case a NVMe SSD drive is installed. \DIFdelbegin %DIFDELCMD < \item \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{AppleUsbKbDxe}}
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{--- }\DIFdelend \DIFaddbegin \\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{OpenCanopy}}\textbf{\DIFadd{*}}
& \hyperref[ueficanopy]{OpenCore plugin} \DIFadd{implementing graphical interface. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{OpenRuntime}}\textbf{\DIFadd{*}}
& \hyperref[uefiruntime]{OpenCore plugin} \DIFadd{implementing }\texttt{\DIFadd{OC\_FIRMWARE\_RUNTIME}} \DIFadd{protocol. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{OpenUsbKbDxe}}\textbf{\DIFadd{*}}
& \DIFaddend USB keyboard driver adding the support of \texttt{AppleKeyMapAggregator} protocols
  ...
  builtin \texttt{KeySupport}, which may work better or worse depending on the firmware. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/AppleSupportPkg}{\texttt{VBoxHfs}}
\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin & \DIFaddend HFS file system driver with bless support. This driver is an alternative to
  ...
  a security audit. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \DIFaddbegin \\
\DIFaddend \href{https://github.com/acidanthera/audk}{\texttt{XhciDxe}}\DIFdelbegin \DIFdel{--- }\DIFdelend \DIFaddbegin \textbf{\DIFadd{*}}
& \DIFaddend XHCI USB controller support driver from \texttt{MdeModulePkg}. This driver is
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{tabular}
\DIFaddend 

\DIFaddbegin \DIFadd{Driver marked with }\textbf{\DIFadd{*}} \DIFadd{are bundled with OpenCore.
}\DIFaddend To compile the drivers from UDK (EDK II) use the same command you \DIFdelbegin \DIFdel{do }\DIFdelend normally use
...
\DIFaddbegin \subsection{\DIFadd{Tools}}\label{uefitools}

\DIFadd{Standalone tools may help to debug firmware and hardware. Some of the known tools are listed below.
While some tools can be launched from within OpenCore many should be run separately either directly
or from }\texttt{\DIFadd{Shell}}\DIFadd{.
}

\DIFadd{To boot into OpenShell or any other tool directly save }\texttt{\DIFadd{OpenShell.efi}}
\DIFadd{under the name of }\texttt{\DIFadd{EFI\textbackslash BOOT\textbackslash BOOTX64.EFI}}
\DIFadd{on a FAT32 partition. In general it is unimportant whether the partitition scheme
is }\texttt{\DIFadd{GPT}} \DIFadd{or }\texttt{\DIFadd{MBR}}\DIFadd{.
}

\DIFadd{While the previous approach works both on Macs and other computers,
an alternative Mac-only approach to bless the tool on an HFS+ or APFS
volume:
}

\DIFmodbegin
\begin{lstlisting}[caption=Blessing tool, label=blesstool, style=ocbash,alsolanguage=DIFcode]
%DIF > sudo bless --verbose --file /Volumes/VOLNAME/DIR/OpenShell.efi \
%DIF >   --folder /Volumes/VOLNAME/DIR/ --setBoot
\end{lstlisting}
\DIFmodend

\emph{\DIFadd{Note 1}}\DIFadd{: You may have to copy }\texttt{\DIFadd{/System/Library/CoreServices/BridgeVersion.bin}}
  \DIFadd{to }\texttt{\DIFadd{/Volumes/VOLNAME/DIR}}\DIFadd{. }\\
\emph{\DIFadd{Note 2}}\DIFadd{: To be able to use }\texttt{\DIFadd{bless}} \DIFadd{you may have to
  }\href{https://developer.apple.com/library/archive/documentation/Security/Conceptual/System_Integrity_Protection_Guide/ConfiguringSystemIntegrityProtection/ConfiguringSystemIntegrityProtection.html}{disable System Integrity Protection}\DIFadd{. }\\
\emph{\DIFadd{Note 3}}\DIFadd{: To be able to boot you may have to }\href{https://support.apple.com/HT208330}{disable Secure Boot}
  \DIFadd{if present.
}

\DIFadd{Some of the known tools are listed below (builtin tools are marked with }\textbf{\DIFadd{*}}\DIFadd{):
}

\begin{tabular}{p{1.3in}p{5.55in}}
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{BootKicker}}\textbf{\DIFadd{*}}
& \DIFadd{Enter Apple BootPicker menu (exclusive for Macs with compatible GPUs). }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{ChipTune}}\textbf{\DIFadd{*}}
& \DIFadd{Test BeepGen protocol and generate audio signals of different style and length. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{CleanNvram}}\textbf{\DIFadd{*}}
& \DIFadd{Reset NVRAM alternative bundled as a standalone tool. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{GopStop}}\textbf{\DIFadd{*}}
& \DIFadd{Test GraphicsOutput protocol with a
  }\href{https://github.com/acidanthera/OpenCorePkg/tree/master/Application/GopStop}{simple scenario}\DIFadd{. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{HdaCodecDump}}\textbf{\DIFadd{*}}
& \DIFadd{Parse and dump High Definition Audio codec information (requires }\texttt{\DIFadd{AudioDxe}}\DIFadd{). }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{KeyTester}}\textbf{\DIFadd{*}}
& \DIFadd{Test keyboard input in }\texttt{\DIFadd{SimpleText}} \DIFadd{mode. }\\
\href{https://www.memtest86.com}{\texttt{MemTest86}}
& \DIFadd{Memory testing utility. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{OpenControl}}\textbf{\DIFadd{*}}
& \DIFadd{Unlock and lock back NVRAM protection for other tools to be able to get full NVRAM access
  when launching from OpenCore. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{OpenShell}}\textbf{\DIFadd{*}}
& \DIFadd{OpenCore-configured }\href{http://github.com/tianocore/edk2}{\texttt{UEFI Shell}} \DIFadd{for compatibility
  with a broad range of firmwares. }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{PavpProvision}}
& \DIFadd{Perform EPID provisioning (requires certificate data configuration). }\\
\href{https://github.com/acidanthera/OpenCorePkg}{\texttt{VerifyMsrE2}}\textbf{\DIFadd{*}}
& \DIFadd{Check }\texttt{\DIFadd{CFG Lock}} \DIFadd{(MSR }\texttt{\DIFadd{0xE2}} \DIFadd{write protection) consistency across all cores.
}\end{tabular}

\subsection{\DIFadd{OpenCanopy}}\label{ueficanopy}

\DIFadd{OpenCanopy is a graphical OpenCore user interface that runs in
}\texttt{\DIFadd{External}} \texttt{\DIFadd{PickerMode}} \DIFadd{and relies on
}\href{https://github.com/acidanthera/OpenCorePkg}{OpenCorePkg} \texttt{\DIFadd{OcBootManagementLib}}
\DIFadd{similar to the builtin text interface.
}

\DIFadd{OpenCanopy requires graphical resources located in }\texttt{\DIFadd{Resources}} \DIFadd{directory to run.
...
\DIFadd{OpenCanopy provides full support for }\texttt{\DIFadd{PickerAttributes}} \DIFadd{and offers a configurable
builtin icon set. The default chosen icon set depends on the }\texttt{\DIFadd{DefaultBackgroundColor}}
\DIFadd{variable value. For Light Gray }\texttt{\DIFadd{Old}} \DIFadd{icon set will be used, for other colours ---
the one without a prefix.
}

\DIFadd{Predefined icons are put to }\texttt{\DIFadd{\textbackslash EFI\textbackslash OC\textbackslash Resources\textbackslash Image}}
\DIFadd{directory. Full list of supported icons (in }\texttt{\DIFadd{.icns}} \DIFadd{format) is provided below. Missing optional
icons will use the closest available icon. External entries will use }\texttt{\DIFadd{Ext}}\DIFadd{-prefixed
icon if available (e.g. }\texttt{\DIFadd{OldExtHardDrive.icns}}\DIFadd{).
}

\begin{itemize}
\tightlist
  \DIFaddend \item \DIFaddbegin \texttt{\DIFadd{Cursor}} \DIFadd{--- Mouse cursor (mandatory).
  }\item \texttt{\DIFadd{Selected}} \DIFadd{--- Selected item (mandatory).
  }\item \texttt{\DIFadd{Selector}} \DIFadd{--- Selecting item (mandatory).
  }\item \texttt{\DIFadd{HardDrive}} \DIFadd{--- Generic OS (mandatory).
  }\item \texttt{\DIFadd{Apple}} \DIFadd{--- Apple OS.
  }\item \texttt{\DIFadd{AppleRecv}} \DIFadd{--- Apple Recovery OS.
  }\item \texttt{\DIFadd{AppleTM}} \DIFadd{--- Apple Time Machine.
  }\item \texttt{\DIFadd{Windows}} \DIFadd{--- Windows.
  }\item \texttt{\DIFadd{Other}} \DIFadd{--- Custom entry (see }\texttt{\DIFadd{Entries}}\DIFadd{).
  }\item \texttt{\DIFadd{ResetNVRAM}} \DIFadd{--- Reset NVRAM system action or tool.
  }\item \texttt{\DIFadd{Shell}} \DIFadd{--- Entry with UEFI Shell name (e.g. }\texttt{\DIFadd{OpenShell}}\DIFadd{).
  }\item \texttt{\DIFadd{Tool}} \DIFadd{--- Any other tool.
}\end{itemize}

\DIFadd{Predefined labels are put to }\texttt{\DIFadd{\textbackslash EFI\textbackslash OC\textbackslash Resources\textbackslash Label}}
\DIFadd{directory. Each label has }\texttt{\DIFadd{.lbl}} \DIFadd{or }\texttt{\DIFadd{.l2x}} \DIFadd{suffix to represent the scaling level.
Full list of labels is provided below. All labels are mandatory.
}

\begin{itemize}
\tightlist
  \item \texttt{\DIFadd{EFIBoot}} \DIFadd{--- Generic OS.
  }\item \texttt{\DIFadd{Apple}} \DIFadd{--- Apple OS.
  }\item \texttt{\DIFadd{AppleRecv}} \DIFadd{--- Apple Recovery OS.
  }\item \texttt{\DIFadd{AppleTM}} \DIFadd{--- Apple Time Machine.
  }\item \texttt{\DIFadd{Windows}} \DIFadd{--- Windows.
...
\DIFadd{Label and icon generation can be performed with bundled utilities: }\texttt{\DIFadd{disklabel}} \DIFadd{and
}\texttt{\DIFadd{icnspack}}\DIFadd{. Please refer to sample data for the details about the dimensions.
}

\emph{\DIFadd{WARNING}}\DIFadd{: OpenCanopy is currently considered experimental and is not recommended for
everyday use. Refer to
}\href{https://github.com/acidanthera/bugtracker/issues/759}{acidanthera/bugtracker\#759}
\DIFadd{for more details regarding the current limitations.
}

\subsection{\DIFadd{OpenRuntime}}\label{uefiruntime}

\texttt{\DIFadd{OpenRuntime}} \DIFadd{is an OpenCore plugin implementing }\texttt{\DIFadd{OC\_FIRMWARE\_RUNTIME}} \DIFadd{protocol.
...
\begin{itemize}
  \item \DIFadd{NVRAM namespaces, allowing to isolate operating systems from accessing select
  variables (e.g. }\texttt{\DIFadd{RequestBootVarRouting}} \DIFadd{or }\texttt{\DIFadd{ProtectSecureBoot}}\DIFadd{).
  }\item \DIFadd{NVRAM proxying, allowing to manipulate multiple variables on variable updates
  (e.g. }\texttt{\DIFadd{RequestBootVarFallback}}\DIFadd{).
  }\item \DIFadd{Read-only and write-only NVRAM variables, enhancing the security of OpenCore,
  Lilu, and Lilu plugins, like VirtualSMC, which implements }\texttt{\DIFadd{AuthRestart}} \DIFadd{support.
  }\item \DIFadd{NVRAM isolation, allowing to protect all variables from being written from
  an untrusted operating system (e.g. }\texttt{\DIFadd{DisableVariableWrite}}\DIFadd{).
  }\item \DIFadd{UEFI Runtime Services memory protection management to workaround read-only
  mapping (e.g. }\texttt{\DIFadd{EnableWriteUnprotector}}\DIFadd{).
}\end{itemize}

\subsection{\DIFadd{Properties}}\label{uefiprops}
...
\item
  \texttt{\DIFadd{ConnectDrivers}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Perform UEFI controller connection after driver loading.
}

  \DIFadd{This option is useful for loading drivers following UEFI driver model
  as they may not start by themselves. Examples of such drivers are filesystem
  or audio drivers. While effective, this option may not be necessary for drivers
  performing automatic connection, and may slightly slowdown the boot.
}

  \emph{\DIFadd{Note}}\DIFadd{: Some firmwares, made by Apple in particular, only connect the boot
  drive to speedup the boot process. Enable this option to be able to see all the
  boot options when having multiple drives.
}

\item
  \texttt{\DIFadd{Drivers}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ array}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: None}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Load selected drivers from }\texttt{\DIFadd{OC/Drivers}}
  \DIFadd{directory.
}

  \DIFadd{Designed to be filled with string filenames meant to be loaded as UEFI
  drivers.
}

\item
  \DIFaddend \texttt{Input}\\
  \textbf{Type}: \texttt{plist\ dict}\\
  \textbf{Failsafe}: None\\
...
\item
  ...
  a separate driver is used, such as \texttt{\DIFdelbegin \DIFdel{AppleUsbKbDxe}\DIFdelend \DIFaddbegin \DIFadd{OpenUsbKbDxe}\DIFaddend }, this option
...
\item
  \DIFaddbegin \texttt{\DIFadd{DirectGopCacheMode}}\\
...
\item
  \DIFaddend \texttt{DirectGopRendering}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{AppleDebugLog}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Reinstalls Apple Debug Log protocol with a builtin
  version.
}

\item
  \DIFaddend \texttt{AppleEvent}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\item
  ...
  \texttt{OC\_FIRMWARE\_RUNTIME} protocol implemented in \texttt{\DIFdelbegin \DIFdel{FwRuntimeServices}\DIFdelend \DIFaddbegin \DIFadd{OpenRuntime}\DIFaddend .efi}.
...
\item
  ...
  This quirk requires \texttt{OC\_FIRMWARE\_RUNTIME} protocol implemented
  in \texttt{\DIFdelbegin \DIFdel{FwRuntimeServices}\DIFdelend \DIFaddbegin \DIFadd{OpenRuntime}\DIFaddend .efi}. The quirk lets default boot entry
...
\item
  ...
  installing \texttt{\DIFdelbegin \DIFdel{FwRuntimeServices}\DIFdelend \DIFaddbegin \DIFadd{OpenRuntime}\DIFaddend } driver and enabling the quirks listed below.
  ...
  \item \texttt{\DIFdelbegin \DIFdel{ProtectCsmRegion}\DIFdelend \DIFaddbegin \DIFadd{ProtectMemoryRegions}\DIFaddend }
