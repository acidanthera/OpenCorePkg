...
\item
  \DIFaddbegin \texttt{\DIFadd{nvram.fallback}} \\
  \DIFadd{OpenCore variable import fallback file.
}\item
  \texttt{\DIFadd{nvram.used}} \\
  \DIFadd{Renamed previous OpenCore variable import file after switch to fallback file.
}\item
  \DIFaddend \texttt{opencore-YYYY-MM-DD-HHMMSS.txt} \\
  OpenCore log file.
\item
...
\item
  ...
  to have \texttt{kprintf} method working properly. This can be achieved by \DIFdelbegin \DIFdel{setting (i.
  e. }\texttt{\DIFdel{Delete}}%DIFAUXCMD
\DIFdel{, then }\texttt{\DIFdel{Add}}%DIFAUXCMD
\DIFdel{)
  the }\texttt{\DIFdel{class-code}} %DIFAUXCMD
\DIFdel{property of the PCI serial port device to }\texttt{\DIFdel{FFFFFFFF}} %DIFAUXCMD
\DIFdel{in }\texttt{\DIFdel{DeviceProperties}} %DIFAUXCMD
\DIFdel{section.
  As an alternative solution, a codeless kext }\texttt{\DIFdel{PCIeSerialDisable.kext}} %DIFAUXCMD
\DIFdel{shown in the spoiler }\texttt{\DIFdel{PCIeSerialDisable.kext/Contents/Info.plist}} %DIFAUXCMD
\DIFdel{at
  }\href{https://github.com/acidanthera/bugtracker/issues/1954}{\DIFdel{acidanthera/bugtracker\#1954}}%DIFAUXCMD
\DIFdel{, may also be used.
  }\DIFdelend \DIFaddbegin \DIFadd{using }\href{https://github.com/joevt/PCIeSerialDisable}{\DIFadd{PCIeSerialDisable}}\DIFadd{.
  }\DIFaddend In addition, for certain Thunderbolt cards the IOKit personality \texttt{IOPCITunnelCompatible} also needs to be set to \texttt{true},
...
\item
  ...
  \tightlist
    ...
    \DIFaddbegin \item \texttt{\DIFadd{0x80}} \DIFadd{(bit }\texttt{\DIFadd{7}}\DIFadd{) --- In combination with }\texttt{\DIFadd{0x40}}\DIFadd{, enable faster but unsafe (see Warning 2 below) file logging.
  }\DIFaddend \end{itemize}
...
\end{lstlisting}
  ...
  \textbf{Warning \DIFaddbegin \DIFadd{1}\DIFaddend }: Certain firmware appear to have defective NVRAM garbage collection.
  ...
  \DIFaddbegin \textbf{\DIFadd{Warning 2}}\DIFadd{: It is possible to enable fast file logging, which requires a fully compliant
  ...
  \DIFaddend When interpreting the log, note that the lines are prefixed with a tag describing
  ...
  \item \DIFaddbegin \texttt{\DIFadd{OCVAR}} \DIFadd{--- OcVariableLib
  }\item \DIFaddend \texttt{OCXML} --- OcXmlLib
  \end{itemize}
...
\item
  \DIFdelbegin \texttt{\DIFdel{LegacyEnable}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Enables loading a NVRAM variable file named }\texttt{\DIFdel{nvram.plist}}
  %DIFAUXCMD
\DIFdel{from EFI volume root.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{This file must have a root }\texttt{\DIFdel{plist\ dictionary}} %DIFAUXCMD
\DIFdel{type and contain two fields:
  }%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD <   \tightlist
%DIFDELCMD <   \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{Version}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{plist\ integer}}%DIFAUXCMD
\DIFdel{, file version, must be set to 1.
  }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{Add}} %DIFAUXCMD
\DIFdel{--- }\texttt{\DIFdel{plist\ dictionary}}%DIFAUXCMD
\DIFdel{, equivalent to }\texttt{\DIFdel{Add}} %DIFAUXCMD
\DIFdel{from
  }\texttt{\DIFdel{config.plist}}%DIFAUXCMD
\DIFdel{.
  }
...
%DIFDELCMD <   %%%
\DIFdel{Variable loading happens prior to the }\texttt{\DIFdel{Delete}} %DIFAUXCMD
\DIFdel{(and }\texttt{\DIFdel{Add}}%DIFAUXCMD
\DIFdel{) phases. Unless
  }\DIFdelend \texttt{LegacyOverwrite}\DIFdelbegin \DIFdel{is enabled, it will not overwrite any existing variable.
  Variables allowed to be set must be specified in }\texttt{\DIFdel{LegacySchema}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{Third-party scripts may be used to create }\texttt{\DIFdel{nvram.plist}}
  %DIFAUXCMD
\DIFdel{file. An example of such script can be found in
  }\href{https://github.com/acidanthera/OpenCorePkg/tree/master/Utilities/LogoutHook}{\texttt{\DIFdel{Utilities/LogoutHook}}%DIFAUXCMD
}%DIFAUXCMD
\DIFdel{.
  The use of third-party scripts may require }\texttt{\DIFdel{ExposeSensitiveData}} %DIFAUXCMD
\DIFdel{set to }\texttt{\DIFdel{0x3}} %DIFAUXCMD
\DIFdel{to provide
  }\texttt{\DIFdel{boot-path}} %DIFAUXCMD
\DIFdel{variable with the OpenCore EFI partition UUID.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\textbf{\DIFdel{Warning}}%DIFAUXCMD
\DIFdel{: This feature can be dangerous, as it passes unprotected data to
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\texttt{\DIFdel{LegacyOverwrite}}%DIFAUXCMD
\DIFdelend \\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
...
\DIFdelbegin %DIFDELCMD < \begin{longtable}{p{1.3in}p{5.55in}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{longtable}{p{1.65in}p{5.25in}}
...
\DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/OcBinaryData}{%%%
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg}{\DIFaddend \texttt{OpenPartitionDxe}}\textbf{*}
& Partition management driver with Apple Partitioning Scheme support.
  ...
  using Apple Partitioning Scheme\DIFaddbegin \DIFadd{, or for loading other macOS Installers where these were
...
onmost Apple hardware, and possibly some others)may be specified in }%DIFDELCMD < \texttt{%%%
...
\begin{itemize}
  \tightlist
	\item \DIFaddbegin \texttt{\DIFadd{-}{}\DIFadd{-codec-setup-delay}} \DIFadd{- Integer value, default }\texttt{\DIFadd{0}}\DIFadd{. }\medskip

  \DIFadd{Amount of time in milliseconds to wait for all widgets to come fully on, applied per codec
  during driver connection phase. In most systems this should not be needed and a faster boot
  will be achieved by using }\texttt{\DIFadd{Audio}} \DIFadd{section }\texttt{\DIFadd{SetupDelay}} \DIFadd{if any audio setup delay
  is required. Where required, values of up to one second may be needed. }\medskip

	\item \texttt{\DIFadd{-}{}\DIFadd{-force-device}} \DIFadd{- String value, no default. }\medskip
...
\DIFaddbegin \subsection{\DIFadd{OpenVariableRuntimeDxe}}\label{emunvram}

\DIFadd{Provides in-memory emulated NVRAM implementation. This can be useful on systems with fragile
(e.g. MacPro5,1, see discussion linked from this }\href{https://forums.macrumors.com/posts/30945127}{\DIFadd{forum post}}\DIFadd{)
or incompatible NVRAM implementations. This driver is included by default in OpenDuet.
}

\DIFadd{In addition to installing emulated NVRAM, this driver additionally installs an OpenCore compatible
protocol enabling the following:
}

\begin{itemize}
\tightlist
  \item \DIFadd{NVRAM values are loaded from }\texttt{\DIFadd{NVRAM/nvram.plist}} \DIFadd{(or from }\texttt{\DIFadd{NVRAM/nvram.fallback}}
  \DIFadd{if it is present and }\texttt{\DIFadd{NVRAM/nvram.plist}} \DIFadd{is missing) on boot
  }\item \DIFadd{The Reset NVRAM option installed by the }\texttt{\DIFadd{ResetNvramEntry}} \DIFadd{driver removes the above files instead
  of affecting underlying NVRAM
  }\item \texttt{\DIFadd{CTRL+Enter}} \DIFadd{in the OpenCore bootpicker updates or creates }\texttt{\DIFadd{NVRAM/nvram.plist}}
\end{itemize}

\DIFadd{Recommended configuration settings for this driver:
}
...
\DIFadd{Variable loading happens prior to the NVRAM }\texttt{\DIFadd{Delete}} \DIFadd{(and }\texttt{\DIFadd{Add}}\DIFadd{) phases. Unless
}\texttt{\DIFadd{LegacyOverwrite}} \DIFadd{is enabled, it will not overwrite any existing variable.
Variables allowed for loading and for saving with }\texttt{\DIFadd{CTRL+Enter}} \DIFadd{must be specified in }\texttt{\DIFadd{LegacySchema}}\DIFadd{.
}

\DIFadd{In order to allow changes to NVRAM within macOS to be captured and saved,
an additional script must be installed. An example of such script can be found in
}\href{https://github.com/acidanthera/OpenCorePkg/tree/master/Utilities/LogoutHook/Launchd.command}{\texttt{\DIFadd{Utilities/LogoutHook/Launchd.command}}}\DIFadd{.
}

\emph{\DIFadd{Note 1}}\DIFadd{: This driver requires working FAT write support in firmware, and sufficient free
space on the OpenCore EFI partition for up to three saved NVRAM files.
}

\emph{\DIFadd{Note 2}}\DIFadd{: The }\texttt{\DIFadd{nvram.plist}} \DIFadd{(and }\texttt{\DIFadd{nvram.fallback}} \DIFadd{if present) files must have a
root }\texttt{\DIFadd{plist\ dictionary}} \DIFadd{type and contain two fields:
}\begin{itemize}
\tightlist
...
\emph{\DIFadd{Note 3}}\DIFadd{: When setting up legacy NVRAM, it can be convenient to set }\texttt{\DIFadd{<string>*</string>}}
\DIFadd{as the value for the following three GUID keys in }\texttt{\DIFadd{LegacySchema}}\DIFadd{:
}\begin{itemize}
\tightlist
 \item \DIFadd{36C28AB5-6566-4C50-9EBD-CBB920F83843
...
\subsubsection{\DIFadd{Managing macOS updates when using emulated NVRAM}}

\DIFadd{OpenCore combined with }\texttt{\DIFadd{OpenVariableRuntimeDxe}} \DIFadd{will only use a given saved }\texttt{\DIFadd{nvram.plist}} \DIFadd{file once,
if it is used to launch a }\texttt{\DIFadd{macOS Installer}} \DIFadd{boot entry. After that the used settings are moved to }\texttt{\DIFadd{nvram.used}}
\DIFadd{and fallback settings, if any, from }\texttt{\DIFadd{nvram.fallback}} \DIFadd{are used instead. }\texttt{\DIFadd{Launchd.command}} \DIFadd{itself
always copies the previous NVRAM settings to fallback, each time it saves new settings.
}

\DIFadd{This strategy is used to work round the limitation that the }\texttt{\DIFadd{Launchd.command}} \DIFadd{script is not running,
...
\DIFadd{In brief, this fallback strategy allows full or incremental OTA updates of recent macOS, which are started from within
an existing macOS (with the }\texttt{\DIFadd{Launchd.command}} \DIFadd{script installed), to proceed without manual intervention.
}

\DIFadd{However, for full installs, there can be more than one full restart back to the }\texttt{\DIFadd{macOS Installer}} \DIFadd{entry. In this case
the fallback strategy will lose track of the correct startup item (i.e. }\texttt{\DIFadd{macOS Installer}}\DIFadd{) from the second reboot
...
in which case a }\texttt{\DIFadd{macOS Installer}} \DIFadd{entry would be continually recreated in the picker menu, even once it no longer exists).
}

\DIFadd{In both the above two cases it is recommended to use the following settings, to make it easy to manually control which boot
entry is selected during the installer process:
}

\begin{itemize}
\tightlist
 \item \DIFadd{Set }\texttt{\DIFadd{ShowPicker=true}}\DIFadd{.
 }\item \DIFadd{Set }\texttt{\DIFadd{Timeout=0}}\DIFadd{.
 }\item \DIFadd{Set }\texttt{\DIFadd{DisableWatchdog=true}}\DIFadd{.
 }\item \DIFadd{If possible, start from a situation where there are no other pending }\texttt{\DIFadd{macOS Installer}} \DIFadd{entries in the boot menu
 (to avoid potential confusion as to which is relevant).
}\end{itemize}

\DIFadd{The first reboot should correctly select }\texttt{\DIFadd{macOS Installer}}\DIFadd{. For second and subsequent reboots, if a }\texttt{\DIFadd{macOS Installer}}
\DIFadd{entry is still present it should be manually selected (using just Enter, not }\texttt{\DIFadd{CTRL+Enter}}\DIFadd{). Once a }\texttt{\DIFadd{macOS Installer}}
\DIFadd{entry is no longer present, the entry for the new OS will still be automatically selected if it was the previous boot default.
If not, it should be manually selected (at this point, }\texttt{\DIFadd{CTRL+Enter}} \DIFadd{is a good idea as any final remaining installion restarts
will be to this entry).
}

\emph{\DIFadd{Note}}\DIFadd{: When using emulated NVRAM but not installing from within an existing installed macOS (i.e. when installing from
within macOS Recovery, or from an installation USB), please refer to this
}\href{https://applelife.ru/posts/916248}{\DIFadd{forum post}} \DIFadd{(in Russian)
...
\item
  ...
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0}\\
  \textbf{Description}: Audio codec reconfiguration delay in \DIFdelbegin \DIFdel{microseconds}\DIFdelend \DIFaddbegin \DIFadd{milliseconds}\DIFaddend .
