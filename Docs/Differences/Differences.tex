...
%DIF ADD ../Configuration.tex        Mon Dec  2 05:21:21 2019
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.5\DIFdelbegin \DIFdel{.2}\DIFdelend \DIFaddbegin \DIFadd{.3}\DIFaddend )
...
\DIFaddbegin \subsection{\DIFadd{Coding conventions}}\label{configuration-conv}
...
\item
  ...
  board \DIFaddbegin \DIFadd{without additional measures}\DIFaddend . In general this frees from 64 to 256
...
\item
  ...
  \emph{Note}: This \DIFdelbegin \DIFdel{may be used to workaround buggy memory maps on older hardware ,
  and is now considered rare legacy. Do not use this unless you fully understand
  the consequences}\DIFdelend \DIFaddbegin \DIFadd{option is deprecated and will be removed in the newer OpenCore
  releases. There is no known hardware that needs this option, and its entire existence
  appears to be a programmer error. Please report on
  }\href{https://github.com/acidanthera/bugtracker}{Acidanthera Bugtracker} \DIFadd{if you need this option}\DIFaddend .
...
%DIFDELCMD <       ParseDarwinVersion(\kappa,\lambda,\mu)=&(\lfloor\frac{\kappa}{10}\rfloor\cdot10
%DIFDELCMD <       + \kappa-\lfloor\frac{\kappa}{10}\rfloor\cdot10)\cdot10000 &&
%DIFDELCMD <         \text{Where }\kappa\in(0,99)\text{ is kernel version major} \\
%DIFDELCMD <       + &(\lfloor\frac{\lambda}{10}\rfloor\cdot10
%DIFDELCMD <       + \lambda-\lfloor\frac{\lambda}{10}\rfloor\cdot10)\cdot100 &&
%DIFDELCMD <         \text{Where }\lambda\in(0,99)\text{ is kernel version minor} \\
%DIFDELCMD <       + &(\lfloor\frac{\mu}{10}\rfloor\cdot10
%DIFDELCMD <       + \mu-\lfloor\frac{\mu}{10}\rfloor\cdot10) &&
...
\item
  ...
  builtin \texttt{\DIFdelbegin \DIFdel{KeySypport}\DIFdelend \DIFaddbegin \DIFadd{KeySupport}\DIFaddend }, which may work better or worse depending on the firmware.
...
\item
  ...
  \texttt{AppleGenericInput} aka \DIFaddbegin \DIFadd{(}\DIFaddend \texttt{AptioIntputFix}), to fill
...
\item
  \DIFaddbegin \texttt{\DIFadd{ClearScreenOnModeSwitch}}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{RequestBootVarFallback}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Request fallback of some }\texttt{\DIFadd{Boot}} \DIFadd{prefixed variables from
  }\texttt{\DIFadd{OC\_VENDOR\_VARIABLE\_GUID}} \DIFadd{to }\newline \texttt{\DIFadd{EFI\_GLOBAL\_VARIABLE\_GUID}}\DIFadd{.
}

  \DIFadd{This quirk requires }\DIFaddend \texttt{RequestBootVarRouting} \DIFaddbegin \DIFadd{to be enabled and therefore
  }\texttt{\DIFadd{OC\_FIRMWARE\_RUNTIME}} \DIFadd{protocol implemented in }\texttt{\DIFadd{FwRuntimeServices.efi}}\DIFadd{.
}

  \DIFadd{By redirecting }\texttt{\DIFadd{Boot}} \DIFadd{prefixed variables to a separate GUID namespace we achieve
  multiple goals:
  }\begin{itemize}
  \tightlist
  \item \DIFadd{Operating systems are jailed and only controlled by OpenCore boot
  environment to enhance security.
  }\item \DIFadd{Operating systems do not mess with OpenCore boot priority, and guarantee
  fluent updates and hibernation wakes for cases that require reboots with OpenCore
  in the middle.
  }\item \DIFadd{Potentially incompatible boot entries, such as macOS entries, are not deleted
  or anyhow corrupted.
  }\end{itemize}

  \DIFadd{However, some firmwares do their own boot option scanning upon startup by checking
  ...
  due to }\texttt{\DIFadd{BootOrder}} \DIFadd{entry duplication (each option will be added twice) making
  it impossible to boot without cleaning NVRAM.
}

  \DIFadd{To trigger the bug one should have some valid boot options (e.g. OpenCore) and then
  install Windows with }\texttt{\DIFadd{RequestBootVarRouting}} \DIFadd{enabled. As Windows bootloader
  ...
  \DIFadd{This quirk forwards all UEFI specification valid boot options, that are not related to
  macOS, to the firmware into }\texttt{\DIFadd{BootF\#\#\#}} \DIFadd{and }\texttt{\DIFadd{BootOrder}} \DIFadd{variables upon
  write. As the entries are added to the end of }\texttt{\DIFadd{BootOrder}}\DIFadd{, this does not
...
\item
  \texttt{\DIFadd{RequestBootVarRouting}}\DIFaddend \\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Request redirect \DIFaddbegin \DIFadd{of all }\DIFaddend \texttt{Boot} prefixed variables from
  \texttt{EFI\_GLOBAL\_VARIABLE\_GUID} to \DIFaddbegin \newline \DIFaddend \texttt{OC\_VENDOR\_VARIABLE\_GUID}.
...
\item
  \texttt{\DIFdelbegin \DIFdel{ClearScreenOnModeSwitch}\DIFdelend \DIFaddbegin \DIFadd{UnblockFsConnect}\DIFaddend }\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  ...
  \emph{Note}: \DIFdelbegin \texttt{\DIFdel{ConsoleControl}} %DIFAUXCMD
...
\item
  ...
  For machine code (x86 code) it is not possible to do \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{differently sized }\DIFaddend replacements due to
