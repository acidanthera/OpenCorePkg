...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (0.6\DIFdelbegin \DIFdel{.5}\DIFdelend \DIFaddbegin \DIFadd{.6}\DIFaddend )
...
%DIFDELCMD <         child [missing] {}
%DIFDELCMD <         child [missing] {}
%DIFDELCMD <         child { node {Bootstrap}
%DIFDELCMD <           child { node [selected] {Bootstrap.efi}}
%DIFDELCMD <         }
%DIFDELCMD <         child [missing] {}
...
\tightlist
\item
  \texttt{BOOTx64.efi} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend \texttt{\DIFdelbegin \DIFdel{Bootstrap}\DIFdelend \DIFaddbegin \DIFadd{BOOTIa32}\DIFaddend .efi} \\
  Initial bootstrap loaders, which \DIFdelbegin \DIFdel{loads }\DIFdelend \DIFaddbegin \DIFadd{load }\DIFaddend \texttt{OpenCore.efi}\DIFdelbegin \DIFdel{unless it was
  already started as a driver}\DIFdelend . \texttt{BOOTx64.efi}
  is loaded by the firmware by default according to UEFI specification, \DIFdelbegin \DIFdel{and }\texttt{\DIFdel{Bootstrap.efi}} %DIFAUXCMD
\DIFdel{can
  be registered as a custom option }\DIFdelend \DIFaddbegin \DIFadd{yet it can
  also be renamed and put to a custom location }\DIFaddend to let OpenCore coexist with operating
  systems using \texttt{BOOTx64.efi} as their own loaders (e.g. Windows), see
  \texttt{\DIFdelbegin \DIFdel{BootProtect}\DIFdelend \DIFaddbegin \DIFadd{LauncherOption}\DIFaddend } for more details.
\item
  \texttt{boot} \\
...
\item
  \texttt{OpenCore.efi} \\
  Main booter \DIFdelbegin \DIFdel{driver }\DIFdelend \DIFaddbegin \DIFadd{application }\DIFaddend responsible for operating system loading. The directory
  ...
  \DIFdelbegin \texttt{\DIFdel{Bootstrap.efi}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{a custom launcher}\DIFaddend , other directories containing \texttt{OpenCore.efi}
...
custom BDS). Without \texttt{\DIFdelbegin \DIFdel{BootProtect}\DIFdelend \DIFaddbegin \DIFadd{LauncherOption}\DIFaddend } it also is possible that other operating systems
...
\item
  \DIFaddbegin \texttt{\DIFadd{LauncherOption}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Disabled}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Register launcher option in firmware preferences for persistence.
}
  ...
  \begin{itemize}
  \tightlist
  \item \texttt{\DIFadd{Disabled}} \DIFadd{--- do nothing.
  }\item \texttt{\DIFadd{Full}} \DIFadd{--- create or update top-priority
  ...
  }\item \texttt{\DIFadd{Short}} \DIFadd{--- create a short boot option instead of a complete one.
  ...
  }\texttt{\DIFadd{\textbackslash EFI\textbackslash BOOT\textbackslash BOOTx64.efi}}
  \DIFadd{file. By creating a custom option in this file path becomes no longer
  used for bootstrapping OpenCore. The path used for bootstrapping is specified
  in }\texttt{\DIFadd{LauncherPath}} \DIFadd{option.
}
  ...
  \emph{\DIFadd{Note 2}}\DIFadd{: Be aware that while NVRAM reset executed from OpenCore should not erase the boot
  option created in }\texttt{\DIFadd{Bootstrap}}\DIFadd{, executing NVRAM reset prior to loading OpenCore will remove it. For significant implementation updates (e.g. in OpenCore 0.6.4) make sure to perform
  NVRAM reset with }\texttt{\DIFadd{Bootstrap}} \DIFadd{disabled before reenabling.
}

\item
  \texttt{\DIFadd{LauncherPath}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ string}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{Default}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Launch path for }\texttt{\DIFadd{LauncherOption}}\DIFadd{.
}

  \texttt{\DIFadd{Default}} \DIFadd{stays for launched }\texttt{\DIFadd{OpenCore.efi}}\DIFadd{,
  any other path, e.g. }\texttt{\DIFadd{\textbackslash EFI\textbackslash Launcher.efi}}\DIFadd{,
  can be used to provide custom loaders, which are supposed to
  load }\texttt{\DIFadd{OpenCore.efi}} \DIFadd{themselves.
}

\item
  \DIFaddend \texttt{PickerAttributes}\\
  \textbf{Type}: \texttt{plist\ integer}\\
  \textbf{Failsafe}: \texttt{0}\\
...
\item
  ...
  unless it is already present. The directory will contain ACPI\DIFdelbegin \DIFdel{and SMBIOSdumps.
}\DIFdelend \DIFaddbegin \DIFadd{, SMBIOS, and audio codec dumps.
...
\end{lstlisting}
  ...
  \item \texttt{OC} --- OpenCore main\DIFaddbegin \DIFadd{, also OcMainLib
  }\DIFaddend \item \texttt{VMOPT} --- VerifyMemOpt
  \end{itemize}
  ...
  \item \DIFdelbegin \texttt{\DIFdel{OCAV}} %DIFAUXCMD
\DIFdel{--- OcAppleImageVerificationLib
  }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\DIFdelend \texttt{OCA} ---- OcAcpiLib
  ...
  \item \DIFaddbegin \texttt{\DIFadd{OCPE}} \DIFadd{--- OcPeCoffLib, OcPeCoffExtLib
  }\item \DIFaddend \texttt{OCPI} --- OcFileLib, partition info
...
\item \DIFdelbegin \texttt{\DIFdel{BootProtect}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Attempt to provide bootloader persistence.
}%DIFDELCMD < 
...
%DIFDELCMD <   \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{None}} %DIFAUXCMD
\DIFdel{--- do nothing.
  }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{Bootstrap}} %DIFAUXCMD
\DIFdel{--- create or update top-priority
  }\texttt{\DIFdel{\textbackslash EFI\textbackslash OC\textbackslash Bootstrap\textbackslash Bootstrap.efi}}
  %DIFAUXCMD
\DIFdel{boot option in UEFI variable storage at bootloader startup. For this option
  to work }\texttt{\DIFdel{RequestBootVarRouting}} %DIFAUXCMD
\DIFdel{is required to be enabled.
  }%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{BootstrapShort}} %DIFAUXCMD
\DIFdel{--- create a short boot option instead of a complete one,
  otherwise equivalent to }\texttt{\DIFdel{Bootstrap}}%DIFAUXCMD
\DIFdel{. This variant is useful for some older firmwares,
...
%DIFDELCMD <   %%%
\DIFdel{This option provides integration with third-party operating system installation and upgrade
  at the times they overwrite }\texttt{\DIFdel{\textbackslash EFI\textbackslash BOOT\textbackslash BOOTx64.efi}}
  %DIFAUXCMD
\DIFdel{file. By creating a custom option in }\texttt{\DIFdel{Bootstrap}} %DIFAUXCMD
\DIFdel{mode this file path becomes no longer
...
\DIFdel{: Be aware that while NVRAM reset executed from OpenCore should not erase the boot
  option created in }\texttt{\DIFdel{Bootstrap}}%DIFAUXCMD
\DIFdel{, executing NVRAM reset prior to loading OpenCore will remove it. For significant implementation updates (e.g. in OpenCore 0.6.4) make sure to perform
  NVRAM reset with }\texttt{\DIFdel{Bootstrap}} %DIFAUXCMD
\DIFdel{disabled before reenabling.
}%DIFDELCMD < 

%DIFDELCMD < \item %%%
\item %DIFAUXCMD
\DIFdelend \label{securedmgloading}
  \texttt{DmgLoading}\\
  \textbf{Type}: \texttt{plist\ string}\\
...
\item
  \DIFaddbegin \texttt{\DIFadd{UseRawUuidEncoding}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Failsafe}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Use raw encoding for SMBIOS UUIDs.
}

  \DIFadd{Each UUID }\texttt{\DIFadd{AABBCCDD-EEFF-GGHH-IIJJ-KKLLMMNNOOPP}} \DIFadd{is
  ...
  \begin{itemize}
    ...
    \item \texttt{\DIFadd{Big Endian}} \DIFadd{--- by writing all the bytes as they are without making
      ...
      (}\texttt{\DIFadd{\{DD CC BB AA FF EE HH GG II JJ KK LL MM NN OO PP\}}}\DIFadd{).
  }\end{itemize}

  \DIFadd{SMBIOS specification did not explicitly specify the encoding format for the
  UUID up to SMBIOS 2.6, where it stated that }\texttt{\DIFadd{Little Endian}} \DIFadd{encoding
  ...
  \DIFadd{OpenCore always sets a recent SMBIOS version (currently 3.2) when generating
  the modified DMI tables. If }\texttt{\DIFadd{UseRawUuidEncoding}} \DIFadd{is enabled, then }\texttt{\DIFadd{Big Endian}}
  \DIFadd{format is used to store the }\texttt{\DIFadd{SystemUUID}} \DIFadd{data. Otherwise
  }\texttt{\DIFadd{Little Endian}} \DIFadd{is used.
}

  \emph{\DIFadd{Note}}\DIFadd{: Since UUIDs used in DataHub and NVRAM are not standardised
  and are added by Apple, this preference does not affect them. Unlike
  SMBIOS they are always stored in the }\texttt{\DIFadd{Big Endian}} \DIFadd{format.
}

\item
  \DIFaddend \texttt{Generic}\\
  \textbf{Type}: \texttt{plist\ dictionary}\\
  \textbf{Description}: Update all fields \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in }\texttt{\DIFadd{Automatic}} \DIFadd{mode.
}

  \emph{\DIFadd{Note}}\DIFadd{: }\DIFaddend This section is \DIFdelbegin \DIFdel{read only
  }\DIFdelend \DIFaddbegin \DIFadd{ignored but may not be removed }\DIFaddend when
  \texttt{Automatic} is \DIFdelbegin \DIFdel{active}\DIFdelend \DIFaddbegin \texttt{\DIFadd{false}}\DIFaddend .
\item
  \texttt{DataHub}\\
  \textbf{Type}: \texttt{plist\ dictionary}\\
...
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdelend \textbf{Description}: Update Data Hub fields \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in non-}\texttt{\DIFadd{Automatic}} \DIFadd{mode.
}

  \emph{\DIFadd{Note}}\DIFadd{: }\DIFaddend This section is \DIFdelbegin \DIFdel{read
  only }\DIFdelend \DIFaddbegin \DIFadd{ignored and may be removed }\DIFaddend when
  \texttt{Automatic} is \DIFdelbegin \DIFdel{not active}\DIFdelend \DIFaddbegin \texttt{\DIFadd{true}}\DIFaddend .
\item
  \texttt{Memory}\\
  \textbf{Type}: \texttt{plist\ dictionary}\\
  \DIFdelbegin \textbf{\DIFdel{Optional}}%DIFAUXCMD
\DIFdel{: When }\texttt{\DIFdel{CustomMemory}} %DIFAUXCMD
\DIFdel{is }\texttt{\DIFdel{false}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdelend \textbf{Description}: Define custom memory configuration.
\DIFaddbegin 
...
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdelend \textbf{Description}: Update platform NVRAM fields \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in non-}\texttt{\DIFadd{Automatic}} \DIFadd{mode.
}

  \emph{\DIFadd{Note}}\DIFadd{: }\DIFaddend This section is \DIFdelbegin \DIFdel{read only }\DIFdelend \DIFaddbegin \DIFadd{ignored and may be removed }\DIFaddend when
  \texttt{Automatic} is \DIFdelbegin \DIFdel{not active}\DIFdelend \DIFaddbegin \texttt{\DIFadd{true}}\DIFaddend .
\item
  \texttt{SMBIOS}\\
  \textbf{Type}: \texttt{plist\ dictionary}\\
...
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdelend \textbf{Description}: Update SMBIOS fields \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in non-}\texttt{\DIFadd{Automatic}} \DIFadd{mode.
}

  \emph{\DIFadd{Note}}\DIFadd{: }\DIFaddend This section is \DIFdelbegin \DIFdel{read only
  }\DIFdelend \DIFaddbegin \DIFadd{ignored and may be removed }\DIFaddend when
  \texttt{Automatic} is \DIFdelbegin \DIFdel{not active}\DIFdelend \DIFaddbegin \texttt{\DIFadd{true}}\DIFaddend .
\end{enumerate}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \DIFdelbegin \texttt{\DIFdel{MacPro6,1}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{OEM specified or not installed}\DIFaddend \\
  \textbf{Description}: Refer to SMBIOS \texttt{SystemProductName}.
\item
  \texttt{SystemSerialNumber}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \DIFdelbegin \texttt{\DIFdel{OPENCORE\_SN1}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{OEM specified or not installed}\DIFaddend \\
  \textbf{Description}: Refer to SMBIOS \texttt{SystemSerialNumber}.
\item
  \texttt{SystemUUID}\\
  \textbf{Type}: \texttt{plist\ string}, GUID\\
  \textbf{Failsafe}: OEM specified \DIFaddbegin \DIFadd{or not installed}\DIFaddend \\
  \textbf{Description}: Refer to SMBIOS \texttt{SystemUUID}.
\item
  \texttt{MLB}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \DIFdelbegin \texttt{\DIFdel{OPENCORE\_MLB\_SN11}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{OEM specified or not installed}\DIFaddend \\
  \textbf{Description}: Refer to SMBIOS \texttt{BoardSerialNumber}.
\item
  \texttt{ROM}\\
  \textbf{Type}: \texttt{plist\ data}, 6 bytes\\
  \textbf{Failsafe}: \DIFdelbegin \DIFdel{all zero}\DIFdelend \DIFaddbegin \DIFadd{OEM specified or not installed}\DIFaddend \\
...
%DIFAUXCMD
%DIFDELCMD < & %%%
\DIFdel{Parse and dump High Definition Audio codec information (requires }\texttt{\DIFdel{AudioDxe}}%DIFAUXCMD
\DIFdel{). }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \href{https://github.com/acidanthera/OpenCorePkg}{\texttt{KeyTester}}\textbf{*}
...
\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: In the following all dimensions are normative for the 1x scaling level and shall be
scaled accordingly for other levels.
}

\DIFaddend \begin{itemize}
\tightlist
  \item \texttt{Cursor} --- Mouse cursor (mandatory\DIFaddbegin \DIFadd{, up to 144x144}\DIFaddend ).
  \item \texttt{Selected} --- Selected item (mandatory\DIFaddbegin \DIFadd{, 144x144}\DIFaddend ).
  \item \texttt{Selector} --- Selecting item (mandatory\DIFaddbegin \DIFadd{, up to 144x40}\DIFaddend ).
  \item \DIFaddbegin \texttt{\DIFadd{Left}} \DIFadd{--- Scrolling left (mandatory, 40x40).
  }\item \texttt{\DIFadd{Right}} \DIFadd{--- Scrolling right (mandatory, 40x40).
  }\item \DIFaddend \texttt{HardDrive} --- Generic OS (mandatory\DIFaddbegin \DIFadd{, 128x128}\DIFaddend ).
  \item \DIFaddbegin \texttt{\DIFadd{Background}} \DIFadd{--- Centred background image.
  }\item \DIFaddend \texttt{Apple} --- Apple OS \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
  \item \texttt{AppleRecv} --- Apple Recovery OS \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
  \item \texttt{AppleTM} --- Apple Time Machine \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
  \item \texttt{Windows} --- Windows \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
  \item \texttt{Other} --- Custom entry (see \texttt{Entries}\DIFaddbegin \DIFadd{, 128x128}\DIFaddend ).
  \item \texttt{ResetNVRAM} --- Reset NVRAM system action or tool \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
  \item \texttt{Shell} --- Entry with UEFI Shell name \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend e.g. \texttt{OpenShell} \DIFaddbegin \DIFadd{(128x128}\DIFaddend ).
  \item \texttt{Tool} --- Any other tool \DIFaddbegin \DIFadd{(128x128)}\DIFaddend .
\end{itemize}
...
\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: All labels must have a height of exactly 12 px. There is no limit for their width.
}

\DIFaddend Label and icon generation can be performed with bundled utilities: \texttt{disklabel} and
\texttt{icnspack}. \DIFdelbegin \DIFdel{Please refer to sample data for the details about the dimensions.
}\DIFdelend Font is Helvetica 12 pt times scale factor.
...
\item
  \texttt{PlayChime}\\
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Failsafe}: \DIFdelbegin \DIFdel{empty string}\DIFdelend \DIFaddbegin \texttt{\DIFadd{Auto}}\DIFaddend \\
...
\item
  ...
  \DIFaddbegin \DIFadd{The value of this property cannot be empty if }\texttt{\DIFadd{PointerSupport}} \DIFadd{is enabled.
}\DIFaddend 
...
\item Sign all OpenCore binaries (\texttt{BOOTX64.efi}, \texttt{BOOTIa32.efi},
  \DIFdelbegin \texttt{\DIFdel{Bootstrap.efi}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \texttt{OpenCore.efi}\DIFaddbegin \DIFadd{, custom launchers}\DIFaddend ) used on this system with
