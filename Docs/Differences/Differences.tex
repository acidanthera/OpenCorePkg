...
%DIF ADD ../Configuration.tex        Wed Jul 29 01:17:13 2020
...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (\DIFdelbegin \DIFdel{0.5.9}\DIFdelend \DIFaddbegin \DIFadd{0.6.0}\DIFaddend )
...
due to area relocation. If you need such changes you may \DIFdelbegin \DIFdel{utilities }\DIFdelend \DIFaddbegin \DIFadd{utilise }\DIFaddend ``proxy''
...
\item
  ...
  \DIFaddbegin \DIFadd{Refer to }\texttt{\DIFadd{OCABC: MAT support is 1/0}} \DIFadd{log entry to determine whether MAT is supported.
}\DIFaddend 
...
\item
  \DIFaddbegin \texttt{\DIFadd{ProvideMaxSlide}}\\
  ...
  \emph{\DIFadd{Note}}\DIFadd{: The necessity of this quirk is determined by random boot failure
  when }\texttt{\DIFadd{ProvideCustomSlide}} \DIFadd{is enabled and the randomized slide fall
  into the unavailable range. When }\texttt{\DIFadd{AppleDebug}} \DIFadd{is enabled, usually the
  debug log may contain messages like }\texttt{\DIFadd{AAPL: }[\DIFadd{EB|`LD:LKC}] \DIFadd{\} Err(0x9)}}\DIFadd{.
  To find the optimal value, manually append }\texttt{\DIFadd{slide=X}} \DIFadd{to }\texttt{\DIFadd{boot-args}}
  \DIFadd{and log the largest one that won't cause boot failure.
}

\item
  \DIFaddend \texttt{RebuildAppleMemoryMap}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  ...
  memory attributes table (MAT). \DIFaddbegin \DIFadd{This quirk is generally unnecessary when using
  }\texttt{\DIFadd{OpenDuetPkg}}\DIFadd{, but may be required to boot macOS 10.6 and earlier for
  unclear reasons.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Device propertties are part of the }\texttt{\DIFadd{IODeviceTree}} \DIFadd{(}\texttt{\DIFadd{gIODT}}\DIFadd{)
...
stage is performed by the XNU kernel in the }\texttt{\DIFadd{IODeviceTreeAlloc}}
\DIFadd{method, the majority of the construction is performed by the platform expert,
implemented in }\texttt{\DIFadd{AppleACPIPlatformExpert.kext}}\DIFadd{.
}

\DIFadd{AppleACPIPlatformExpert incorporates two stages of }\texttt{\DIFadd{IODeviceTree}}
\DIFadd{construction implemented by calling }\\
\texttt{\DIFadd{AppleACPIPlatformExpert::mergeDeviceProperties}}\DIFadd{:
}

\begin{enumerate}
  \tightlist
  \item \DIFadd{During ACPI table initialisation through the recursive ACPI namespace scanning
  by the calls to }\\
  \texttt{\DIFadd{AppleACPIPlatformExpert::createDTNubs}}\DIFadd{.
  }\item \DIFadd{During IOService registration (}\texttt{\DIFadd{IOServices::registerService}}\DIFadd{) callbacks
  implemented as a part of }\\
  \texttt{\DIFadd{AppleACPIPlatformExpert::platformAdjustService}}
  \DIFadd{function and its private worker method }\\
  \texttt{\DIFadd{AppleACPIPlatformExpert::platformAdjustPCIDevice}}
  \DIFadd{specific to the PCI devices.
}\end{enumerate}

\DIFadd{The application of the stages depends on the device presence in ACPI tables.
...
\DIFadd{For all kernel drivers, which may inspect the }\texttt{\DIFadd{IODeviceTree}} \DIFadd{plane without
probing (e.g. }\texttt{\DIFadd{Lilu}} \DIFadd{and its plugins like }\texttt{\DIFadd{WhateverGreen}}\DIFadd{) it is particularly
important to ensure device presence in the ACPI tables. Failing to do so may result
}\textbf{\DIFadd{in all kinds of erratic behaviour}} \DIFadd{caused by ignoring the injected device properties
as they were not constructed at the first stage. See }\texttt{\DIFadd{SSDT-IMEI.dsl}} \DIFadd{and
}\texttt{\DIFadd{SSDT-BRG0.dsl}} \DIFadd{for an example.
}

\DIFaddend \subsection{Properties}\label{devpropsprops}
...
\item
  ...
  \DIFaddbegin \DIFadd{To track the dependency order one can inspect the }\texttt{\DIFadd{OSBundleLibraries}} \DIFadd{key
...
\item
  ...
  \item \texttt{0x0001} --- \texttt{OC\_ATTR\_USE\_VOLUME\_ICON}, provides custom icons
    ...
    \DIFadd{For }\texttt{\DIFadd{Tools}} \DIFadd{OpenCore will try to load a custom icon and fallback to the default icon:
    }\DIFaddend \begin{itemize}
    \tightlist
      \item \DIFaddbegin \texttt{\DIFadd{ResetNVRAM}} \DIFadd{--- }\texttt{\DIFadd{Resources\textbackslash Image\textbackslash ResetNVRAM.icns}}
        \DIFadd{--- }\texttt{\DIFadd{ResetNVRAM.icns}} \DIFadd{from icons directory.
      }\item \texttt{\DIFadd{Tools\textbackslash <TOOL\_RELATIVE\_PATH>.icns}}
        \DIFadd{--- icon near the tool file with appended }\texttt{\DIFadd{.icns}} \DIFadd{extension.
    }\end{itemize} \medskip

    \DIFadd{For custom boot }\texttt{\DIFadd{Entries}} \DIFadd{OpenCore will try to load a custom icon and fallback
    to the volume icon or the default icon:
    }\begin{itemize}
    \tightlist
      \item \texttt{\DIFadd{<ENTRY\_PATH>.icns}} \DIFadd{--- icon near the entry file with appended }\texttt{\DIFadd{.icns}} \DIFadd{extension.
    }\end{itemize} \medskip

    \DIFadd{For all other entries OpenCore will try to load a volume icon and fallback to the default icon:
    }\begin{itemize}
    \tightlist
      \item \DIFaddend \texttt{.VolumeIcon.icns} file at \texttt{Preboot} root for APFS.
      \item \texttt{.VolumeIcon.icns} file at volume root for other filesystems.
    \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\texttt{\DIFdel{<TOOL\_NAME>.icns}} %DIFAUXCMD
\DIFdel{file for }\texttt{\DIFdel{Tools}}%DIFAUXCMD
\DIFdel{.
    }\DIFdelend \end{itemize} \DIFaddbegin \medskip

    \DIFaddend Volume icons can be set in Finder. Note, that enabling this may result in
  ...
  \item \texttt{0x0002} --- \texttt{OC\_ATTR\_USE\_DISK\_LABEL\_FILE}, provides custom
    ...
    \tightlist
      ...
      \item \texttt{<TOOL\_NAME\DIFaddbegin \DIFadd{>}\DIFaddend .lbl} (\texttt{<TOOL\_NAME\DIFaddbegin \DIFadd{>}\DIFaddend .l2x}) file near tool for \texttt{Tools}.
    \end{itemize}
...
\item
  ...
  still be found in \DIFaddbegin \\
  \DIFaddend \texttt{/Library/Logs/DiagnosticReports} directory.
...
\end{lstlisting}
  ...
  \texttt{DisableWatchDog} is set to \texttt{true} when you use a slow drive. \DIFaddbegin \DIFadd{Try to
...
\href{https://github.com/acidanthera/OcBinaryData}{OcBinaryData repository}. \DIFaddbegin \DIFadd{You can find customised icons over the internet 
(e.g. }\href{https://github.com/blackosx/OpenCanopyIcons}{here} \DIFadd{or }\href{https://applelife.ru/threads/kastomizacija-opencanopy.2945020/}{there}\DIFadd{).
}\DIFaddend 
...
\item
  ...
  \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Protocol/OcAudio.h}{\texttt{OC\_VOICE\_OVER\_AUDIO\_FILE} definition}%%%
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Acidanthera/Protocol/OcAudio.h}{\texttt{OC\_VOICE\_OVER\_AUDIO\_FILE} definition}\DIFaddend .
...
\item
  ...
  \begin{itemize}
    ...
    and \DIFdelbegin %DIFDELCMD < \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Library/OcApfsLib.h}{\texttt{OcApfsLib}}%%%
\DIFdelend \DIFaddbegin \href{https://github.com/acidanthera/OpenCorePkg/blob/master/Include/Acidanthera/Library/OcApfsLib.h}{\texttt{OcApfsLib}}\DIFaddend .
  \end{itemize}
...
\item
  ...
  \texttt{EnableWriteUnprotector}, \texttt{ProtectMemoryRegions}\DIFdelbegin \DIFdel{,
  }\texttt{\DIFdel{RebuildAppleMemoryMap}}%DIFAUXCMD
\DIFdelend ,
