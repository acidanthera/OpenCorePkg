...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual (1.0\DIFdelbegin \DIFdel{.3}\DIFdelend \DIFaddbegin \DIFadd{.4}\DIFaddend )
...
\item
  ...
  Things to keep in mind:
    ...
    \tightlist
      ...
      \item Where both apply \texttt{.VolumeIcon.icns} takes precedence over \texttt{.contentFlavour}.
...
\end{lstlisting}
  ...
  \item \texttt{\DIFdelbegin \DIFdel{NTBT}\DIFdelend \DIFaddbegin \DIFadd{NETB}\DIFaddend } --- OpenNetworkBoot
...
& \DIFdelbegin %DIFDELCMD < \hyperref[uefipxe]{OpenCore plugin} %%%
\DIFdelend \DIFaddbegin \hyperref[uefinetboot]{OpenCore plugin} \DIFaddend implementing \texttt{OC\_BOOT\_ENTRY\_PROTOCOL}
...
\subsection{OpenNetworkBoot}\DIFdelbegin %DIFDELCMD < \label{uefipxe}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{uefinetboot}
\DIFaddend 
...
The \DIFdelbegin \DIFdel{following }\DIFdelend \DIFaddbegin \DIFadd{below }\DIFaddend configuration options may be specified in the \texttt{Arguments} section for this driver\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

\DIFaddbegin \emph{\DIFadd{Note}}\DIFadd{: There is no problem if configuration options within }\texttt{\DIFadd{<Arguments>...</Arguments>}} \DIFadd{are given on
multiple lines, and option values enclosed within quotes can also span multiple lines. This applies to all drivers
which use OpenCore argument parsing, and can be particularly convenient when multiple long options such as }\texttt{\DIFadd{uri}}\DIFadd{,
}\texttt{\DIFadd{static4}} \DIFadd{and particularly }\texttt{\DIFadd{enroll-cert}} \DIFadd{may be needed.  
}

\DIFaddend \begin{itemize}
  \item \DIFdelbegin \texttt{\DIFdel{-4}} %DIFAUXCMD
\DIFdel{- Boolean flag, enabled if present.
}%DIFDELCMD < \medskip
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{If specified enable IPv4 for PXE and HTTP(S) Boot. Disable IPV6
  unless the }\texttt{\DIFdel{-6}} %DIFAUXCMD
\DIFdel{flag is also present. If neither flag is
  present, both are enabled by default. }%DIFDELCMD < \medskip
%DIFDELCMD < 

%DIFDELCMD <   \item %%%
\item %DIFAUXCMD
\texttt{\DIFdel{-6}} %DIFAUXCMD
\DIFdel{- Boolean flag, enabled if present. }%DIFDELCMD < \medskip
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{If specified enable IPv6 for PXE and HTTP(S) Boot. Disable IPV4
  unless the }\texttt{\DIFdel{-4}} %DIFAUXCMD
\DIFdel{flag is also present. If neither flag is
  present, both are enabled by default.  
}%DIFDELCMD < \medskip
%DIFDELCMD < 

%DIFDELCMD <   \item %%%
\item %DIFAUXCMD
\DIFdelend \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-aux}\DIFdelend \DIFaddbegin \DIFadd{aux}\DIFaddend } - Boolean flag, enabled if present. \medskip

  If specified the driver will generate auxiliary boot entries. \medskip

  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-all-certs}\DIFdelend \DIFaddbegin \DIFadd{delete-all-certs}\DIFaddend [:\{OWNER\_GUID\}]} - Default: not set. \medskip
  ...
  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-cert}\DIFdelend \DIFaddbegin \DIFadd{delete-cert}\DIFaddend [:\{OWNER\_GUID\}]="\{cert-text\}"} - Default: not set. \medskip
  ...
  \DIFdelbegin \texttt{\DIFdel{OWNER\_GUID}} %DIFAUXCMD
\DIFdel{is optional, and will default to all zeros if not specified.
  }\DIFdelend A single PEM file can contain one or more certicates.
  Multiple instances of this option can be used to delete multiple different
  PEM files, if required.
  \DIFaddbegin \texttt{\DIFadd{OWNER\_GUID}} \DIFadd{is optional, and will default to all zeros if not specified.
  }\medskip
\DIFaddend 

  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend [:\{OWNER\_GUID\}]="\{cert-text\}"} - Default: not set. \medskip
...
\DIFdel{is optional, and will default to all zeros if not specified.
  ...
  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-http}\DIFdelend \DIFaddbegin \DIFadd{http}\DIFaddend } - Boolean flag, enabled if present. \medskip

  If specified enable HTTP(S) Boot. Disable PXE Boot unless
  the \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-pxe}\DIFdelend \DIFaddbegin \DIFadd{pxe}\DIFaddend } flag is also present. If neither flag is
  present, both are enabled by default. \medskip

  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-https}\DIFdelend \DIFaddbegin \DIFadd{https}\DIFaddend } - Boolean flag, enabled if present. \medskip

  If enabled, allow only \texttt{https://} URIs for HTTP(S) Boot.
  Additionally has the same behaviour as the \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-http}\DIFdelend \DIFaddbegin \DIFadd{http}\DIFaddend } flag. \medskip

  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-pxe}\DIFdelend \DIFaddbegin \DIFadd{ipv4}\DIFaddend } - Boolean flag, enabled if present. \medskip

  If specified enable \DIFdelbegin \DIFdel{PXE Boot, and }\DIFdelend \DIFaddbegin \DIFadd{IPv4 for PXE and HTTP(S) Boot. Disable IPV6
  unless the }\texttt{\DIFadd{ipv6}} \DIFadd{flag is also present. If neither flag is
  present, both are enabled by default. }\medskip

  \item \texttt{\DIFadd{ipv6}} \DIFadd{- Boolean flag, enabled if present. }\medskip

  \DIFadd{If specified enable IPv6 for PXE and HTTP(S) Boot. Disable IPV4
  unless the }\texttt{\DIFadd{ipv4}} \DIFadd{flag is also present. If neither flag is
  present, both are enabled by default. }\medskip

  \item \texttt{\DIFadd{pxe}} \DIFadd{- Boolean flag, enabled if present. }\medskip

  \DIFadd{If specified enable PXE Boot, and }\DIFaddend disable HTTP(S) Boot unless
  the \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-http}\DIFdelend \DIFaddbegin \DIFadd{http}\DIFaddend } or \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-https}\DIFdelend \DIFaddbegin \DIFadd{https}\DIFaddend } flags are present.
  If none of these flags are present, both PXE and HTTP(S) Boot are
  enabled by default. \medskip

  \item \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-uri}\DIFdelend \DIFaddbegin \DIFadd{static4:\{MAC\_ADDR\}[\textbackslash VLAN\_ID][="\{IP\},\{MASK\},\{GATEWAY\}[,\{DNS\}]"]}\DIFaddend } - String value\DIFdelbegin \DIFdel{, no default}\DIFdelend . \medskip

  \DIFaddbegin \DIFadd{Specify static IPv4 address for the network interface with the MAC address given by }\texttt{\DIFadd{MAC\_ADDR}}\DIFadd{. }\texttt{\DIFadd{MAC\_ADDR}}
  \DIFadd{must be specified as 12 consecutive hex digits, with no spaces, colons or hyphens separating digit pairs. In some advanced
  ...
  \DIFadd{Required elements in value are IP address in }\texttt{\DIFadd{IP}}\DIFadd{, network mask in }\texttt{\DIFadd{MASK}}
  \DIFadd{and gateway in }\texttt{\DIFadd{GATEWAY}}\DIFadd{. Optional is an additional space separated list of one or more DNS servers
  in }\texttt{\DIFadd{DNS}}\DIFadd{. }\texttt{\DIFadd{DNS}} \DIFadd{will be needed if the boot file URI includes a domain name rather than an IP address.
  }

  \texttt{\DIFadd{MAC\_ADDR}} \DIFadd{is not optional.
}

  \DIFadd{If value is omitted, then any static IP for this MAC address (and VLAN ID when present) will be deleted.
}

  \begin{itemize}
  \item \DIFadd{Example 1: }\texttt{\DIFadd{static4:112233445566="192.168.1.20,255.255.255.0,192.168.1.1,8.8.8.8 4.4.4.4"}}\DIFadd{.
  }\item \DIFadd{Example 2: }\texttt{\DIFadd{static4:112233445566\textbackslash 0001="10.0.0.2,255.255.255.0,10.0.0.1"}}\DIFadd{.
  }\end{itemize}

  \emph{\DIFadd{Note 1}}\DIFadd{: This option is written to NVRAM and will remain present even if the option is removed from the driver
  }\texttt{\DIFadd{Arguments}}\DIFadd{, unless NVRAM is cleared or an alternative value is written or the value deleted, using this option.
  }

  \emph{\DIFadd{Note 2}}\DIFadd{: This setting will normally cause a static IP to be assigned during pre-boot, even in vendor-provided
  network stacks. However, due to a quirk of the design of PXE and HTTP boot, any such static assignment will then be
  ignored and DHCP used instead, during network boot. The OpenCore network stack (specifically }\texttt{\DIFadd{HttpBootDxe.efi}}\DIFadd{)
  is unusual in that it will allow HTTP boot from a static IP address, as long as an HTTP boot URI has also been specified,
  using the }\texttt{\DIFadd{uri}} \DIFadd{option for this driver (or e.g. in the OVMF admin screens if using OVMF, or similar options
  where present in other firmeare). If HTTP boot from static
  IP is required, then any pre-existing vendor-specific version of }\texttt{\DIFadd{HttpBootDxe.efi}} \DIFadd{will need to be unloaded
  (see }\texttt{\DIFadd{UEFI}} \texttt{\DIFadd{Unload}} \DIFadd{option) and the OpenCore version used instead.
}

  \emph{\DIFadd{Note 3}}\DIFadd{: If }\texttt{\DIFadd{Ip4Dxe}} \DIFadd{is loaded before OpenCore then any setting here will only take effect after one
  reboot. If }\texttt{\DIFadd{Ip4Dxe}} \DIFadd{is loaded after }\texttt{\DIFadd{OpenNetworkBoot}} \DIFadd{the setting will take effect immediately.
  }\medskip

  \emph{\DIFadd{Note 4}}\DIFadd{: In the majority of cases this option is not required, and the default DHCP behaviour should be
  ...
  \item \texttt{\DIFadd{uri}} \DIFadd{- String value, no default. }\medskip

  \DIFaddend If present, specify the URI to use for HTTP(S) Boot. If not present then
...
a certificate has been enrolled, it will remain enrolled even if the \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend } config
option is removed. \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-cert}\DIFdelend \DIFaddbegin \DIFadd{delete-cert}\DIFaddend } or \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-all-certs}\DIFdelend \DIFaddbegin \DIFadd{delete-all-certs}\DIFaddend }
should be used to remove enrolled certificates.

Checking for certificate presence by the \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend }
and \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-cert}\DIFdelend \DIFaddbegin \DIFadd{delete-cert}\DIFaddend } options uses the simple algorithm
of matching by exact file contents, not by file meaning. The intended
usage is to leave an \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend } option present in the config
file until it is time to delete it, e.g. after another more up-to-date
\texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend } option has been added and tested. At this point
the user can change \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend } to \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-cert}\DIFdelend \DIFaddbegin \DIFadd{delete-cert}\DIFaddend }
...
already enrolled).
Avoid combinations such as \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-all-certs}\DIFdelend \DIFaddbegin \DIFadd{delete-all-certs}\DIFaddend } followed by
\texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend }, as this will modify the NVRAM certificate
storage twice on every boot. However a combination such as
\texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-delete-cert}\DIFdelend \DIFaddbegin \DIFadd{delete-cert}\DIFaddend ="\{certA-text\}"} followed by \texttt{\DIFdelbegin \DIFdel{-}%DIFDELCMD < {}%%%
\DIFdel{-enroll-cert}\DIFdelend \DIFaddbegin \DIFadd{enroll-cert}\DIFaddend ="\{certB-text\}"}
...
When using the debug version of this driver, the OpenCore debug log includes \texttt{\DIFdelbegin \DIFdel{NTBT}\DIFdelend \DIFaddbegin \DIFadd{NETB}\DIFaddend :} entries
...
In addition to the \hyperref[uefilinux]{OpenLinuxBoot} and \DIFdelbegin %DIFDELCMD < \hyperref[uefipxe]{OpenNetworkBoot} %%%
\DIFdelend \DIFaddbegin \hyperref[uefinetboot]{OpenNetworkBoot} \DIFaddend plugins,
