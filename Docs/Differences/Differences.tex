...
%DIF ADD ../Configuration.tex        Sun Jan  3 10:04:28 2021
...
\begin{titlepage}
   \begin{center}
       ...
       Copyright \textcopyright \DIFdelbegin \DIFdel{2018-2020 }\DIFdelend \DIFaddbegin \DIFadd{2018-2021 }\DIFaddend vit9696
...
\item
  ...
  \item \DIFaddbegin \texttt{\DIFadd{ioaccel\_debug=VALUE}} \DIFadd{(}\texttt{\DIFadd{IOAccelerator}} \DIFadd{debug mask)
  }\item \DIFaddend \texttt{keepsyms=1} (show panic log debug symbols)
  ...
  \item \texttt{lapic\_dont\_panic=1} \DIFaddbegin \DIFadd{(disable lapic spurious interrupt panic on AP cores)
  }\DIFaddend \item \DIFaddbegin \texttt{\DIFadd{panic\_on\_display\_hang=1}} \DIFadd{(trigger panic on display hang)
  }\item \texttt{\DIFadd{panic\_on\_gpu\_hang=1}} \DIFadd{(trigger panic on GPU hang)
  }\item \DIFaddend \texttt{slide=VALUE} (manually set KASLR slide)
  \item \texttt{smcdebug=VALUE} (\texttt{AppleSMC} debug mask)
  \item \DIFaddbegin \texttt{\DIFadd{spin\_wait\_for\_gpu=1}} \DIFadd{(reduces GPU timeout on high load)
  }\item \DIFaddend \texttt{-amd\_no\_dgpu\_accel} (alternative to \href{https://github.com/acidanthera/WhateverGreen}{WhateverGreen}'s \texttt{-radvesa} for new GPUs)
...
\item
  \DIFdelbegin \texttt{\DIFdel{DeduplicateBootOrder}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
...
\DIFdel{: Remove duplicate entries in }\texttt{\DIFdel{BootOrder}} %DIFAUXCMD
\DIFdel{variable
  in }\texttt{\DIFdel{EFI\_GLOBAL\_VARIABLE\_GUID}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{This quirk requires }\texttt{\DIFdel{RequestBootVarRouting}} %DIFAUXCMD
\DIFdel{to be enabled and therefore
  }\texttt{\DIFdel{OC\_FIRMWARE\_RUNTIME}} %DIFAUXCMD
\DIFdel{protocol implemented in }\texttt{\DIFdel{OpenRuntime.efi}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{By redirecting }\texttt{\DIFdel{Boot}} %DIFAUXCMD
\DIFdel{prefixed variables to a separate GUID namespace
  with the help of }\texttt{\DIFdel{RequestBootVarRouting}} %DIFAUXCMD
\DIFdel{quirk we achieve multiple goals:
  }%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD <   \tightlist
%DIFDELCMD <   \item %%%
\item%DIFAUXCMD
\DIFdel{Operating systems are jailed and only controlled by OpenCore boot
  environment to enhance security.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Operating systems do not mess with OpenCore boot priority, and guarantee
  fluent updates and hibernation wakes for cases that require reboots with OpenCore
  in the middle.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Potentially incompatible boot entries, such as macOS entries, are not deleted
  or anyhow corrupted.
  }
...
%DIFDELCMD <   %%%
\DIFdel{However, some types of firmware do their own boot option scanning on startup by checking
  ...
  due to }\texttt{\DIFdel{BootOrder}} %DIFAUXCMD
\DIFdel{entry duplication (each option will be added twice) making
  it impossible to boot without resetting NVRAM.
}%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{To trigger the bug, some valid boot options (e.g. OpenCore) are required. Then
  install Windows with }\texttt{\DIFdel{RequestBootVarRouting}} %DIFAUXCMD
\DIFdel{enabled. As the Windows bootloader
...
%DIFDELCMD <   %%%
\DIFdel{This quirk removes all duplicates in }\texttt{\DIFdel{BootOrder}} %DIFAUXCMD
\DIFdel{variable attempting to resolve
  the consequences of the bugs upon OpenCore loading. It is recommended to use this key
  along with }\texttt{\DIFdel{BootProtect}} %DIFAUXCMD
\DIFdel{option.
}%DIFDELCMD < 
...
\DIFaddbegin 
  ...
  \DIFadd{By redirecting }\texttt{\DIFadd{Boot}} \DIFadd{prefixed variables to a separate GUID namespace
  with the help of }\texttt{\DIFadd{RequestBootVarRouting}} \DIFadd{quirk we achieve multiple goals:
  }\begin{itemize}
  \tightlist
