...
\begin{titlepage}
   \begin{center}
       ...
       Reference Manual \DIFaddbegin \DIFadd{(0.0.2)
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Currently this file has the following entries not completed:
}\DIFdelend 
...
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Not all }\texttt{\DIFdel{NVRAM}} %DIFAUXCMD
\DIFdel{variables are properly described (e.g. boot-args).
}
...
%DIFDELCMD < %%%
\DIFdelend \section{Generic Terms}\label{generic-terms}
...
\item
  ...
  require its presence, which is what \texttt{\DIFdelbegin \DIFdel{ProvideConsoleControl}\DIFdelend \DIFaddbegin \DIFadd{ConsoleControl}\DIFaddend } UEFI \DIFdelbegin \DIFdel{quirk }\DIFdelend \DIFaddbegin \DIFadd{protocol }\DIFaddend is for.
  ...
  (requires \texttt{\DIFdelbegin \DIFdel{ProvideConsoleControl}\DIFdelend \DIFaddbegin \DIFadd{ConsoleControl}\DIFaddend }).
  \item \texttt{ForceGraphics} --- Switch to graphics mode and preserve it
  (require \texttt{\DIFdelbegin \DIFdel{ProvideConsoleControl}\DIFdelend \DIFaddbegin \DIFadd{ConsoleControl}\DIFaddend }).
  \end{itemize}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ string}\\
  \textbf{Default value}: Empty string\\
  \textbf{Description}: Sets console output screen resolution\DIFdelbegin \DIFdel{as specified
  with the }\DIFdelend \DIFaddbegin \DIFadd{.
}

  \begin{itemize}
  \tightlist
  \item \DIFadd{Set to }\DIFaddend \texttt{WxH@Bpp} (e.g. \texttt{1920x1080@32}) \DIFdelbegin \DIFdel{or
  }\DIFdelend \texttt{WxH}
  (e.g. \texttt{1920x1080}) formatted string \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{to request custom resolution
  from GOP if available.
  }\item \DIFaddend Set to empty string not to change screen resolution.
  \DIFaddbegin \item \DIFaddend Set to \texttt{Max} to try to use largest available screen resolution.
  \DIFaddbegin \end{itemize}
\DIFaddend 

  \DIFaddbegin \DIFadd{On HiDPI screens }\texttt{\DIFadd{APPLE\_VENDOR\_VARIABLE\_GUID}} \texttt{\DIFadd{UIScale}}
  ...
  \DIFaddend \emph{Note}: This will fail when console handle has no GOP protocol. When
...
\item
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{0x00000002} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{1}}\DIFadd{) }\DIFaddend --- \texttt{DEBUG\_WARN} in \texttt{DEBUG},
      \texttt{NOOPT}, \texttt{RELEASE}.
    \item \texttt{0x00000040} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{6}}\DIFadd{) }\DIFaddend --- \texttt{DEBUG\_INFO} in \texttt{DEBUG},
      \texttt{NOOPT}.
    \item \texttt{0x00400000} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{22}}\DIFadd{) }\DIFaddend --- \texttt{DEBUG\_VERBOSE} in custom builds.
    \item \texttt{0x80000000} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{31}}\DIFadd{) }\DIFaddend --- \texttt{DEBUG\_ERROR} in \texttt{DEBUG},
      \texttt{NOOPT}, \texttt{RELEASE}.
  \end{itemize}
...
\DIFdelend \texttt{Target}\\
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{0x01} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{0}}\DIFadd{) }\DIFaddend --- Enable logging, otherwise all log is discarded.
    \item \texttt{0x02} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{1}}\DIFadd{) }\DIFaddend --- Enable basic console (onscreen) logging.
    \item \texttt{0x04} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{2}}\DIFadd{) }\DIFaddend --- Enable logging to Data Hub.
    \item \texttt{0x08} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{3}}\DIFadd{) }\DIFaddend --- Enable serial port logging.
    \item \texttt{0x10} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{4}}\DIFadd{) }\DIFaddend --- Enable UEFI variable logging.
    \item \texttt{0x20} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{5}}\DIFadd{) }\DIFaddend --- Enable non-volatile UEFI variable logging.
    \item \texttt{0x40} \DIFaddbegin \DIFadd{(bit }\texttt{\DIFadd{6}}\DIFadd{) }\DIFaddend --- Enable logging to file.
  \end{itemize}
...
\end{lstlisting}
  ...
  \DIFaddbegin \emph{\DIFadd{Warning}}\DIFadd{: Some firmwares are reported to have broken NVRAM garbage collection.
...
\DIFaddbegin \item
  ...
  \begin{itemize}
  \tightlist
    \item \texttt{\DIFadd{0x00000001}} \DIFadd{(bit }\texttt{\DIFadd{0}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_FILE\_SYSTEM\_LOCK}}\DIFadd{, restricts
    ...
    }\item \texttt{\DIFadd{0x00000002}} \DIFadd{(bit }\texttt{\DIFadd{1}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_DEVICE\_LOCK}}\DIFadd{, restricts scanning
    ...
    }\item \texttt{\DIFadd{0x00040000}} \DIFadd{(bit }\texttt{\DIFadd{18}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_SCSI}}\DIFadd{, allow
    scanning SCSI devices.
    }\item \texttt{\DIFadd{0x00080000}} \DIFadd{(bit }\texttt{\DIFadd{19}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_NVME}}\DIFadd{, allow
    scanning NVMe devices.
    }\item \texttt{\DIFadd{0x00100000}} \DIFadd{(bit }\texttt{\DIFadd{20}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_ATAPI}}\DIFadd{, allow
    scanning CD/DVD devices.
    }\item \texttt{\DIFadd{0x00200000}} \DIFadd{(bit }\texttt{\DIFadd{21}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_USB}}\DIFadd{, allow
    scanning USB devices.
    }\item \texttt{\DIFadd{0x00400000}} \DIFadd{(bit }\texttt{\DIFadd{22}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_FIREWIRE}}\DIFadd{, allow
    scanning FireWire devices.
    }\item \texttt{\DIFadd{0x00800000}} \DIFadd{(bit }\texttt{\DIFadd{23}}\DIFadd{) --- }\texttt{\DIFadd{OC\_SCAN\_ALLOW\_DEVICE\_SDCARD}}\DIFadd{, allow
...
\item
  ...
  e.g. \texttt{ru-RU:\DIFdelbegin \DIFdel{19456}\DIFdelend \DIFaddbegin \DIFadd{252}\DIFaddend } for \DIFdelbegin \DIFdel{Mac }\DIFdelend \DIFaddbegin \DIFadd{Russian locale and ABC }\DIFaddend keyboard. Also accepts short forms:
  \texttt{ru:\DIFdelbegin \DIFdel{19456}\DIFdelend \DIFaddbegin \DIFadd{252}\DIFaddend } or \texttt{ru:0} \DIFdelbegin \DIFdel{. Full decoded list of keyboards in }\DIFdelend \DIFaddbegin \DIFadd{(U.S. keyboard, compatible with 10.9). Full decoded
  keyboard list from }\DIFaddend \texttt{AppleKeyboardLayouts-L.dat} can be found on
  \href{https://www.applelife.ru/posts/763537}{AppleLife}. \DIFaddbegin \DIFadd{Using non-latin keyboard on 10.14
...
\item
  ...
  \DIFaddbegin \DIFadd{Some of the known boot arguments include:
}\DIFaddend 

  \begin{itemize}
  \item \DIFdelbegin \DIFdel{FIXME: document several known values! debug, keepsyms, slide, -v, -s,
  -x, cpus=x, io=x, kextlog=x, -nehalem\_error\_disable -no\_compat\_check nvda\_drv=1, etc?
  }\DIFdelend \DIFaddbegin \texttt{\DIFadd{acpi\_layer=0xFFFFFFFF}}
  \item \texttt{\DIFadd{acpi\_level=0xFFFF5F}} \DIFadd{(implies
    }\href{https://github.com/acpica/acpica/blob/master/source/include/acoutput.h}
    {\texttt{\DIFadd{ACPI\_ALL\_COMPONENTS}}}\DIFadd{)
  }\item \texttt{\DIFadd{cpus=VALUE}}
  \item \texttt{\DIFadd{debug=VALUE}}
  \item \texttt{\DIFadd{io=VALUE}}
  \item \texttt{\DIFadd{keepsyms=1}}
  \item \texttt{\DIFadd{kextlog=VALUE}}
  \item \texttt{\DIFadd{nvda\_drv=1}}
  \item \texttt{\DIFadd{slide=VALUE}}
  \item \texttt{\DIFadd{-nehalem\_error\_disable}}
  \item \texttt{\DIFadd{-no\_compat\_check}}
  \item \texttt{\DIFadd{-s}}
  \item \texttt{\DIFadd{-v}}
  \item \texttt{\DIFadd{-x}}
...
\item
  ...
  \DIFaddbegin \break
  \DIFadd{Current saved host name. ASCII string.
}\DIFaddend \item
  \texttt{7C436110-AB2A-4BBB-A880-FE41995C9F82:nvda\_drv}
  \DIFaddbegin \break
  \DIFadd{NVIDIA Web Driver control variable. Takes ASCII digit }\texttt{\DIFadd{1}} \DIFadd{or }\texttt{\DIFadd{0}}
  \DIFadd{to enable or disable installed driver.
}\DIFaddend \end{itemize}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Default value}: \texttt{false}\\
  \textbf{Description}: Update \DIFdelbegin \DIFdel{DataHub }\DIFdelend \DIFaddbegin \DIFadd{Data Hub }\DIFaddend fields. These fields are read
...
\item
  ...
  \item \DIFdelbegin %DIFDELCMD < \href{https://github.com/tianocore/edk2/tree/UDK2018}{\texttt{DataHubDxe}}
%DIFDELCMD <   %%%
\DIFdel{--- Intel DataHub driver from }\texttt{\DIFdel{IntelFrameworkModulePkg}}%DIFAUXCMD
\DIFdel{. It is believed
  that this driver is included in all UEFI firmwares, and is not required.
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \href{https://github.com/tianocore/edk2/tree/UDK2018}{\texttt{EmuVariableRuntimeDxe}}
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Default value}: \texttt{false}\\
  \textbf{Description}: Reinstalls \DIFdelbegin \DIFdel{apple boot policy }\DIFdelend \DIFaddbegin \DIFadd{Apple Boot Policy }\DIFaddend protocol with a builtin
  version. This may be used to ensure APFS compatibility on VMs or legacy Macs.

\item
  \DIFaddbegin \texttt{\DIFadd{ConsoleControl}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Default value}}\DIFadd{: }\texttt{\DIFadd{NO}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Replaces Console Control protocol with a builtin version.
}

  \DIFadd{macOS bootloader requires console control protocol for text output, which some firmwares
  miss. This option is required to be set when the protocol is already available in the
  firmware, and other console control options are used, such as }\texttt{\DIFadd{IgnoreTextInGraphics}}\DIFadd{,
  }\texttt{\DIFadd{SanitiseClearScreen}}\DIFadd{, and sometimes }\texttt{\DIFadd{ConsoleBehaviourOs}} \DIFadd{with
  }\texttt{\DIFadd{ConsoleBehaviourUi}}\DIFadd{).
}

\item
  \texttt{\DIFadd{DataHub}}\\
  \textbf{\DIFadd{Type}}\DIFadd{: }\texttt{\DIFadd{plist\ boolean}}\\
  \textbf{\DIFadd{Default value}}\DIFadd{: }\texttt{\DIFadd{false}}\\
  \textbf{\DIFadd{Description}}\DIFadd{: Reinstalls Data Hub protocol with a builtin version.
  This will drop all previous properties if the protocol was already installed.
}

\item
  \DIFaddend \texttt{DeviceProperties}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Default value}: \texttt{false}\\
  \textbf{Description}: Reinstalls \DIFdelbegin \DIFdel{device property }\DIFdelend \DIFaddbegin \DIFadd{Device Property }\DIFaddend protocol with a builtin
...
\item
  ...
  onscreen error messages. \texttt{\DIFdelbegin \DIFdel{ProvideConsoleControl}\DIFdelend \DIFaddbegin \DIFadd{ConsoleControl}\DIFaddend } \DIFdelbegin \DIFdel{is required }\DIFdelend \DIFaddbegin \DIFadd{may need }\DIFaddend to be set to
  \texttt{true} for this to work.

\item
  \DIFdelbegin \texttt{\DIFdel{ProvideConsoleControl}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Type}}%DIFAUXCMD
\DIFdel{: }\texttt{\DIFdel{plist\ boolean}}%DIFAUXCMD
...
%DIFDELCMD <   %%%
\textbf{\DIFdel{Default value}}%DIFAUXCMD
\DIFdel{: }\texttt{\DIFdel{NO}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <   %%%
\textbf{\DIFdel{Description}}%DIFAUXCMD
\DIFdel{: macOS bootloader requires console control protocol
  for text output, some firmwares miss it. This option is required to be set
  when other console control options are used, such as }\texttt{\DIFdel{IgnoreTextInGraphics}}%DIFAUXCMD
\DIFdel{,
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend \texttt{ProvideConsoleGop}\\
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Default value}: \texttt{NO}\\
...
\item
  ...
  \emph{Note}: \texttt{\DIFdelbegin \DIFdel{ProvideConsoleControl}\DIFdelend \DIFaddbegin \DIFadd{ConsoleControl}\DIFaddend } \DIFdelbegin \DIFdel{is required }\DIFdelend \DIFaddbegin \DIFadd{may need }\DIFaddend to be set to
