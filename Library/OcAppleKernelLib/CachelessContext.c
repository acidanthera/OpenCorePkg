  ...
  )
{
  ...
  do {
    ...
    if (FileInfoSize > 0) {
      if (StrCmp (&FileInfo->FileName[StrLen (FileInfo->FileName) - 5], L".kext") == 0) {
  ...
  )
{
  ...
  // Info.plist.
  //
  if (StrLen (FileName) > L_STR_LEN(L"Info.plist")
    && StrCmp (&FileName[StrLen (FileName) - L_STR_LEN(L"Info.plist")], L"Info.plist") == 0) {
