  ...
  )
{
  ...
  if (!CustomEntry->UnmanagedBootAction && !CustomEntry->SystemAction && !CustomEntry->UnmanagedBootDevicePath) {
  ...
  if (CustomEntry->UnmanagedBootAction) {
    ...
    if (IsBootEntryProtocol) {
      if (CustomEntry->UnmanagedBootDevicePath) {
        BootEntry->DevicePath = DuplicateDevicePath (CustomEntry->UnmanagedBootDevicePath);
      } else {
        UnicodeUefiSlashes (PathName);
      ...
      ASSERT (CustomEntry->UnmanagedBootDevicePath == NULL);
