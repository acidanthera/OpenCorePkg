...
#define ISO_CHAR_BPP    32

STATIC UINT8 mIsoFontData[(ISO_CHAR_MAX - ISO_CHAR_MIN + 1)*ISO_CHAR_HEIGHT] = {
...
/**
  Render character to ISO_CHAR_WIDTH*ISO_CHAR_HEIGHT buffer.
  ...
  )
{
  ...
  //
  // TODO: Support HiDPI rendering based on UIScale.
  //

  if ((Char >= 0 && Char < ISO_CHAR_MIN) || Char == 0x7F) {
    SetMem32 (Buffer, ISO_CHAR_HEIGHT * ISO_CHAR_WIDTH * sizeof (UINT32), BgColor);
    return;
  }
  ...
  DstBuffer = Buffer;
  SrcBuffer = mIsoFontData + ((Char - ISO_CHAR_MIN) * ISO_CHAR_HEIGHT);
  ...
  )
{
  ...
  if (Info->HorizontalResolution < ISO_CHAR_WIDTH * 3
    || Info->VerticalResolution < ISO_CHAR_HEIGHT * 3
    || (Info->PixelFormat != PixelRedGreenBlueReserved8BitPerColor
      && Info->PixelFormat != PixelBlueGreenRedReserved8BitPerColor)) {
    return EFI_DEVICE_ERROR;
  }

  mConsoleWidth            = (Info->HorizontalResolution / ISO_CHAR_WIDTH) - 2;
  mConsoleHeight           = (Info->VerticalResolution / ISO_CHAR_HEIGHT) - 2;
...
{
  UINTN                          Index;
  EFI_GRAPHICS_OUTPUT_BLT_PIXEL  Buffer[ISO_CHAR_WIDTH * ISO_CHAR_HEIGHT];
  ...
  for (Index = 0; String[Index] != '\0'; ++Index) {
    ...
    if ((UINTN) This->Mode->CursorRow >= mConsoleHeight) {
      ...
      mGraphicsOutput->Blt (
        ...
        ISO_CHAR_WIDTH,
        ISO_CHAR_HEIGHT,
        MIN (mConsoleMaxPosX * ISO_CHAR_WIDTH, mGraphicsOutput->Mode->Info->HorizontalResolution - ISO_CHAR_WIDTH),
        MIN ((mConsoleMaxPosY + 1) * ISO_CHAR_HEIGHT, mGraphicsOutput->Mode->Info->VerticalResolution - ISO_CHAR_HEIGHT),
        0
        );
    ...
    RenderChar (String[Index], mForegroundColor.Raw, mBackgroundColor.Raw, Buffer);
    ...
    mGraphicsOutput->Blt (
      ...
      0,
      0,
      ISO_CHAR_WIDTH + This->Mode->CursorColumn * ISO_CHAR_WIDTH,
      ISO_CHAR_HEIGHT + This->Mode->CursorRow * ISO_CHAR_HEIGHT,
      ISO_CHAR_WIDTH,
      ISO_CHAR_HEIGHT,
      0
      );
  ...
  )
{
  ...
  if (mConsoleMaxPosX > 0) {
    mGraphicsOutput->Blt (
      ...
      0,
      0,
      MIN ((mConsoleMaxPosX + 1) * ISO_CHAR_WIDTH, mGraphicsOutput->Mode->Info->HorizontalResolution),
      MIN ((mConsoleMaxPosY + 2) * ISO_CHAR_HEIGHT, mGraphicsOutput->Mode->Info->VerticalResolution),
      0
      );
  }
