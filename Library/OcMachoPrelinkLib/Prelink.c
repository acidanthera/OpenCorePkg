  ...
  )
{
  ...
  if (Relocation->Extern != 0) {
    Success = MachoGetSymbolByIndex64 (
                MachoContext,
    ...
    Success = MachoGetSectionByIndex64 (
                MachoContext,
                Relocation->SymbolNumber,
                &Section
                );
    if (!Success || (Section == NULL)) {
      return FALSE;
    }
  }
  ...
  )
{
  ...
  FirstSegment = NULL;
  Result = MachoGetNextSegment64 (MachoContext, &FirstSegment);
  if (!Result) {
    return FALSE;
  }
  //
  ...
  Segment = NULL;
  while (MachoGetNextSegment64 (MachoContext, &Segment)) {
    if (Segment == NULL) {
      //
    ...
    Section = NULL;
    while (MachoGetNextSection64 (MachoContext, Segment, &Section)) {
      if (Section == NULL) {
        Result = TRUE;
        break;
      }
