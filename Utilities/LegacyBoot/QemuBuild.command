...
if [[ ! $(diskutil info disk"${N}" | sed -n 's/.*Device Node: *//p') ]]; then
...
if [ "$(diskutil info disk"${N}" | sed -n 's/.*Content (IOContent): *//p')" == "FDisk_partition_scheme" ]; then
